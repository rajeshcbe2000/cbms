 ...
 **
 *
 * SuspenseAccountProductDAO.java
 *
...
/**
 * SuspenseAccountProduct DAO.
 *
 */
public class SuspenseAccountProductDAO extends TTDAO {

    private static SqlMap sqlMap = null;
    private SuspenseAccountProductTO objTO;
    ...
    /**
     * Creates a new instance of SuspenseAccountProductDAO
     */
    public SuspenseAccountProductDAO() throws ServiceLocatorException {
    ...
    private HashMap getData(HashMap map) throws Exception {
        ...
        List list = (List) sqlMap.executeQueryForList("getSelectSuspenseAccountProductTO", where);
        returnMap.put("SuspenseAccountProductTO", list);
        return returnMap;
    }
...
////        objTO.setTxtSuspenseActNum(suspenseActNum);
    ...
    private void insertData() throws Exception {
        try {
            ...
            sqlMap.executeUpdate("insertSuspenseAccountProduct", objTO);
    ...
    private void updateData() throws Exception {
        try {
            ...
            objTO.setStatusBy(userID);
            objTO.setStatusDt(currDt);
            sqlMap.executeUpdate("updateSuspenseAccountProduct", objTO);
            sqlMap.commitTransaction();
        } catch (SQLException e) {
            sqlMap.rollbackTransaction();
            e.printStackTrace();
    ...
    private void deleteData() throws Exception {
        try {
            ...
            objTO.setStatusBy(userID);
            objTO.setStatusDt(currDt);
            sqlMap.executeUpdate("deleteSuspenseAccountProduct", objTO);
            sqlMap.commitTransaction();
        } catch (SQLException e) {
            sqlMap.rollbackTransaction();
            e.printStackTrace();
    ...
    public static void main(String str[]) {
        try {
            SuspenseAccountProductDAO dao = new SuspenseAccountProductDAO();
        } catch (Exception ex) {
            ex.printStackTrace();
        }
    }
    ...
    public HashMap execute(HashMap map) throws Exception {
        objTO = (SuspenseAccountProductTO) map.get("SuspenseAccountProductTO");
    ...
    public HashMap executeQuery(HashMap condition) throws Exception {
        ...
        if (!condition.containsKey("AUTH_DATA")) {
            System.out.println("###whereconditions  SuspenseAccountProductTO" + whereConditions);
            getSuspenseProductData();
            makeNull();
        } else if (condition.containsKey("AUTH_DATA")) {
            System.out.println("@#$@#$@#$condition:" + condition);
            sqlMap.executeUpdate("authorizeSuspenseAccountProduct", condition);
        }
        return data;
    ...
    private void getSuspenseProductData() throws Exception {
        if (data == null) {
            data = new HashMap();
        }
        whereConditions.put("CURRENT_DT", currDt);
        System.out.println("#@$@#$whereConditions:" + whereConditions);
        List list = (List) sqlMap.executeQueryForList("getSelectSuspenseProductTO", whereConditions);
        System.out.println("@#$@#$@#4list :" + list);
        SuspenseAccountProductTO objSuspenseAccountProductTO = new SuspenseAccountProductTO();
        if (list.size() > 0) {
            objSuspenseAccountProductTO = (SuspenseAccountProductTO) list.get(0);
            data.put("SuspenseAccountProductTO", objSuspenseAccountProductTO);
        }
