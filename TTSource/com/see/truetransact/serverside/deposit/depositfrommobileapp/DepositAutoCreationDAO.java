 ...
 */
package com.see.truetransact.serverside.deposit.depositfrommobileapp;
...
import com.see.truetransact.serverside.share.ShareDAO;
 ...
 *
 */
public class DepositAutoCreationDAO extends TTDAO {
...
//    private LinkedHashMap deletedCaseDetailMap = null;
     ...
     */
    public DepositAutoCreationDAO() throws ServiceLocatorException {
...
private void updateVehicleTableDetails() throws Exception {
    ...
    private void updateDepositTypeDetails(String loanNo) throws Exception {
        System.out.println("inside updateDepositTypeDetails ::  deletedDepositTableValues " + deletedDepositTableValues);
...
//                    objTermLoanDepositTypeTO.setAcctNum(acct_No);
                    ...
                    objTermLoanDepositTypeTO.setStatus("CREATED");
                    objTermLoanDepositTypeTO.setStatusBy(userId);
                    objTermLoanDepositTypeTO.setStatusDt(curr_dt);                   
                    ...
                    getActNoMap.put("USER_ID",userId);
                    getActNoMap.put("AUTHORIZEDT",curr_dt);
                    getActNoMap.put("ACCT_NUM",loanNo);
                    ...
                    sqlMap.executeUpdate("updateTermLoanDepositTypeTO", objTermLoanDepositTypeTO);
                }
            }
        }
        if (deletedDepositTableValues != null && deletedDepositTableValues.size() > 0) {
            ...
            TermLoanFacilityTO objTermLoanFacilityTO = null;;
            Set keySet;
            Object[] objKeySet;
            ...
            for (int i = facilityMap.size() - 1, j = 0; i >= 0; --i, ++j) {
                ...
                if (objTermLoanFacilityTO.getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
                    ...
                    if (objTermLoanFacilityTO.getTxtValueOfGold() != null && objTermLoanFacilityTO.getTxtValueOfGold() > 0) {
                        ...
                        objTermLoanSecurityTO.setGrossWeight(CommonUtil.convertObjToDouble(objTermLoanFacilityTO.getTxtGrossWeight()));
                        objTermLoanSecurityTO.setNetWeight(CommonUtil.convertObjToDouble(objTermLoanFacilityTO.getTxtNetWeight()));
...
//                        mdsUpdateMap.put("CHITTAL_NO", CommonUtil.convertObjToStr(depositCustDetMap.get("DEPOSIT_NO")));
                        ...
                        mdsUpdateMap.put("SUB_NO", CommonUtil.convertObjToInt(subNo2));
...
//                    executeUpdate("updateMDSLoanStatus", mdsUpdateMap);
                ...
                if (multipleDeposit != null && objTermLoanFacilityTO.getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
                    ...
                    for (int z = 0; z < multipleDeposit.size(); z++) {
                        ...
                        obj.setDepositSubNo(CommonUtil.convertObjToInt((1)));
                    ...
                    if (updateLien != null) {
                        ...
                        for (int t = updateLien.size() - 1, u = 0; t >= 0; --t, ++u) {
                            ...
                            if (CommonUtil.convertObjToStr(updateMap.get("STATUS")).equals("CREATED")) {
                                ...
                                depLienTO.setCreditLienAcct("NA");
                                depLienTO.setDepositNo(CommonUtil.convertObjToStr(updateMap.get("DEPOSIT_NO")));
                                depLienTO.setDepositSubNo(CommonUtil.convertObjToInt((1)));
                                depLienTO.setLienProdId(objTermLoanFacilityTO.getProdId());
                                depLienTO.setLienNo("-");
                                depLienTO.setStatus(objTermLoanFacilityTO.getStatus());
                                depLienTO.setStatusBy(objTermLoanFacilityTO.getStatusBy());
...
//                double newAmount = CommonUtil.convertObjToDouble(mdsReceiptEntryTO.getNetAmt()).doubleValue();
    ...
    private void updateSMSSubscription(TermLoanFacilityTO objTermLoanFacilityTO) throws Exception {
        ...
        if (CommonUtil.convertObjToStr(objSMSSubscriptionTO.getMobileNo()) != null &&
                CommonUtil.convertObjToStr(objSMSSubscriptionTO.getMobileNo()).length() > 0 && objSMSSubscriptionTO.getSubscriptionDt() != null) {
...
//        cashAuthMap.put("DAILY", "DAILY");
    ...
    public HashMap execute(HashMap map) throws Exception {
        HashMap returnMap = new HashMap();
        List CustCreationList ;
        List shareOpeningList;
        List depositOpeningList;
        List sbOpeningList;
        ...
        System.out.print("inside DepositAutoCreationDAO#######" + map);
        ...
        if(map.containsKey("OPEN_CUSTOMER_ID") && map.get("OPEN_CUSTOMER_ID") != null && map.get("OPEN_CUSTOMER_ID").equals("OPEN_CUSTOMER_ID")){
            if(map.containsKey("CUSTOMER_CREATION_LIST") && map.get("CUSTOMER_CREATION_LIST") != null){
                CustCreationList = (List) map.get("CUSTOMER_CREATION_LIST");
                returnMap = createCustomer(CustCreationList);
            }
        }      
        
       if(map.containsKey("OPEN_SHARE_ACT") && map.get("OPEN_SHARE_ACT") != null && map.get("OPEN_SHARE_ACT").equals("OPEN_SHARE_ACT")){
            if(map.containsKey("SHARE_OPENING_LIST") && map.get("SHARE_OPENING_LIST") != null){
                shareOpeningList = (List) map.get("SHARE_OPENING_LIST");
                returnMap = openShareForCustomers(shareOpeningList);
            }
        }  
       
       if(map.containsKey("OPEN_DEPOSIT_ACT") && map.get("OPEN_DEPOSIT_ACT") != null && map.get("OPEN_DEPOSIT_ACT").equals("OPEN_DEPOSIT_ACT")){
            if(map.containsKey("DEPOSIT_OPENING_LIST") && map.get("DEPOSIT_OPENING_LIST") != null){
                depositOpeningList = (List) map.get("DEPOSIT_OPENING_LIST");
                returnMap = openDepositForCustomers(depositOpeningList);
            }
        } 
       
        if(map.containsKey("OPEN_SB_ACT") && map.get("OPEN_SB_ACT") != null && map.get("OPEN_SB_ACT").equals("OPEN_SB_ACT")){
            if(map.containsKey("SB_OPENING_LIST") && map.get("SB_OPENING_LIST") != null){
                sbOpeningList = (List) map.get("SB_OPENING_LIST");
                returnMap = openSBForCustomers(sbOpeningList);
            }
        } 
    ...
    private HashMap createCustomer(List custCreationList) throws SQLException {
        HashMap returnMap = new HashMap();
        try {
            if (custCreationList != null && custCreationList.size() > 0) {
                for (int i = 0; i < custCreationList.size(); i++) {
                    HashMap custDataMap = (HashMap) custCreationList.get(i);
                    ...
                    if (null != custDataMap && custDataMap.size() > 0) {
                        ...
                        while (iterate.hasNext()) {
                            ...
                            HashMap custMap = (HashMap) value;
                            CustomerDAO dao = new CustomerDAO();
                            custMap.put("FROM_MOBILE_APP_CUST_CREATION", "FROM_MOBILE_APP_CUST_CREATION");
                            HashMap custReturnMap = dao.execute(custMap);  
                            String mobileDepId = CommonUtil.convertObjToStr(key1);
                            custReturnMap.put("MOB_DEP_ID", mobileDepId);
                            sqlMap.executeUpdate("updateCustomerIdInMobileAppData", custReturnMap);
                            sqlMap.executeUpdate("updateCustomerPhotoInMobileAppData", custReturnMap);
                            if(i == 0){
                                returnMap.put("CUST_CREATE_STATUS", "SUCCESS");
                                returnMap.put("FROM",custReturnMap.get("CUST_ID"));
                            }else if( i == custCreationList.size() -1){
                                returnMap.put("TO",custReturnMap.get("CUST_ID"));
                                returnMap.put("CUST_CREATE_STATUS", "SUCCESS");
                            }
                        }
                    }                    
    ...
    private HashMap openShareForCustomers(List shareOpeningList) throws SQLException {
        System.out.println("Inside openShareForCustomers");
        String mobileDepId = "";
        HashMap returnMap = new HashMap();
        try {
            if (shareOpeningList != null && shareOpeningList.size() > 0) {
                for (int i = 0; i < shareOpeningList.size(); i++) {
                    HashMap shareDataMap = (HashMap) shareOpeningList.get(i);
                    Iterator iterate = shareDataMap.entrySet().iterator();
                    if (null != shareDataMap && shareDataMap.size() > 0) {
                        ArrayList list = new ArrayList();
                        while (iterate.hasNext()) {
                            String shareNo = "";
                            List transferTransList = new ArrayList();
                            HashMap transDetailMap =  new HashMap();
                            Map.Entry entry = (Map.Entry) iterate.next();
                            Object key1 = (Object) entry.getKey();
                            Object value = (Object) entry.getValue();
                            HashMap singleShareMap = (HashMap)value;                            
                            ShareAccInfoTO objShareAccInfoTO = (ShareAccInfoTO)singleShareMap.get("ShareAccInfo");                            
                            if(objShareAccInfoTO.getTxtRatification().equals("Y")){ 
                               shareNo = getShareApplnNo(CommonUtil.convertObjToStr(singleShareMap.get("SHARE_TYPE"))); 
                               objShareAccInfoTO.setTxtApplicationNo(shareNo);
                               objShareAccInfoTO.setShareAcctNo(null);
                            }else{
                               shareNo = getShareNo(CommonUtil.convertObjToStr(singleShareMap.get("SHARE_TYPE")));
                               objShareAccInfoTO.setShareAcctNo(shareNo); 
                            }                                                      
                            double totalShares = CommonUtil.convertObjToDouble(singleShareMap.get("NO_OF_SHARES"));
                            HashMap shareNoAmtMap = new HashMap();
                            shareNoAmtMap.put("AMOUNT", singleShareMap.get("TOTAL_AMOUNT"));
                            transDetailMap.put(shareNo+"_0", shareNoAmtMap);
                            singleShareMap.put("ShareTransDetails", transDetailMap);
                            singleShareMap.put("ShareAccInfo",objShareAccInfoTO);
                            System.out.println("map to share DAO :: " + singleShareMap);
                            ShareDAO dao = new ShareDAO();
                            singleShareMap.put("FROM_MOBILE_APP_CUST_CREATION","FROM_MOBILE_APP_CUST_CREATION");
                            singleShareMap.put(CommonConstants.AUTHORIZEMAP, getShareAuthorizeMap(singleShareMap,shareNo, CommonUtil.convertObjToStr(singleShareMap.get("SHARE_TYPE")), totalShares,objShareAccInfoTO.getTxtRatification()));
                           try{ 
                            HashMap shareReturnMap = dao.execute(singleShareMap);  
                            System.out.println("shareReturnMap :: " + shareReturnMap);  
                            if (shareReturnMap != null && shareReturnMap.containsKey("TRANSFER_TRANS_LIST")) {
                                transferTransList = (ArrayList) shareReturnMap.get("TRANSFER_TRANS_LIST");
                                if (transferTransList != null && transferTransList.size() > 0) {
                                    mobileDepId = CommonUtil.convertObjToStr(key1);
                                    shareReturnMap.put("MOB_DEP_ID", mobileDepId);
                                    if(objShareAccInfoTO.getTxtRatification().equals("Y")){
                                        //shareNo = CommonUtil.convertObjToStr(shareReturnMap.get("TRANS_ID"));
                                        shareReturnMap.put("SHARE_NO",shareNo);
                                    }
                                    sqlMap.executeUpdate("updateShareNoInMobileAppData", shareReturnMap);
                                    if(objShareAccInfoTO.getTxtRatification().equals("N")){
                                      getShareAcct_No(CommonUtil.convertObjToStr(singleShareMap.get("SHARE_TYPE")));
                                    }
                                    if (i == 0) {
                                        returnMap.put("SHARE_OPENING_STATUS", "SUCCESS");
                                        returnMap.put("FROM", shareNo);
                                    } else if (i == shareOpeningList.size() - 1) {
                                        returnMap.put("TO", shareNo);
                                        returnMap.put("SHARE_OPENING_STATUS", "SUCCESS");
                                    }
                                }
                            } 
                                ...
                                if (e instanceof TTException) {
                                    ...
                                    exceptionHashMap.put("MOB_DEP_NO", key1);
                                    throw e;
                                } else {
    ...
    private HashMap getShareAuthorizeMap(HashMap singleShareMap,String shareNo, String shareType, double totlaNoOfShares, String ratification) {
        String shareAcctNo = "";
        String shareApplnNo = "";
        HashMap authMap = new HashMap();
        //TRANS_TYPE,OUTSTANDING_AMOUNT,AVILABLE_NO_SHARES,USER_ID,AUTHORIZESTATUS,CASHIER_AUTH_ALLOWED,APPLN_DT,SHARE_ACCT_NO
        if(ratification.equals("Y")){
            shareAcctNo = null;
            shareApplnNo = shareNo;
        }else{
            shareAcctNo = shareNo;
            shareApplnNo = null;
        }
        authMap.put("TRANS_TYPE", shareType);
        authMap.put("OUTSTANDING_AMOUNT", CommonUtil.convertObjToDouble(singleShareMap.get("TOTAL_AMOUNT")));
        authMap.put("AVILABLE_NO_SHARES", totlaNoOfShares);
        authMap.put("USER_ID", logTO.getUserId());
        authMap.put("AUTHORIZESTATUS", "AUTHORIZED");
        authMap.put("CASHIER_AUTH_ALLOWED", "N");
        authMap.put("APPLN_DT", curr_dt);
        authMap.put("SHARE_ACCT_NO", shareAcctNo);
        HashMap singleAuthorizeMap = new HashMap();
        ArrayList arrList = new ArrayList();
        HashMap authDataMap = new HashMap();
        HashMap listMap = new HashMap();
        authDataMap.put(CommonConstants.USER_ID, logTO.getUserId());
        authDataMap.put("SHARE ACCOUNT NO", shareAcctNo);
        authDataMap.put("SHARE APPLICATION NO",shareApplnNo);
        authDataMap.put("CASHIER_AUTH_ALLOWED", "N");
        authDataMap.put("SHARE DETAIL NO", "1");
        authDataMap.put("SHARE TYPE", shareType);
        arrList.add(authDataMap);
        authMap.put("AUTHORIZEDATA", arrList);
        return authMap;
    }
    
    
     private void getShareAcct_No(String shareType) throws Exception {     
        HashMap where = new HashMap();        
    ...
    private String getShareNo(String shareType) throws SQLException{
        String shareNo = "";
        HashMap hmap=new HashMap();
        hmap.put("SHARE_TYPE",shareType);
        List list=sqlMap.executeQueryForList("getMinIntialShares", hmap);        
        if( list != null && list.size()>0){
            hmap=(HashMap)list.get(0);           
            ...
            shareNo=prefix+suffix;
        }
        return shareNo;
    }
    
    
     private String getShareApplnNo(String shareType) throws SQLException, ServiceLocatorException, Exception{       
        final IDGenerateDAO dao = new IDGenerateDAO();
        final HashMap where = new HashMap();
   ...
   private HashMap openDepositForCustomers(List depositOpeningList) throws SQLException {
        System.out.println("Inside openDepositForCustomers");
        String mobileDepId = "";
        HashMap returnMap = new HashMap();
        try {
            if (depositOpeningList != null && depositOpeningList.size() > 0) {
                System.out.println("depositOpeningList :: " + depositOpeningList);
                for (int i = 0; i < depositOpeningList.size(); i++) {
                    HashMap shareDataMap = (HashMap) depositOpeningList.get(i);
                    Iterator iterate = shareDataMap.entrySet().iterator();
                    if (null != shareDataMap && shareDataMap.size() > 0) {
                        ArrayList list = new ArrayList();
                        while (iterate.hasNext()) {
                            ...
                            Object key1 = (Object) entry.getKey();
                            Object value = (Object) entry.getValue();
                            HashMap singleDepositMap = (HashMap)value;                           
                            TermDepositDAO dao = new TermDepositDAO();
                            singleDepositMap.put("FROM_MOBILE_APP_CUST_CREATION","FROM_MOBILE_APP_CUST_CREATION");
                            try {
                                HashMap depositReturnMap = dao.execute(singleDepositMap);
                                System.out.println("depositReturnMap :: " + depositReturnMap);
                                if (depositReturnMap.containsKey("DEPOSIT NO") && depositReturnMap.get("DEPOSIT NO") != null) {
                                    mobileDepId = CommonUtil.convertObjToStr(key1);
                                    depositReturnMap.put("MOB_DEP_ID", mobileDepId);
                                    sqlMap.executeUpdate("updateBankDepositCreationStatus", depositReturnMap);
                                    if (i == 0) {
                                        returnMap.put("DEPOSIT_OPENING_STATUS", "SUCCESS");
                                        returnMap.put("FROM", depositReturnMap.get("DEPOSIT NO"));
                                    } else if (i == depositOpeningList.size() - 1) {
                                        returnMap.put("TO", depositReturnMap.get("DEPOSIT NO"));
                                        returnMap.put("DEPOSIT_OPENING_STATUS", "SUCCESS");
                                    }
                                }
                            } catch (Exception e) {
...
//                    hashMap.put("TODAY_DT", paramMap.get("TODAY_DT"));
 ...
 private void insertServiceTaxDetails(ServiceTaxDetailsTO objserviceTaxDetailsTO) {
  ...
  private String getServiceTaxNo() throws Exception {
    ...
    private void deleteSMSSubscription(TermLoanFacilityTO objTermLoanFacilityTO) throws Exception {
        HashMap checkMap = new HashMap();
        checkMap.put("PROD_TYPE", "TL");
        checkMap.put("PROD_ID", objTermLoanFacilityTO.getProdId());
        checkMap.put("ACT_NUM", objTermLoanFacilityTO.getAcctNum());
        checkMap.put(CommonConstants.USER_ID, objTermLoanFacilityTO.getStatusBy());
        sqlMap.executeUpdate("deleteSMSSubscriptionMap", checkMap);
        checkMap.clear();
        checkMap = null;
    }
    
    private HashMap openSBForCustomers(List sbOpeningList) throws SQLException {
        HashMap returnMap = new HashMap();
        String mobileDepId = "";
        System.out.println("sbOpeningList :: " + sbOpeningList);
        try {
            if (sbOpeningList != null && sbOpeningList.size() > 0) {
                System.out.println("sbOpeningList :: " + sbOpeningList);
                System.out.println("sbOpeningList size :: " + sbOpeningList.size());
                for (int i = 0; i < sbOpeningList.size(); i++) {
                    HashMap sbDataMap = (HashMap) sbOpeningList.get(i);
                    Iterator iterate = sbDataMap.entrySet().iterator();
                    if (sbDataMap != null && sbDataMap.size() > 0) {
                        while (iterate.hasNext()) {
                            Map.Entry entry = (Map.Entry) iterate.next();
                            Object key1 = (Object) entry.getKey();
                            Object value = (Object) entry.getValue();
                            HashMap singleSBMap = (HashMap)value;                           
                            AccountDAO dao = new AccountDAO();
                            singleSBMap.put("FROM_MOBILE_APP_CUST_CREATION","FROM_MOBILE_APP_CUST_CREATION");
                            try {
                                HashMap sbReturnMap = dao.execute(singleSBMap);
                                System.out.println("SBReturnMap :: " + sbReturnMap);
                                if (sbReturnMap.containsKey("TRANS_ID") && sbReturnMap.get("TRANS_ID") != null) {
                                    mobileDepId = CommonUtil.convertObjToStr(key1);
                                    sbReturnMap.put("MOB_DEP_ID", mobileDepId);
                                    sqlMap.executeUpdate("updateBankDepositCreationStatus", sbReturnMap);
                                    if (i == 0) {
                                        returnMap.put("SB_OPENING_STATUS", "SUCCESS");
                                        returnMap.put("FROM", sbReturnMap.get("TRANS_ID"));
                                    } else if (i == sbOpeningList.size() - 1) {
                                        returnMap.put("TO", sbReturnMap.get("TRANS_ID"));
                                        returnMap.put("SB_OPENING_STATUS", "SUCCESS");
                                    }
                                }
                            } catch (Exception e) {
