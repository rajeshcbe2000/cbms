 ...
 * @author Suresh
 */
public class RecoveryListGenerationDAO extends TTDAO {
    ...
    //Added By Akhila ends
    Rounding rd = new Rounding();

    /**
     * Creates a new instance of OperativeAcctProductDAO
     */
    public RecoveryListGenerationDAO() throws ServiceLocatorException {
    ...
    public void insertAdd(HashMap map) {
        ...
        try {
            ...
            HashMap newMemberMap = new HashMap();
            newMemberMap.put("EMP_REF_NO", CommonUtil.convertObjToStr(newMap.get("EMP_REF_NO")));
            List memberEmpTypeList = sqlMap.executeQueryForList("getMemberEmpType", newMemberMap);
            if (memberEmpTypeList != null && memberEmpTypeList.size() > 0) {
                HashMap MemberMap = (HashMap) memberEmpTypeList.get(0);
                if (MemberMap != null && MemberMap.size() > 0 && MemberMap.containsKey("CUST_TYPE_ID")) {
                    String memberType = CommonUtil.convertObjToStr(MemberMap.get("CUST_TYPE_ID"));
                    if (memberType != null && !memberType.equals("") && memberType.equals("MEMBER")) {
                        objSalaryRecoveryListDetailTO.setExemptionMode(CommonUtil.convertObjToStr("1"));
                    }
                    if (memberType != null && !memberType.equals("") && memberType.equals("CANTEEN")) {
                        objSalaryRecoveryListDetailTO.setExemptionMode(CommonUtil.convertObjToStr("0"));
                    }
                }
            }
            ...
            if (map.containsKey("INSERT")) {
                sqlMap.executeUpdate("insertSalaryRecoveryListDetailTO", objSalaryRecoveryListDetailTO);
...
//        System.out.println("@#$@@$@@@$ intUptoDt: " + intUptoDt);
    ...
    private void insertMasterData(HashMap map) throws Exception {
        try {
            if (map.containsKey("CALC_INT_UPTO_DT")) {
                ...
                SalaryRecoveryListMasterTO objSalaryRecoveryListMasterTO = new SalaryRecoveryListMasterTO();
                ...
                List listGeneration = sqlMap.executeQueryForList("getSelectRecoveryListGenerationProcessed", listGenerationMap);
                if (listGeneration != null && listGeneration.size() > 0) {
                    sqlMap.executeUpdate("updateRecoveryListGeneration", objSalaryRecoveryListMasterTO);
                }else{
                    objSalaryRecoveryListMasterTO.setCreatedDt((Date)currDt.clone());
                    sqlMap.executeUpdate("insertRecoveryListGeneration", objSalaryRecoveryListMasterTO);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void insertOmitPrincipleInterestProdData(HashMap map) throws Exception {
        try {
            HashMap omitDetailsMap = new HashMap();
            if (map != null && map.containsKey("OMIT_DETAILS")) {
                HashMap prodMap;
                omitDetailsMap = (HashMap) map.get("OMIT_DETAILS");
                Iterator iterate = omitDetailsMap.entrySet().iterator();
                while (iterate.hasNext()) {
                    prodMap = new HashMap();
                    Map.Entry entry = (Map.Entry) iterate.next();
                    Object key1 = (Object) entry.getKey();
                    List valueLst = (List) entry.getValue();
                    System.out.println("valueLst ::" + key1 + "::" + valueLst);
                    prodMap.put("PROD_ID", valueLst.get(4));
                    String omitInt = CommonUtil.convertObjToStr(valueLst.get(1));
                    String omitPrinc = CommonUtil.convertObjToStr(valueLst.get(2));
                    prodMap.put("OMIT_DATE", map.get("CALC_INT_UPTO_DT"));
                    if (omitInt != null && omitInt.equals("true")) {
                        prodMap.put("OMIT_INTEREST", "Y");
                    } else {
                        prodMap.put("OMIT_INTEREST", "N");
                    }
                    if (omitPrinc != null && omitPrinc.equals("true")) {
                        prodMap.put("OMIT_PRINCIPLE", "Y");
                    } else {
                        prodMap.put("OMIT_PRINCIPLE", "N");
                    }
                    sqlMap.executeUpdate("insertSalaryRecoveryOmitProdDetails", prodMap);
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private HashMap getData(HashMap obj) throws Exception {
        ...
        if (obj != null && obj.containsKey("DE_STATUS")) {
            //listForRecovery = sqlMap.executeQueryForList("getDueDetailsForRecoveryList", _branchCode);
            listForRecovery = sqlMap.executeQueryForList("getDueDetailsForRecoveryList", chkMap);
        } else {
            //listForRecovery = sqlMap.executeQueryForList("getDetailsForRecoveryList", _branchCode);
            listForRecovery = sqlMap.executeQueryForList("getDetailsForRecoveryList", chkMap);
        }
...
//                boolean insert = true;
                HashMap existMap = new HashMap();
                 HashMap omitDetailsMap = new HashMap();
                 if(obj!=null && obj.containsKey("OMIT_DETAILS")){
                    omitDetailsMap =  (HashMap)obj.get("OMIT_DETAILS");
                 }
                //System.out.println("omitDetailsMap :: " + omitDetailsMap); 
                existMap.put("INT_CALC_UPTO_DT", intUptoDt);
                List existList = sqlMap.executeQueryForList("checkingSameDateRecord", existMap);
                if (existList != null && existList.size() > 0) {
                    sqlMap.executeUpdate("deleteRecoveryDetailsData", existMap);
                    sqlMap.executeUpdate("deleteRecoveryDetailsMDSData", existMap);
                    sqlMap.executeUpdate("deleteRecoveryDetailsRDData", existMap);
                    sqlMap.executeUpdate("deleteRecoveryChargeDetails", existMap);
//                    insert = false;
                }
                for (int i = 0; i < listForRecovery.size(); i++) {
                    boolean retired = false;
                    recoveryMap = (HashMap) listForRecovery.get(i);
                    System.out.println("recoveryMap" + recoveryMap);
                    singleRowMap = new HashMap();
                    double glAmount = 0.0;
                    String empRefNoNew = CommonUtil.convertObjToStr(recoveryMap.get("EMP_REFNO"));
                    ...
                    if (retired == false) {
                        ...
                        if (prodType.equals(TransactionFactory.SUSPENSE)) {
                            ...
                            List aList = sqlMap.executeQueryForList("GetSuspenseDuedetails2", map1);
                            if (aList.size() > 0) {
                        ...
                        if (singleRowMap != null && singleRowMap.size() > 0) {
                             if (omitDetailsMap != null && omitDetailsMap.containsKey(CommonUtil.convertObjToStr(recoveryMap.get("PROD_DESC")))) {
                            ArrayList singList = (ArrayList) omitDetailsMap.get(CommonUtil.convertObjToStr(recoveryMap.get("PROD_DESC")));
                            if (singList != null && singList.size() > 0) {
                                String omitInt = CommonUtil.convertObjToStr(singList.get(1));
                                String omitPrinc = CommonUtil.convertObjToStr(singList.get(2));
                                 if (omitInt != null && omitInt.equals("true")) {
                                    double demand = CommonUtil.convertObjToDouble(singleRowMap.get("TOTAL_DEMAND"));
                                    double intAmt = CommonUtil.convertObjToDouble(singleRowMap.get("INTEREST"));
                                    demand = demand - intAmt; // Removed the comment by nithya on 13-06-2018 for KD 84 : 0011427: Salary recovery - Recovery list generation- while click on omit principal then total demand column became wrong in table.                         
                                    singleRowMap.put("TOTAL_DEMAND", CommonUtil.convertObjToStr(demand));
                                    singleRowMap.put("INTEREST", "0");
                                    singleRowMap.put("OMIT_INTEREST","Y");
                                }else{
                                    singleRowMap.put("OMIT_INTEREST","N");
                                }     
                                if (omitPrinc != null && omitPrinc.equals("true")) {
                                    double demand = CommonUtil.convertObjToDouble(singleRowMap.get("TOTAL_DEMAND"));
                                    double princAmt = CommonUtil.convertObjToDouble(singleRowMap.get("PRINCIPAL"));
                                    demand = demand - princAmt;   // Removed the comment by nithya on 13-06-2018 for KD 84 : 0011427: Salary recovery - Recovery list generation- while click on omit principal then total demand column became wrong in table.                         
                                    singleRowMap.put("TOTAL_DEMAND", CommonUtil.convertObjToStr(demand));
                                    singleRowMap.put("PRINCIPAL", "0");
                                    singleRowMap.put("OMIT_PRINCIPAL","Y");
                                }else{
                                    singleRowMap.put("OMIT_PRINCIPAL","N");
                                }                                                           
                            }
                        }else{
                           singleRowMap.put("OMIT_PRINCIPAL","N");
                        ...
                        if (totaldemand > 0) {
                            ...
                            if(singleRowMap.containsKey("OMIT_PRINCIPAL") && singleRowMap.get("OMIT_PRINCIPAL") != null && !singleRowMap.equals("") ){
                                if((CommonUtil.convertObjToStr(singleRowMap.get("OMIT_PRINCIPAL"))).equalsIgnoreCase("Y")){
                                   recoveryRowList.add("Y"); 
                                }else{
                                   recoveryRowList.add("N"); 
                                }
                            }else{
                                recoveryRowList.add("N");
                            }
                            if(singleRowMap.containsKey("OMIT_INTEREST") && singleRowMap.get("OMIT_INTEREST") != null && !singleRowMap.equals("") ){
                                if((CommonUtil.convertObjToStr(singleRowMap.get("OMIT_INTEREST"))).equalsIgnoreCase("Y")){
                                   recoveryRowList.add("Y"); 
                                }else{
                                   recoveryRowList.add("N");
                                }
                            ...
                            if (recoveryMap != null && recoveryMap.containsKey("DE_STATUS")) {
                                System.out.println("DE_STATUS====" + CommonUtil.convertObjToStr(recoveryMap.get("DE_STATUS")));
                                objSalaryRecoveryListDetailTO.setRecoveryType(CommonUtil.convertObjToStr(recoveryMap.get("DE_STATUS")));
                            } else {
                                objSalaryRecoveryListDetailTO.setRecoveryType("");
                            }
                            ...
                            objSalaryRecoveryListDetailTO.setExemptionMode(empType);     
                            // Added by nithya
                            if(singleRowMap.containsKey("OMIT_PRINCIPAL") && singleRowMap.get("OMIT_PRINCIPAL") != null && !singleRowMap.equals("") ){
                                if((CommonUtil.convertObjToStr(singleRowMap.get("OMIT_PRINCIPAL"))).equalsIgnoreCase("Y")){
                                   objSalaryRecoveryListDetailTO.setOmitPrincipal("Y");
                                }else{
                                   objSalaryRecoveryListDetailTO.setOmitPrincipal("N"); 
                                }
                            }else{
                                objSalaryRecoveryListDetailTO.setOmitPrincipal("N");
                            }
                            if(singleRowMap.containsKey("OMIT_INTEREST") && singleRowMap.get("OMIT_INTEREST") != null && !singleRowMap.equals("") ){
                                if((CommonUtil.convertObjToStr(singleRowMap.get("OMIT_INTEREST"))).equalsIgnoreCase("Y")){
                                   objSalaryRecoveryListDetailTO.setOmitInterest("Y");
                                }else{
                                   objSalaryRecoveryListDetailTO.setOmitInterest("N"); 
                                }
                            }else{
                                objSalaryRecoveryListDetailTO.setOmitInterest("N");
                            }    
                            System.out.println("objSalaryRecoveryListDetailTO :: " + objSalaryRecoveryListDetailTO);
                            // End
                            sqlMap.executeUpdate("insertSalaryRecoveryListDetailTO", objSalaryRecoveryListDetailTO);
                            if (glAmount == 0 && !prodType.equals(TransactionFactory.GL) && !prodType.equals(TransactionFactory.OPERATIVE) && !prodType.equals(TransactionFactory.SUSPENSE)) {                        //Update LOCK_STATUS YES
                                ...
                                updateMap.put("LOCK_STATUS", "");//modified by chithra on 25-06-14
                                System.out.println("updateMapupdateMap" + updateMap);
...
//                if (insert) {
                ...
                insertOmitPrincipleInterestProdData(obj);
//                }
                sqlMap.commitTransaction();
            } catch (Exception e) {
                sqlMap.rollbackTransaction();
...
//        System.out.println("dateeee.." + (int) rod.getNearest((long) (d * 100), 100) / 100);
        ...
        //return (int)((date2.getTime() - date1.getTime())/AVERAGE_MILLIS_PER_MONTH);
    }
...
//            map1.put("INSTALLMENT_DATE", currDt1);
            map1.put("ASONDATE", intUptoDt.clone());
...
//                    Double mm = monthsBetween(date2, tempDate);
...
//            }
    ...
    public HashMap calcMDS(String chittal_No, String schemeName, String prodType) throws Exception {
        System.out.println("chittal_No" + chittal_No + "schemeName" + schemeName + "prodType" + prodType);
        HashMap dataMap = new HashMap();
        ...
        String chittalNo = "";
        String subNo = "";
        if (chittal_No.contains("_")) {
            System.out.println("chittal_No.indexOf(_)" + chittal_No.indexOf("_"));
        ...
        System.out.println("JEFF********chittal_No" + dataMap.get("CHITTAL_NO"));
        System.out.println("JEFF********schemeName" + dataMap.get("SCHEME_NAME"));
        double insAmt = 0.0;
        long pendingInst = 0;
        int divNo = 0;
        long insDueAmt = 0;
...
//            System.out.println("productMapproductMapproductMap"+productMap);
...
//            bonusFirstInst = CommonUtil.convertObjToStr(productMap.get("BONUS_FIRST_INSTALLMENT"));
        ...
        System.out.println("insAmt" + insAmt + "totIns" + totIns + "startDate" + startDate + "insDate" + insDate + "bonusFirstInst" + bonusFirstInst);
        Date endDate = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(productMap.get("SCHEME_END_DT")));
        System.out.println("endDateendDate" + endDate);
        ...
        System.out.println("noOfInsPaid" + noOfInsPaid);
...
//            System.out.println("pendingInstpendingInst"+pendingInst);
            if (pendingInst > 0) {              //pending installment calculation starts...
                ...
                System.out.println("penalCalcBaseOnpenalCalcBaseOn" + penalCalcBaseOn);
                if (prized == true) {
                    ...
                    System.out.println("penalIntType" + penalIntType + "penalGraceType here" + penalGraceType + "penalValue" + penalValue + "penalGraceValue" + penalGraceValue);
                ...
                for (int j = startNoForPenal; j < noOfInstPay + startNoForPenal; j++) {
                    calc = 0;//added by chithra on 25-06-14
...
//                                System.out.println("#### diffDay Holiday False : " + diffDayPending);
                            ...
                            if (penalGraceType != null && !penalGraceType.equals("") && penalGraceType.equals("Days")) {
                                ...
                                if (diffDayPending > penalGraceValue) {
                                    if (penalIntType != null && !penalIntType.equals("") && penalIntType.equals("Percent")) {
                                        ...
                                        System.out.println("where calc" + calc);
                                    } else if (penalIntType != null && !penalIntType.equals("") && penalIntType.equals("Absolute")) {
                                        calc += penalValue;
                                        System.out.println("might calc" + calc);
                                    }
                                }
                            }
                            ...
                            if (penalGraceType != null && !penalGraceType.equals("") && penalGraceType.equals("Days")) {
                                ...
                                if (diffDayPending > penalGraceValue) {
                                    if (penalIntType != null && !penalIntType.equals("") && penalIntType.equals("Percent")) {
                                        //                                                calc = insAmt*(((pendingInst+1)*pendingInst/2)*penalValue)/1200;
                                        calc += (double) ((installmentAmt * penalValue) / 1200.0) * pendingInst--;
                                        System.out.println("jimcy calc" + calc);
                                    } else if (penalIntType != null && !penalIntType.equals("") && penalIntType.equals("Absolute")) {
                                        calc += penalValue;
                                        System.out.println("jency calc" + calc);
                                    }
                                }
                            }
                        }
                        ...
                        System.out.println("penalpenalpenalpenalpenalpenalpenal" + penal);
                        penalList.add(String.valueOf(penal));
                        installmentMap.put("PENAL", penalList);
                        penal = calc + 0.5;
                        System.out.println("pen" + penal);
                        System.out.println("penalpenal" + penal);
                    }
                    if (calc > 0) {
                        penalAmt += (long) (calc + 0.5);
                        System.out.println("penalAmtpenalAmt" + penalAmt);
                    }
                }
...
//                System.out.println("###### insertMap" + insertMap);
            ...
            System.out.println("insAmt" + insAmt + "noOfInstPay" + noOfInstPay + "penalAmt" + penalAmt + "totBonusAmt" + totBonusAmt + "totDiscAmt" + totDiscAmt + "chargeAmt" + chargeAmt);
            ...
            System.out.println("insAmtPayableinsAmtPayable" + insAmtPayable);
...
//                System.out.println("keyyyyyyyyyy iterator" + hash.keySet().iterator());
...
//                                    diffDayPending1 = DateUtil.dateDiff((Date) lastIntCalcDt, (Date) maxInstDt);
//                                    System.out.println("### diffDayPending : " + diffDayPending1);
//                                    if (diffDayPending1 <= 0) {
...
//                System.out.println("@#@ asAndWhenMap is >>>>jeffffffffff" + asAndWhenMap);
//                System.out.println("transDetail" + actNum +"BRANCHHHHHHHHHHHH" +_branchCode);
...
//                    List lst1 = null;
//                    if (inst_dt != null && (totPrinciple > 0.0)) {
...
//                                // totPrinciple += principle;
//                                totPrinciple = principle;
//                                System.out.println("totddddddddddddd" + totPrinciple);
//                            }
//                        } else {
...
//                                    insertPenal.put("CURR_MONTH_PRINCEPLE", (instalAmt - (CommonUtil.convertObjToDouble(asAndWhenMap.get("INTEREST")).doubleValue())));
//                                }
//                            }
//                            if (facilitylst != null && facilitylst.size() > 0) {
...
//                        // long diffDayPending = DateUtil.dateDiff(intUptoDt,expDt);
//                        System.out.println("############# Insert PBalance" + pBalance + "######diffDayPending :" + diffDayPending);
//                        if (diffDayPending > 0 && pBalance > 0.0) {
//                            System.out.println("############# Insert PBalance111" + pBalance + "######diffDayPending 1111:" + diffDayPending);
...
//                }
//
//                System.out.println("########## inst_dt" + inst_dt);
//                System.out.println("########## intUptoDt" + intUptoDt);
...
//                        } else {
//                            dataMap.put("PRINCIPAL", String.valueOf(Math.round(principalAmount)));
//                        }
//                        System.out.println("########## PRINCIPAL_YES" + String.valueOf(principalAmount));
//                    } else {
//                        if (installtype.equals("UNIFORM_PRINCIPLE_EMI") && emi_uniform.equals("Y")) {
//                            principalAmount = principalAmount - CommonUtil.convertObjToDouble(insertPenal.get("CURR_MONTH_INT"));
//                            dataMap.put("PRINCIPAL", String.valueOf(principalAmount));
...
//                        }
//                    }
//                    System.out.println("########## PRINCIPAL" + String.valueOf(principalAmount));
//                    System.out.println("########## recoveryYesNo" + recoveryYesNo);
//                    // IF SALARY_RECOVERY = 'YES' ONLY
//                    if ((recoveryYesNo.equals("Y") && principalAmount == 0) || (installtype.equals("UNIFORM_PRINCIPLE_EMI") && emi_uniform.equals("Y"))) {
...
//                                            dataMap.put("PRINCIPAL", String.valueOf(Math.round(finalDemandAmt)));
//                                            principalAmount = finalDemandAmt;
//                                        } else {
//                                            dataMap.put("PRINCIPAL", String.valueOf(Math.round(balanceLoanAmt)));
...
//                                        Date expiry_dt = null;
//                                        expiry_dt = (Date) recordMap.get("TO_DT");
//                                        if (installtype != null && installtype.equals("LUMP_SUM")) {
//                                            if (DateUtil.dateDiff(intUptoDt, expiry_dt) <= 0) {
//                                                dataMap.put("PRINCIPAL", CommonUtil.convertObjToStr(balanceLoanAmt));
//                                                principalAmount = balanceLoanAmt;
//                                            }
//                                        } else {
//                                            dataMap.put("PRINCIPAL", CommonUtil.convertObjToStr(balanceLoanAmt));
...
//                            HashMap whereMap1;
//                            Date expDt = null;
//                            List expDtList1 = sqlMap.executeQueryForList("getLoanExpDate1", loanInstall);
//                            if (expDtList1 != null && expDtList1.size() > 0) {
//                                whereMap1 = new HashMap();
//                                whereMap1 = (HashMap) expDtList1.get(0);
//                                HashMap balanceMap1 = new HashMap();
//                                double balanceLoanAmt1 = 0.0;
//                                double finalDemandAmt1 = 0.0;
//                                System.out.println("############ actNum : " + actNum);
//                                balanceMap1.put("ACCOUNTNO", actNum);
//                                expDt = (Date) whereMap1.get("TO_DT");
//                                System.out.println("expDt" + expDt);
//                                List balannceAmtLst1 = sqlMap.executeQueryForList("getBalanceLoanPrincipalAmt", balanceMap1);
//                                if (balannceAmtLst1 != null && balannceAmtLst1.size() > 0) {
//                                    balanceMap1 = (HashMap) balannceAmtLst1.get(0);
//                                    balanceLoanAmt1 = CommonUtil.convertObjToDouble(balanceMap.get("LOAN_BALANCE_PRINCIPAL")).doubleValue();
//                                    System.out.println("intUptoDt" + intUptoDt);
//                                    long diffDayPending4 = DateUtil.dateDiff(expDt, intUptoDt);
//                                    System.out.println("diffDayPending4" + diffDayPending4);
//                                    if (diffDayPending4 > 0) {
//
//                                        dataMap.put("PRINCIPAL", balanceLoanAmt1);
//                                    }
//                                }
//                            }
//                        }//ended
...
//                }
//                if (prodType.equals("AD")) {
//                    System.out.println("PRINCIPAL PRINCIPAL" + principalAmount);
...
//                    Date upto = (Date) intUptoDt.clone();
//                    emi.put("ACC_NUM", actNum);
//                    emi.put("UP_TO", setProperDtFormat(intUptoDt));
//
...
//
//                    }
//                    System.out.println("principle" + dueamount);
...
//                    dataMap.put("TOTAL_DEMAND", new Double(Math.round(totalDemand)));
//                    System.out.println("mmmmmmiiinnneee" + dataMap);
//
//
//                } else {
//                    /////-----------------
//                    System.out.println("one    principalAmount :"+principalAmount+"totalDemand :"+totalDemand+ ""+ actNum);
...
//    }
    public HashMap calcLoanPayments(String actNum, String prodId, String prodType, String recoveryYesNo) throws Exception {
        ...
        if (parameterList != null && parameterList.size() > 0) {
            ...
            repayDate = (java.util.Date) whereMap.get("RECOVERY_DATE");//zoho - 97973
...
//                                    System.out.println("clear_balance ="+clear_balance+" aaa---"+CommonUtil.convertObjToStr(loanInstall.get("ACT_NUM")));
                                        ...
                                        if (asAndWhenMap.containsKey("PREMATURE") && asAndWhenMap.containsKey("PREMATURE_INT_CALC_AMT") && CommonUtil.convertObjToStr(asAndWhenMap.get("PREMATURE_INT_CALC_AMT")).equals("LOANSANCTIONAMT")) {
                                            ...
                                            System.out.println("dtt: "+dtt+"moret:"+moret);
...
//                        System.out.println("####### insertPenal : "+insertPenal);
                            ...
                            if (insertPenal.containsKey("CURR_MONTH_PRINCEPLE")) {
                                ...
                                totalDemand = rd.getNearestHigher(CommonUtil.convertObjToDouble(insertPenal.get("CURR_MONTH_INT")).doubleValue(),1) + CommonUtil.convertObjToDouble(insertPenal.get("PENAL_INT")).doubleValue() + chargeAmt;
...
//                                        System.out.println("penallcalcuuu"+penal);
                                ...
                                principle = dueamount - interst;
//                                principle = rd.getNearestHigher(principle, 1);
                                System.out.println("actNum : "+actNum+" principle : "+principle+"mmmprinciple : " + principle + " penal : " + penal + " interst : " + interst + " dueamount : " + dueamount);
                                if (principle > 0) {
                                    totalDemand = principle + penal + interst;
                                } else {
...
//                                totalDemand = rd.getNearestHigher(totalDemand, 1);
...
//                            System.out.println("tttttoooo"+totalDemand);
                                ...
                                System.out.println("insttAmt" + insttAmt + "tempPrin" + tempPrin + "tempInt" + tempInt + " temmmpPrinc : " + temmmpPrinc+" totalDemand : "+totalDemand);
                                if (tempPrin < temmmpPrinc) {
                                    totalDemand -= tempPrin;
                                    totalDemand += temmmpPrinc;
                                    dataMap.put("PRINCIPAL", temmmpPrinc);
                                    System.out.println("prasnth tempPrin < temmmpPrinc " + totalDemand);
                                } else {
//                                    totalDemand += tempPrin;
                                    System.out.println("prasnth tempPrin < temmmpPrinc else : " + totalDemand);
                                }
                                ...
                                if (princHere > clearBal) {
                                    ...
                                    System.out.println("prasnth princHere > clearBal : " + totalDemand);
...
//                            System.out.println("mmmmmmiiinnneee"+dataMap);
                               ...
                               // dataMap.put("INTEREST", Long.valueOf(Math.round(Double.parseDouble(insertPenal.get("CURR_MONTH_INT").toString()))));
                                dataMap.put("INTEREST",rd.getNearestHigher(Double.parseDouble(insertPenal.get("CURR_MONTH_INT").toString()),1));
                                dataMap.put("PENAL", insertPenal.get("PENAL_INT"));
                                if (totalDemand > 0) {
                                    dataMap.put("TOTAL_DEMAND", totalDemand);
                                } else {
                        ...
                        if (prodType != null && !prodType.equals("") && prodType.equals("AD")) {
                            ...
                            intMap.put("AS_ON_DATE", intUptoDt.clone());
                        ...
                        if (prodType != null && !prodType.equals("") && prodType.equals("TL")) {
                            ...
                            System.out.println("intUptoDt ----------------------- :" + hmap);
                            ...
                            if (resltList != null && resltList.size() > 0) {
                                ...
                                if (ansMap != null && ansMap.containsKey("RESULT")) {
                                    ...
                                    if (ans != null && ans.length() > 0) {
                                        ...
                                        if (resltList != null && resltList.size() > 0) {
                                            System.out.println("dataMap ***SSSS***CCCCCC**** ZZZ :" + dataMap);
                                            ...
                                            System.out.println("dataMap ****ZZZZZZBBBBBBBBBBBBZZZZZZZ****** aaa :" + dataMap);
                                        }
                                    }
                                }
                            }
...
//                            System.out.println("############# MATURITY_DT" + matDate);
//                            System.out.println("############# CurrentDate" + currDate);
//                            System.out.println("date difference" + DateUtil.dateDiff((Date) matDt, (Date) currDate));
...
//                                int dep = depDt.getMonth(); //Changed By Nidhin - 9840 Recovery List Generation, Reccuring Installment is not correct /not getting
...
//                                    }
//                                    System.out.println("totalDelay 11 :"+totalDelay);
//                                    totalDelay = totalDelay + delayedInstallment;                                 
//                                    System.out.println("delayedInstallment---0000---"+delayedInstallment);
//                                }
//                            }
//                            lstRec = null;
...
//                                        delayedInstallment = 0;
//                                    }
//                                    System.out.println("totalDelay 22 :"+totalDelay);
//                                    totalDelay = totalDelay + delayedInstallment;
//                                    System.out.println("delayedInstallment---111---"+delayedInstallment);                                      
//                                }
//                            }
//                            lstRec = null;
...
//                            System.out.println(" MATURITY_DT" + matDate);
//                            System.out.println(" CurrentDate" + currDate);
//                            System.out.println("depDt"+depDt);
                            if (DateUtil.dateDiff((Date) matDt, (Date) currDate) > 0) {
...
//                        System.out.println("DateUtil.dateDiff((Date) matDt, (Date) currDate)"+DateUtil.dateDiff((Date) matDt, (Date) currDate));
                        if (DateUtil.dateDiff((Date) matDt, (Date) currDate) < 0 || insBeyondMaturityDat.equals("Y")) {
                            System.out.println("Executing RD...");
...
//                                System.out.println(" balanceAmt>" + balanceAmt + " totalDelay" + totalDelay);
                            ...
                            if (penalCalcType != null && !penalCalcType.equals("") && penalCalcType.equals("Days")) {
                                ...
                                depRecMap.put("DEPOSIT_DT", lastMap.get("DEPOSIT_DT"));
                                depRecMap.put("CURR_DT", currDate);
                                depRecMap.put("SL_NO", tot_Inst_paid);
                                lstRec = sqlMap.executeQueryForList("getDepTransRecurr", depRecMap);
                                ...
                                if (lstRec != null && lstRec.size() > 0) {
                                    for (int i = 0; i < size; i++) {
                                        ...
                                        sqlMap.executeUpdate("insertSalaryRecoveryRDDetails", RDInstallMap);
                                        tempPenal = 0.0;
                                    }
                                }
                                totalInst += 1;
                                if (actualDelay > 0) {
                                    ...
                                    sqlMap.executeUpdate("insertSalaryRecoveryRDDetails", RDInstallMapCurr);
                                }
                                penal = rd.getNearestHigher(penal, 1);
                                double principal = actualDelay * CommonUtil.convertObjToDouble(rdDataMap.get("DEPOSIT_AMT"));
...
//                    }
                    ...
                    dataMap.put("INTEREST", new Double(0));
                    dataMap.put("CHARGES", new Double(0));
                    dataMap.put("ADT_AMT", rdDataMap.get("ADT_AMT"));
                    dataMap.put("CLEAR_BALANCE", new Double(0));
                    if (CommonUtil.convertObjToDouble(rdDataMap.get("TOTAL_DEMAND")).doubleValue() <= 0.0) {
                        dataMap = null;
                    }                    
                 ...
                 if (behavesLike.equals("THRIFT")||behavesLike.equals("BENEVOLENT")) {   
                  dataMap=   calcThirftAndBenavolent(actNum,prodId,prodType);
                }
            }
        }
        return dataMap;
    }
    public HashMap calcThirftAndBenavolent(String actNum, String prodId, String prodType) throws Exception {
        HashMap dataMap = new HashMap();
        HashMap whereMap = new HashMap();
        String behavesLike = "";
        if (actNum.indexOf("_") != -1) {
        ...
        if (behavesLikeList != null && behavesLikeList.size() > 0) {
            ...
            behavesLike = CommonUtil.convertObjToStr(whereMap.get("BEHAVES_LIKE"));
            if (!behavesLike.equals("") && behavesLike != null) {
                if (behavesLike.equals("THRIFT")||behavesLike.equals("BENEVOLENT")) {            //Recurring Deposit
                    HashMap accountMap = new HashMap();
                    HashMap lastMap = new HashMap();
                    HashMap rdDataMap = new HashMap();
                    rdDataMap.put("DEPOSIT_NO", actNum);
                    accountMap.put("DEPOSIT_NO", actNum);
                    accountMap.put("BRANCH_ID", _branchCode);
                    accountMap.put("BEHAVESLIKE",behavesLike);
                    accountMap.put("ASONDT",(Date) intUptoDt.clone());
                    List lst = sqlMap.executeQueryForList("getProductIdForDeposits", accountMap);
                    if (lst != null && lst.size() > 0) {
                        accountMap = (HashMap) lst.get(0);
                        Date currDate = (Date) intUptoDt.clone();
                        ...
                        //                        Date currDate = (Date) currDt.clone();
                        String insBeyondMaturityDat = "";
                         List recurringLst = sqlMap.executeQueryForList("getThriftBenRecvoeryAmount", accountMap);
                         if (recurringLst != null && recurringLst.size() > 0) {
                               HashMap recurringMap = new HashMap();
                               recurringMap = (HashMap) recurringLst.get(0);
                                rdDataMap.put("PRINCIPAL", recurringMap.get("RECAMOUNT"));
                                 rdDataMap.put("TOTAL_DEMAND", recurringMap.get("RECAMOUNT"));
                         }
                     //   List recurringLst = sqlMap.executeQueryForList("getRecurringDepositDetails", accountMap);
//                        if (recurringLst != null && recurringLst.size() > 0) {
//                            HashMap recurringMap = new HashMap();
...
//                            beg_date = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(lastMap.get("DEPOSIT_DT")));
//                            end_date = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(lastMap.get("MATURITY_DT")));
//                            totInst_temp = differenceInMonths(beg_date, end_date);
//                            end_date = currDate;
//                            tot_Inst_paid = differenceInMonths(beg_date, end_date);
//                            HashMap prematureDateMap = new HashMap();
//                            double monthPeriod = 0.0;
//                            Date matDate = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(lastMap.get("MATURITY_DT")));
//                            if (matDate.getDate() > 0) {
...
//                            HashMap depRecMap = new HashMap();
//                            depRecMap.put("DEPOSIT_NO", actNum + "_1");
//                            double pay_amt =0,inst_amt=0;
//                            List lstRec = sqlMap.executeQueryForList("getMaxDueDateForDepRecurring", depRecMap);
//                            if (lstRec != null && lstRec.size() > 0) {
//                               depRecMap = (HashMap) lstRec.get(0);
//                               Date dueDate = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(depRecMap.get("DUE_DATE")));
//                               Date  end_date = currDate;
//                               double totPaidAmount = CommonUtil.convertObjToDouble(depRecMap.get("AMOUNT"));
//                               inst_amt= CommonUtil.convertObjToDouble(accountMap.get("DEPOSIT_AMT"));
//                                double tot_inst_amt = (tot_Inst_paid+1) * inst_amt;
//                                System.out.println("totPaidAmount:"+totPaidAmount+"inst_amt :"+inst_amt+"tot_inst_amt :"+tot_inst_amt+"diff :"+(tot_inst_amt-totPaidAmount));
//                                int mnthDiff =  differenceInMonths(dueDate, end_date);
//                                pay_amt = tot_inst_amt-totPaidAmount;
//                            }
//                          
//                            lstRec = null;
//                            depRecMap = new HashMap();
...
//                            double oldPenalAmt = CommonUtil.convertObjToDouble(accountMap.get("DELAYED_AMOUNT")).doubleValue();
//                            long oldPenalMonth = CommonUtil.convertObjToLong(accountMap.get("DELAYED_MONTH"));
//                            double balanceAmt = pay_amt;
//                            totalDelay = (long) (pay_amt/inst_amt);
...
//                            addAmt=totalDelay * CommonUtil.convertObjToDouble(accountMap.get("ADT_AMT"));
//                            }
//                            principal= principal +addAmt;
//                            double totalDemand = principal ;
//                            rdDataMap.put("DELAYED_MONTH", accountMap.get("DELAYED_MONTH"));
//                            rdDataMap.put("DEPOSIT_PENAL_AMT", "0.0");
//                            rdDataMap.put("PRINCIPAL", String.valueOf(principal));
//                            rdDataMap.put("TOTAL_DEMAND", String.valueOf(totalDemand));
//                            rdDataMap.put("DEPOSIT_PENAL_MONTH", String.valueOf(totalDelay));
//                            rdDataMap.put("ADT_AMT",accountMap.get("ADT_AMT"));
//                           }
                        //end rish
                    }
                    System.out.println("#### rdDataMap--->" + rdDataMap);
