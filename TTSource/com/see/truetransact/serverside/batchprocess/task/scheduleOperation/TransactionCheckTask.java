 ...
 */
public class TransactionCheckTask extends Task {
    ...
    public TransactionCheckTask(TaskHeader header) throws Exception {
        setHeader(header);
    ...
    private void initializeTaskObj(HashMap dataMap) throws Exception {
        ...
        sqlMap = (SqlMap) locate.getDAOSqlMap();
          taskLable = "TransactionDataCheck";
    }
    ...
    public TaskStatus executeTask() throws Exception {
        ...
        System.out.println("paramMap@@" + paramMap);
        lst = sqlMap.executeQueryForList("checkTransactionData", paramMap);
               System.out.println("@@@@@lst@@@RISH@@@"+lst);
        if (lst != null) {
            if (lst.size() > 0) {
                for (int a = 0, b = lst.size(); a < b; a++) {
                    data = (HashMap) lst.get(a);
                    if(data.containsKey("TABLE_NAME")&&data.get("TABLE_NAME")!=null){
                    vaultMap.put("TABLE_NAME", data.get("TABLE_NAME"));
                    vaultMap.put("ACT_NUM", data.get("ACT_NUM"));}
                }
                //                System.out.println("vaultMap"+vaultMap);
                System.out.println("valuemap size"+vaultMap.size());
                if (vaultMap.size() > 0) {
