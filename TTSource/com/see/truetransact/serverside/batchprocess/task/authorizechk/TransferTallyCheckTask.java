 ...
 * @author Sunil (152691)
 */
public class TransferTallyCheckTask extends Task {

    private static SqlMap sqlMap = null;
    private String branch = null;

    /**
     * Creates a new instance of TransferTallyCheckTask
     */
    public TransferTallyCheckTask(TaskHeader header) throws Exception {
        setHeader(header);
    ...
    public TaskStatus executeTask() throws Exception {
        ...
        if (sqlMap.executeQueryForList("getNonTallyTransfers", paramMap).size() > 0) {
            status.setStatus(BatchConstants.ERROR);
//            throw new TTException("Non tallied records exist");
        } else {
    ...
    public static void main(String args[]) {
        try {
            ...
            header.setTaskClass("TransferTallyCheckTask");
            HashMap paramMap = new HashMap();
            header.setBranchID("BRAN");
            header.setTaskParam(paramMap);
            TransferTallyCheckTask ft = new TransferTallyCheckTask(header);
            ft.executeTask();
        } catch (Exception E) {
            E.printStackTrace();
        }
