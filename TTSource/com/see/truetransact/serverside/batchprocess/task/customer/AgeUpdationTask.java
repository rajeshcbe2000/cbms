 ...
 * @author rahul
 */
public class AgeUpdationTask extends Task {
    ...
    /**
     * Creates a new instance of AgeUpdationTask
     */
    public AgeUpdationTask(TaskHeader header) throws Exception {
        System.out.println("age updation task header :: " + header);
        setHeader(header);
        ...
        if (paramMap != null && paramMap.containsKey("CHK_AGE_UPDATION")) {
            taskLable = CommonUtil.convertObjToStr(paramMap.get("CHK_AGE_UPDATION"));
        }
    }

    public TaskStatus executeTask() throws Exception {
        System.out.println("age updation execute");
            ...
            for (int b = 0; b < branchList.size(); b++) {
                ...
                if (compLst.size() <= 0 || compStatus.equals("ERROR")) {
                    ...
                    try { 
                        ...
                        updateLoanMap.put("BRANCH_CODE", branch);
                        updateLoanMap.put("CUSTOMER_AGE", "CUSTOMER_AGE");
                        sqlMap.startTransaction();
                        sqlMap.executeUpdate("updateCustomerAge", updateLoanMap);
                        sqlMap.commitTransaction();
                    } catch (Exception e) {
                        ...
                        if (e instanceof TTException) {
                            ...
                            if (tte != null) {
                                ...
                                if (exceptionMap != null) {
                                    ...
                                    errorMap.put("ERROR_CLASS", errClassName);
                                    errorMap.put("ACT_NUM", "Age Updation");
                                    errorMap.put("BRANCH_ID", branch);
                                    sqlMap.startTransaction();
                                    sqlMap.executeUpdate("insertError_showing", errorMap);
                                    sqlMap.commitTransaction();
