 ...
 *
 */
public class MDSClosedReciptDAO extends TTDAO {
     ...
     */
    public MDSClosedReciptDAO() throws ServiceLocatorException {
    ...
    private HashMap getData(HashMap map) throws Exception {
        ...
        List list = (List) sqlMap.executeQueryForList("getMDSReceiptEntryTOClosed", editMap);
...
//            penalTransHappened=false;
            ...
            mdsReceiptEntryTO = (MDSClosedReciptTO) ((List) getReturnMap.get("MDSReceiptEntryTO")).get(0);
...
//                System.out.println("DISCOUNT_TRANSACTION_TRANSFER :"+getReturnMap);
    ...
    private void insertData(HashMap map, LogDAO objLogDAO, LogTO objLogTO) throws Exception {
        try {
            ...
            MDSClosedReciptTO mdsBankAdvReceiptEntryTO = null;
            MDSClosedReciptTO mdsCurrentReceiptEntryTO = null;
...
//                        System.out.println("########## AFETER bankAdvInsAmt : " + bankAdvInsAmt);
...
//                        System.out.println("########## bankAdvMap : " + bankAdvMap);
                        ...
                        if (CommonUtil.convertObjToDouble(installMap.get("BONUS")).doubleValue() == 0) {
                            ...
                            //System.out.println("BANKK BONUSSSS>>>" + bankBonusReverseAmt + "   " + bankBonusAmt);
                        }
...
//                        System.out.println("########## instalmentPayAmt : " + instalmentPayAmt);
                ...
                System.out.println("testing :: bankBonusAmt ::"+ bankBonusAmt +" bankAdvInsAmt :: " +bankAdvInsAmt);
                mdsBankAdvReceiptEntryTO = getMDSReceiptEntryTO(bankBonusAmt, bankAdvInsAmt);
                mdsCurrentReceiptEntryTO = (MDSClosedReciptTO) map.get("mdsReceiptEntryTO");
                //double debitAmtForClosure = mdsCurrentReceiptEntryTO.getNetAmt();// Added by nithya on 30-11-2018 for KD-301 MDS AUTHORIZATION ISSUE.
                double debitAmtForClosure = mdsBankAdvReceiptEntryTO.getNetAmt() + mdsBankAdvReceiptEntryTO.getPenalAmtPayable()+mdsBankAdvReceiptEntryTO.getArbitrationAmt()+mdsBankAdvReceiptEntryTO.getNoticeAmt();
                ...
                insertBankAdvData(map, mdsBankAdvReceiptEntryTO, objLogDAO, objLogTO,debitAmtForClosure);
            }
            ...
            if (mdsReceiptEntryTO != null) { //INSERT RECIEPT TABLE
                if (bankAdvInsAmt > 0) {
                    if (bankAdvInsAmt > 0 && instalmentPayAmt > 0) {
                        ...
                        if (map.containsKey("BANK_ADVANCE_NET_TRANS_ID") && map.get("BANK_ADVANCE_NET_TRANS_ID") != null && CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_NET_TRANS_ID")).length() > 0) {//2093
                        ...
                        if (map.containsKey("BANK_ADVANCE_BONUS_TRANS_ID") && map.get("BANK_ADVANCE_BONUS_TRANS_ID") != null && CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_BONUS_TRANS_ID")).length() > 0) { //2093
               ...
               // System.out.println("###### mdsReceiptEntryTO ###### : " + mdsReceiptEntryTO);
                ...
                if (isSplitMDSTransaction != null && isSplitMDSTransaction.equals("Y")) {
                    if (map.containsKey("mdsClosedReceiptEntryTOList") && map.get("mdsClosedReceiptEntryTOList") != null) {
                        ...
                        //System.out.println("Going to Inserting ....in Closure");
                        for (int i = 0; i < mdssReceiptList.size(); i++) {
                            ...
                            MDSTo.setForfeitBonusTransId(""); //2093
...
//                setTransDetails(mdsReceiptEntryTO);
    ...
    private MDSClosedReciptTO getMDSReceiptEntryTO(double bonusAmt, double instAmt) {
        double netAmt = 0.0;
        netAmt = instAmt - bonusAmt;
        MDSClosedReciptTO mdsBankAdvReceiptEntryTO = new MDSClosedReciptTO();
    ...
    private void insertReceiptData(HashMap map, MDSClosedReciptTO mdsCurrentReceiptEntryTO, LogDAO objLogDAO, LogTO objLogTO) throws Exception {
        try {
//            System.out.println("###### MDS REPAYMENT ###### : ");
    ...
    //            System.out.println("postageAmt****"+postageAmt);
      ...
      //      System.out.println("###### insertTransactionData : " + map);
        ...
        //        System.out.println("newTransactionTo"+transactionTO);
          ...
          //          System.out.println("###### insertTransactionData mdsCurrentReceiptEntryTO : " + mdsCurrentReceiptEntryTO);
                    if (transactionTO.getTransType().equals("TRANSFER")) {
                        if (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNetAmt()).doubleValue() > 0) {
                            ...
                            } else if (!transactionTO.getProductType().equals("") && transactionTO.getProductType().equals("AB")) { //KD-3393
                                txMap.put(TransferTrans.DR_AC_HD, (String) debitMap.get("ACCT_HEAD"));
                                txMap.put(TransferTrans.DR_ACT_NUM, transactionTO.getDebitAcctNo());
                                txMap.put(TransferTrans.DR_PROD_ID, transactionTO.getProductId());
...
//                          }
                            ...
                            if (interBranchCodeList != null && interBranchCodeList.size() > 0 && !transactionTO.getProductType().equals("GL")) { // GL checking added by nithya on 24-11-2020 for KD-2457
...
//                                            txMap.put(TransferTrans.CR_AC_HD, applicationMap.get("CASE_EXPENSE_HEAD"));
						...
						//added by chithra for service Tax
                            ...
                            //    serTaxAmt -= (swachhCess + krishikalyanCess);
                                if(swachhCess > 0){
                                    txMap = new HashMap();
                                    transferTo.setInstrumentNo2("APPL_GL_TRANS");
                                    txMap.put(TransferTrans.CR_AC_HD, CommonUtil.convertObjToStr(serviceTaxMap.get("SWACHH_HEAD_ID")));
                                    txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                                    txMap.put(TransferTrans.CR_BRANCH, mdsCurrentReceiptEntryTO.getBranchCode()); //BRANCH_ID
                                    if (map.containsKey("FROM_PARTICULARS_TRANSALL")) {
                                        txMap.put(TransferTrans.PARTICULARS, "CGST" + map.get("FROM_PARTICULARS_TRANSALL"));
                                    } else {
                                        txMap.put(TransferTrans.PARTICULARS, "CGST :" + applicationMap.get("MP_MDS_CODE") + "-" + mdsCurrentReceiptEntryTO.getChittalNo() + "_" + mdsCurrentReceiptEntryTO.getSubNo());
                                    }
                                    txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                                    txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                                    txMap.put("TRANS_MOD_TYPE", "MDS");
                                ...
                                if(krishikalyanCess > 0){
                                    txMap = new HashMap();
                                    transferTo.setInstrumentNo2("APPL_GL_TRANS");
                                    txMap.put(TransferTrans.CR_AC_HD, CommonUtil.convertObjToStr(serviceTaxMap.get("KRISHIKALYAN_HEAD_ID")));
                                    txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                                    txMap.put(TransferTrans.CR_BRANCH, mdsCurrentReceiptEntryTO.getBranchCode()); //BRANCH_ID
                                    if (map.containsKey("FROM_PARTICULARS_TRANSALL")) {
                      ...
                      //  System.out.println("transferTo List 4 : " + TxTransferTO);
                        ...
                        if(penalTransHappened && bankAdvInsAmt > 0){
                            finalReceivingAmt = CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNetAmt()).doubleValue() - service_taxAmt;
                        }else{
                            finalReceivingAmt = CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNetAmt()).doubleValue()
...
//                                sqlMap.executeUpdate("updateLinkBatchIdCash", linkBatchMap);
                            ...
                            //Case Expense Amount Transaction
                            System.out.println("arcWaiveAmt check :: " + arcWaiveAmt);
                            if(arcWaiveAmt > 0){
                                arcHead = getARCHead(mdsCurrentReceiptEntryTO);
                            }
                            if (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getArbitrationAmt()).doubleValue() - arcWaiveAmt > 0) {
...
//            setTransDetails(mdsReceiptEntryTO);
    ...
    private void insertBankAdvData(HashMap map, MDSClosedReciptTO mdsBankAdvReceiptEntryTO, LogDAO objLogDAO, LogTO objLogTO, double debitAmtForClosure) throws Exception {
        try {
          ...
          //  System.out.println("###### insertTransactionData : " + map);
            ...
            //        System.out.println("###### insertTransactionData mdsBankAdvReceiptEntryTO : " + mdsBankAdvReceiptEntryTO);
              ...
              //              System.out.println("Transfer Started debit : ");
                            ...
                            transferTo = transactionDAO.addTransferDebitLocal(txMap, debitAmtForClosure + CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getForfeitBonusAmtPayable()).doubleValue() - totalWaiveAmt);// Added by nithya on 30-11-2018 for KD-301 MDS AUTHORIZATION ISSUE.
                            transferTo.setTransId("-");
                            transferTo.setBatchId("-");
                            transferTo.setTransDt(currDt);
                            transferTo.setBranchId(BRANCH_ID);
                            ...
                            transferTo.setInitTransId(CommonUtil.convertObjToStr(map.get("USER_ID")));
                            transferTo.setTransAllId(CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                            transferTo.setSingleTransId(generateSingleTransId);// Added by nithya on 14-08-2018 for KD-205 0014600: mds receipt entry after closure as transfer entry not getting for authorization
                            transactionDAO.setLinkBatchID(CommonUtil.convertObjToStr(mdsBankAdvReceiptEntryTO.getChittalNo()));
                            transferTo.setScreenName((String) map.get(CommonConstants.SCREEN));
                            transferTo.setNarration(transactionTO.getParticulars());//KD-3393
                            TxTransferTO.add(transferTo);
...
//                        if(bankBonusAmt>0){
                        ...
                        if (bonusreversal) {
                            ...
                            if (map.containsKey("MDS_CLOSURE")) {
                                ...
                                txMap.put(TransferTrans.CR_AC_HD, applicationMap.get("MDS_FORFEITED_ACCT_HEAD")); //2093
                            }
                            txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                            txMap.put(TransferTrans.CR_BRANCH, mdsBankAdvReceiptEntryTO.getBranchCode());//BRANCH_ID);
...
//                            System.out.println("bankBonusReverseAmt" + bankBonusReverseAmt);
                            ...
                            transferTo.setTransDt(currDt);
                            transferTo.setInitiatedBranch(BRANCH_ID);
                            transferTo.setInstrumentNo2("BONUS_REVERSAL");// 2093
                            transferTo.setSingleTransId(generateSingleTransId);
                            transferTo.setStatusBy(CommonUtil.convertObjToStr(map.get("USER_ID")));
                            transferTo.setInitTransId(CommonUtil.convertObjToStr(map.get("USER_ID")));
                            transferTo.setTransAllId(CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                        ...
                        if (mdsBankAdvReceiptEntryTO.getForfeitBonusAmtPayable() != null && CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getForfeitBonusAmtPayable()).doubleValue() > 0.0) {
                            if (bonusreversal) {
                               ...
                               // System.out.println("####### cashTransMap : " + cashTransMap);
                                ...
                                mdsBankAdvReceiptEntryTO.setForfeitBonusTransId(CommonUtil.convertObjToStr(cashMap.get("TRANS_ID"))); //2093
                                HashMap linkBatchMap = new HashMap();
                                if (!transactionTO.getProductType().equals("") && transactionTO.getProductType().equals("GL")
                                        && transactionTO.getProductId().equals("") && !map.containsKey("LOCKER_SURRENDER_DAO")) {
                                    linkBatchMap.put("LINK_BATCH_ID", cashMap.get("TRANS_ID"));
...
//            setTransDetails(mdsReceiptEntryTO);
    ...
    private void setTransDetails(MDSClosedReciptTO mdsReceiptEntryTO) throws Exception {
//        if(!penalTransHappened){
...
//            objCashTO.setInitiatedBranch(CommonUtil.convertObjToStr(applnMap.get("BRANCH_CODE")));
    ...
    public void commonTransactionCashandTransfer(HashMap map, MDSClosedReciptTO mdsCurrentReceiptEntryTO, HashMap debitMap, HashMap applicationMap) throws Exception {
        String BRANCH_ID = CommonUtil.convertObjToStr(map.get("BRANCH_CODE"));
        ...
        if (map.containsKey("GRACE_MAP") && map.get("GRACE_MAP") != null) {
            graceForFeiteMap = (HashMap) map.get("GRACE_MAP");
            System.out.println("graceForFeiteMap :: " + graceForFeiteMap);
            if (graceForFeiteMap != null && graceForFeiteMap.containsKey("FORFEITE_BONUS_AMOUNT_LIST") && graceForFeiteMap.get("FORFEITE_BONUS_AMOUNT_LIST") != null) {
        ...
        System.out.println("forfeiteBonusAmount :: " + forfeiteBonusAmount);
        transferTrans.setInitiatedBranch(BRANCH_ID);
        transactionDAO = new TransactionDAO(CommonConstants.DEBIT);
        transactionDAO.setInitiatedBranch(_branchCode);
        transactionDAO.setCommandMode(CommonConstants.TOSTATUS_INSERT);
...
//            System.out.println("txMap : " + txMap+"serviceAmt :"+mdsCurrentReceiptEntryTO.getBonusAmtPayable());
            txMap.put("TRANS_MOD_TYPE", "MDS");
            transferTo = transactionDAO.addTransferDebitLocal(txMap, CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getBonusAmtPayable()).doubleValue() + forfeiteBonusAmount);
            transferTo.setTransId("-");
            transferTo.setBatchId("-");
...
//By Nidhin
            System.out.println("Execute inside bonus");
            transferTo = new TxTransferTO();
            txMap = new HashMap();
            transferTo.setInstrumentNo2("APPL_GL_TRANS");
            if (map.containsKey("MDS_CLOSURE")) {
            ...
            transferTo.setInstrumentNo2("FORFEITE_BONUS");
            transferTo.setTransId("-");
            transferTo.setBatchId("-");
            transferTo.setTransType(CommonConstants.DEBIT);
            transferTo.setProdType(TransactionFactory.GL);
...
//            System.out.println("txMap : " + txMap+"serviceAmt :"+mdsCurrentReceiptEntryTO.getDiscountAmt());
        ...
        if (receivingGLAmt > 0 || forfeiteBonusAmount > 0) {
...
//            System.out.println("txMap : " + txMap+"serviceAmt :"+receivingGLAmt);
            ...
            // Forfiet credit
             if (forfeiteBonusList != null && forfeiteBonusList.size() > 0) {
                System.out.println("forfeited case :: credit");
        ...
        if (map.containsKey("TransactionTO")) {
            ...
            if (transactionTO.getTransType().equals("TRANSFER") || transactionTO.getTransType().equals("CASH")) {
                ...
                if (transactionMap != null && transactionMap.get("BONUS_TRANS_ID") != null && !transactionMap.get("BONUS_TRANS_ID").equals("")) {
                    ...
                    //cashAuthMap.put(CommonConstants.BRANCH_ID, map.get(CommonConstants.BRANCH_ID));
                    cashAuthMap.put(CommonConstants.USER_ID, map.get(CommonConstants.USER_ID));
                    cashAuthMap.put("DEBIT_LOAN_TYPE", "DP");//KD-3698
                    transactionDAO = new TransactionDAO(CommonConstants.DEBIT);
                    serviceAuthMap.put("AUTHORIZE_STATUS", authorizeStatus);
                    serviceAuthMap.put("AUTHORIZE_BY", map.get("USER_ID"));
                    ArrayList transList = new ArrayList();
                ...
                if (mdsReceiptEntryTO.getBankPay().equals("N")) {
                    ...
                    mdsTransMap.put("MDS_INTEREST", mdsReceiptEntryTO.getMdsInterset());
                    mdsTransMap.put("NOTICE_AMT", CommonUtil.convertObjToDouble(mdsReceiptEntryTO.getNoticeAmt()));//9535
                    ...
                    //mdsTransMap.put("SUBSCRIPTION_AMT", mdsReceiptEntryTO.getSubscriptionAmt());//01-11-2019
                    mdsTransMap.put("STATUS", CommonConstants.STATUS_CREATED);
...
//                authorizeReceiptMap.put("NET_TRANS_ID",transactionTO.getBatchId());
    ...
    public HashMap execute(HashMap map, boolean isTransaction) throws Exception {
        ...
        if (map.containsKey("mdsReceiptEntryTO")) {
            mdsReceiptEntryTO = new MDSClosedReciptTO();
            mdsReceiptEntryTO = (MDSClosedReciptTO) map.get("mdsReceiptEntryTO");
    ...
    private void clearAllFileds() { // Added by nithya for KD-2093 
        bankAdvInsAmt = 0.0;        
        bankBonusAmt = 0.0;      
    }
    ...
    public static void main(String str[]) {
        try {
            MDSClosedReciptDAO dao = new MDSClosedReciptDAO();
    ...
    public void commonTransactionWaiveOff(HashMap map, MDSClosedReciptTO mdsCurrentReceiptEntryTO, HashMap debitMap, HashMap applicationMap,double waiveAmount, String waiveParam, String arcHead) throws Exception {
        System.out.println("arcHead ... :: " + arcHead);
        String BRANCH_ID = CommonUtil.convertObjToStr(map.get("BRANCH_CODE"));
        System.out.println("mdsCurrentReceiptEntryTO####" + mdsCurrentReceiptEntryTO + map);
        TransferTrans transferTrans = new TransferTrans();
        TxTransferTO transferTo = new TxTransferTO();
    ...
    private String getARCHead(MDSClosedReciptTO mdsCurrentReceiptEntryTO) throws SQLException {
        String arcHead = "";
        HashMap whereMap = new HashMap();
        whereMap.put("ACT_NUM", CommonUtil.convertObjToStr(mdsCurrentReceiptEntryTO.getChittalNo()) + "_" + CommonUtil.convertObjToStr(mdsCurrentReceiptEntryTO.getSubNo()));
        List chargeList = (List) sqlMap.executeQueryForList("getMDSCaseChargeDetails", whereMap);
