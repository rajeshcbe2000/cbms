 ...
 *
 */
public class LockerProdDAO extends TTDAO {

    private static SqlMap sqlMap = null;
    private LockerProdTO objTO;
    private final static Logger _log = Logger.getLogger(LockerProdDAO.class);
    private LogDAO logDAO;
    private LogTO logTO;
    private ArrayList billChargeTabTO;
    private ArrayList billChargesTabTO;
    private ArrayList billDelChargeTabTO;
    private LockerProdChargesTO billChargeTO;
    private LockerProdChargesTO billChargeTabSelectTO;
    private LockerProdChargesTO billDelChargesTO;
    ...
    private HashMap getData(HashMap map) throws Exception {
        ...
        List list = (List) sqlMap.executeQueryForList("getSelectLockerProdTO", whereMap);
        System.out.println("getBillsTO#####" + list);
        returnMap.put("LockerProdTO", list);
        list = (List) sqlMap.executeQueryForList("getSelectLockerProdChargesTO", whereMap);
        System.out.println("getBillschargesTO#####" + list);
        returnMap.put("LockerProdChargesTO", list);
        list = null;
        where = null;
        return returnMap;
    }
    ...
    //    }
    private void insertData(LockerProdTO objTO) throws Exception {
        try {
    ...
    private void updateData(LockerProdTO objTO, HashMap map) throws Exception {
        try {
            sqlMap.startTransaction();
            ...
            logTO.setPrimaryKey(objTO.getKeyData());
            logTO.setStatus(objTO.getCommand());
            sqlMap.executeUpdate("updateLockerProdTO", objTO);
            logDAO.addToLog(logTO);
            if (map.containsKey("LockerProdChargesTO")) {

                billChargesTabTO = new ArrayList();
                billChargesTabTO = (ArrayList) map.get("LockerProdChargesTO");
                System.out.println("billChargesTabTO&&&&&&&" + billChargesTabTO);
                int size = billChargesTabTO.size();
                System.out.println("size#&&&&&&&&" + size);
                sqlMap.executeUpdate("deleteLockerProdCharges", objTO);
                for (int i = 0; i < size; i++) {
                    billChargeTabSelectTO = (LockerProdChargesTO) billChargesTabTO.get(i);
...
//                            sqlMap.executeUpdate("updateLockerProdChargesTO", billChargeTabSelectTO);
                    } else {
                        billChargeTabSelectTO.setStatus(CommonConstants.STATUS_CREATED);
                        billChargeTabSelectTO.setProdID(objTO.getProdId());
//                            sqlMap.executeUpdate("insertLockerProdChargesTO",billChargeTabSelectTO);
                    }
                    sqlMap.executeUpdate("insertLockerProdChargesTO", billChargeTabSelectTO);
                }
            }
...
//                billDelChargeTabTO=(ArrayList)map.get("DeletedChargesTO");
                ...
                System.out.println("size#&&&&&&&&" + size);
                for (int i = 0; i < size; i++) {
                    billDelChargesTO = (LockerProdChargesTO) billDelChargeTabTO.get(i);
...
//                        sqlMap.executeUpdate("deleteLockerProdChargesTO", billDelChargesTO);
                    sqlMap.executeUpdate("insertLockerProdChargesTO", billDelChargesTO);
    ...
    private void deleteData(LockerProdTO objTO) throws Exception {
        try {
            sqlMap.startTransaction();
            ...
            logTO.setPrimaryKey(objTO.getKeyData());
            logTO.setStatus(objTO.getCommand());
            sqlMap.executeUpdate("deleteLockerProdTO", objTO);
            if (billChargeTabSelectTO != null) {
                billChargeTabSelectTO.setStatus(objTO.getStatus());
                billChargeTabSelectTO.setProdID(objTO.getProdId());
                sqlMap.executeUpdate("deleteLockerProdChargesTO", billChargeTabSelectTO);
            }
            logDAO.addToLog(logTO);
            sqlMap.commitTransaction();
        } catch (Exception e) {
    ...
    public HashMap execute(HashMap map) throws Exception {
        System.out.println("execute map#LOCKER" + map);
        _branchCode = (String) map.get(CommonConstants.BRANCH_ID);
        objTO = (LockerProdTO) map.get("LockerProdTO");
        ...
        if (map.containsKey("LockerProdChargesTO")) {
            ...
            billChargeTabTO = (ArrayList) map.get("LockerProdChargesTO");
            System.out.println("billchargetabto######" + billChargeTabTO);
        }
        if (map.containsKey("DeletedChargesTO")) {
            billDelChargeTabTO = new ArrayList();
