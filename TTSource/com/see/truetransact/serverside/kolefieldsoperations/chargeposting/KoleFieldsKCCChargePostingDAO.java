 ...
 *
 */
public class KoleFieldsKCCChargePostingDAO extends TTDAO {
    private final static Logger log = Logger.getLogger(KoleFieldsKCCChargePostingDAO.class);
...
//    private LinkedHashMap deletedCaseDetailMap = null;
     ...
     */
    public KoleFieldsKCCChargePostingDAO() throws ServiceLocatorException {
    ...
    public HashMap execute(HashMap map) throws Exception  {
        try{
          System.out.println("map inside KoleFieldsKCCChargePostingDAO"+map); 
        ...
        ArrayList chargesList = (ArrayList) map.get("CHARGE_LIST");
        if(map.containsKey("POST_CHARGES") && map.get("POST_CHARGES") !=  null && map.get("POST_CHARGES").equals("Y")){
            processChargesTransaction(map, chargesList);
        } else if(map.containsKey("CHARGE_DEBIT_FROM_LOAN") && map.get("CHARGE_DEBIT_FROM_LOAN") !=  null && map.get("CHARGE_DEBIT_FROM_LOAN").equals("Y")){
            processDebitLoanTypeTransaction(map, chargesList);
        }
        }catch(Exception e){
            e.printStackTrace();
            log.error(e);
    ...
    private void processChargesTransaction(HashMap mapRiskFnd, List lstRisk) throws Exception {
        ...
        String behavesLike = CommonUtil.convertObjToStr(mapRiskFnd.get("BEHAVES_LIKE"));
        String screenName = CommonUtil.convertObjToStr(mapRiskFnd.get("SCREEN"));
        int count = 0;
           try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if(lstSubRsk.size() > 7){ //KD-3483
                 narration = CommonUtil.convertObjToStr(lstSubRsk.get(8));
                }
                List listRisk = null;
                mapSubRisk1.put("PROD_ID", mapRiskFnd.get("PROD_ID_DR"));
                listRisk = sqlMap.executeQueryForList("getKoleFieldKCCChargesHead", mapSubRisk1);
                if (listRisk != null && listRisk.size() > 0) {
                    ...
                    }else if(CommonUtil.convertObjToStr(mapRiskFnd.get("CHRG_TYP")).equalsIgnoreCase("KOLEFIELD OPERATION")){
                        accHead =  CommonUtil.convertObjToStr(mapSubRisk1.get("KOLE_FIELD_OPERATION"));
                    }else if(CommonUtil.convertObjToStr(mapRiskFnd.get("CHRG_TYP")).equalsIgnoreCase("KOLEFIELD EXPENSE")){
                        accHead =  CommonUtil.convertObjToStr(mapSubRisk1.get("KOLE_FIELD_EXPENSE"));
                    }
                }
...
//                    txMap.put(TransferTrans.DR_PROD_TYPE, CommonUtil.convertObjToStr(mapRiskFnd.get("PROD_TYP_DR")));
                ...
                //[0001212000004, STAFF OVERDRAFT, , BABYPRASANNA, 0, 198060, 500, 45.0, null]
                transAmt = CommonUtil.convertObjToDouble(lstSubRsk.get(7));
            ...
            if (transferList != null && transferList.size() > 0) {
                doDebitCredit(transferList, _branchCode, false,behavesLike);
            }
      ...
      private void doDebitCredit(ArrayList batchList, String branchCode, boolean isAutoAuthorize,String behavesLike) throws Exception {
        ...
        authorizeMap.put("USER_ID", user_id);
        authorizeMap.put("LOAN_TRANS_OUT","LOAN_TRANS_OUT");
        ...
        data.put("LOAN_TRANS_OUT","LOAN_TRANS_OUT");
    ...
    private void processTransaction(HashMap mapRiskFnd, List lstRisk)  throws Exception {
        try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                objRiskfndTo.setRiskFundAmt(CommonUtil.convertObjToDouble(lstSubRsk.get(7)));
            ...
            if (transferList != null && transferList.size() > 0) {
                doDebitCredit(transferList, _branchCode, false,behaves_like);
            }
            getTransDetails(CommonUtil.convertObjToStr(loanDataMap.get("SINGLE_TRANS_ID")));
            returnDataMap.put("SINGLE_TRANS_ID", CommonUtil.convertObjToStr(returnDataMap.get("SINGLE_TRANS_ID")));
        } catch (Exception e) {
   ...
   private void processChargesWithoutTransaction(HashMap mapRiskFnd, List lstRisk) throws Exception {  
     ...
     private void processRiskFundWithoutTransaction(HashMap mapRiskFnd, List lstRisk)  throws Exception {
      ...
      private void processDebitLoanTypeTransaction(HashMap mapRiskFnd, List lstRisk) throws Exception {
        TransferTrans objTransferTrans = new TransferTrans();
        ArrayList transferList = new ArrayList();
        HashMap txMap = new HashMap();
        String accHead = "";
        ...
        int count = 0;
           try {
            ...
            for (int i = 0; i < size; i++) {
                ...
                if (listRisk != null && listRisk.size() > 0) {
                        mapSubRisk1 = (HashMap) listRisk.get(0);
                        accHead =  CommonUtil.convertObjToStr(mapSubRisk1.get("ACCT_HEAD"));
                }
                txMap.put(TransferTrans.DR_AC_HD, accHead);
               ...
               if(mapRiskFnd.containsKey("PROD_NAME_DR") && CommonUtil.convertObjToStr(mapRiskFnd.get("PROD_NAME_DR")).equals("MDS")){
                ...
                termLoanChargeTO.setAuthorize_by(CommonUtil.convertObjToStr(mapRiskFnd.get("USER_ID")));
                termLoanChargeTO.setAuthorize_Status(CommonConstants.STATUS_AUTHORIZED);
                termLoanChargeTO.setPaidAmount(transAmt);
                termLoanChargeTO.setNarration(narration);
                sqlMap.executeUpdate("insertTermLoanChargeTO", termLoanChargeTO);                
            }
            txMap = new HashMap();
