...
//        executeSecurityTabQuery();
    ...
    private void insertData(HashMap map, HashMap transDetailMap,String generateCashId) throws Exception {
        try {
            ...
            //Added By Suresh R  KDSA-467  Refered By Jithesh  21-Aug-2019 To Avoid OD A/c Authorization Failed, in case of ROI given in Product level.
            if(CommonUtil.convertObjToStr(objTermLoanBorrowerTO.getCategory()).length()<=0){
                objTermLoanBorrowerTO.setCategory("GENERAL_CATEGORY");
            }
...
//                executeUpdate("insertTermLoanSecuritySalaryTO", objTermLoanSecuritySalaryTO);
            ...
            if(transDetailMap.containsKey("LOAN_RENEWAL") && transDetailMap.get("LOAN_RENEWAL") != null && transDetailMap.get("LOAN_RENEWAL").equals("LOAN_RENEWAL")){
                // update the temp table for loan renewal
                String oldLoanNo = "";
                if(transDetailMap.containsKey("OLD_LOAN_NO") && transDetailMap.get("OLD_LOAN_NO") != null){
                    oldLoanNo = CommonUtil.convertObjToStr(transDetailMap.get("OLD_LOAN_NO"));
                }
                HashMap renewalMap = new HashMap();
                renewalMap.put("NEW_LOAN_NO",acct_No);
                renewalMap.put("OLD_LOAN_NO",oldLoanNo);
                sqlMap.executeUpdate("updateLoanODRenewalDetails", renewalMap);
            }
   ...
   private void insertInstallmentSchedule() throws Exception{
    ...
    private void executeSalaryDetailsQuery() throws Exception {
        ...
        keySetFacility = facilityMap.keySet();
        objKeySet = (Object[]) keySetFacility.toArray();
        ...
        if (termLoanSecuritySalaryTODeletedMap != null && termLoanSecuritySalaryTODeletedMap.size() > 0) {
            ...
            for (int i = 0; i < keyset.size(); i++) {
                ...
                objTermLoanSecuritySalaryTO.setStatusBy(objTermLoanFacilityTO.getStatusBy());
                objTermLoanSecuritySalaryTO.setStatusDt(curr_dt);
...
//                    objTermLoanSecuritySalaryTO.setStatusBy(objTermLoanFacilityTO.getStatusBy());
//                    objTermLoanSecuritySalaryTO.setStatusDt(curr_dt);
                    objTermLoanSecuritySalaryTO.setStatusDt(getProperDateFormat(objTermLoanSecuritySalaryTO.getStatusDt()));
                    executeUpdate("updateTermLoanSecuritySalaryTO", objTermLoanSecuritySalaryTO);
                }
            }
        }
        // Moved code block to starting
//        if (termLoanSecuritySalaryTODeletedMap != null && termLoanSecuritySalaryTODeletedMap.size() > 0) {
//            Set keyset = termLoanSecuritySalaryTODeletedMap.keySet();
//            Object obj[] = (Object[]) keyset.toArray();
//            for (int i = 0; i < keyset.size(); i++) {
...
//                    System.out.println("recovery000111>>>" + recovery);
                        ...
                        if (interest > 0) {
                            ...
                            if (listData != null && listData.size() > 0) {
                                ...
                                if (map1.get("CASHIER_AUTH_ALLOWED") != null && map1.get("CASHIER_AUTH_ALLOWED").toString().equals("Y")
                                    ...
                                    objCashTO.setAuthorizeStatus_2(null); // KD-4324 -- Added by nithya on 30 jun 2025
                                } else {
                    ...
                    if (dataMap.get("BEHAVES_LIKE").equals("OD") && dataMap.containsKey("LOAN_RENEWAL") && CommonUtil.convertObjToStr(dataMap.get("LOAN_RENEWAL")).equals("LOAN_RENEWAL")) {                        
                        interestAmt = CommonUtil.convertObjToDouble(dataMap.get("OLD_LOAN_AMT"));
                        transferTo = transactionDAO.addTransferDebitLocal(txMap, interestAmt + TotchargeAmt);
                    }else{
                    ...
                    txMap.put(TransferTrans.CR_PROD_ID, renewalcrMap.get("PROD_ID"));
                         ...
                         HashMap renewalBranchCodeMap = new HashMap();
                         renewalBranchCodeMap.put("ACT_NUM", dataMap.get("RENWAL_ACT_NUM"));
                         List renewalBranchCodeList = sqlMap.executeQueryForList("getSelectInterBranchCode", renewalBranchCodeMap);
                         if (renewalBranchCodeList != null && renewalBranchCodeList.size() > 0) {
                             renewalBranchCodeMap = (HashMap) renewalBranchCodeList.get(0);
                             //  System.out.println("interBranchCodeMap : " + interBranchCodeMap);
                         }
                    
                    if (renewalBranchCodeMap != null && !renewalBranchCodeMap.isEmpty()) {
                        //txMap.put(TransferTrans.CR_BRANCH, interBranchCodeMap.get("BRANCH_CODE"));
                        txMap.put(TransferTrans.CR_BRANCH, renewalBranchCodeMap.get("BRANCH_CODE"));
                    } else {
...
//                    txMap.put("DEBIT_LOAN_TYPE", "DP");
                    ...
                    System.out.println("balance amount :: " + interestAmt);
                    System.out.println("Total charge Amount");
                         if (interestAmt <= 0 && !dataMap.get("BEHAVES_LIKE").equals("OD")) {
                             double recoveryAmt = (-1) * interestAmt;
                             ...
                             transferTo = transactionDAO.addTransferDebitLocal(txMap, recoveryAmt);
                             transferTo.setAuthorizeStatus_2("ENTERED_AMOUNT");
                             transferTo.setStatusBy(CommonUtil.convertObjToStr(dataMap.get("USER_ID")));
                             transferTo.setInitTransId(CommonUtil.convertObjToStr(dataMap.get("USER_ID")));
                             transferTo.setLinkBatchId((String) dataMap.get("ACT_NUM"));
                    ...
                    if(dataMap.containsKey("LOAN_RENEWAL") && CommonUtil.convertObjToStr(dataMap.get("LOAN_RENEWAL")).equals("LOAN_RENEWAL") && dataMap.get("CR_ACT_NUM").equals("LOAN_RECEIPT")){
                        ...
                        txMap.put(TransferTrans.CR_BRANCH, dataMap.get("BRANCH_CODE"));
                        txMap.put(TransferTrans.PARTICULARS, "receipt - " + dataMap.get("CR_ACT_NUM"));
                        txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                        //txMap.put(TransferTrans.R_INST_TYPE, "VOUCHER");
                        txMap.put("AUTHORIZEREMARKS", "DP");
                        txMap.put("DEBIT_LOAN_TYPE", "DP");
                        System.out.println("txMap  ###" + txMap + "transferDao   :" + transferDao);
                        txMap.put("SINGLE_TRANS_ID", generateCashId);
                        ...
                        transferTo.setScreenName(CommonUtil.convertObjToStr(map.get(CommonConstants.SCREEN)));
                        transferList.add(transferTo);
                    } else if(! dataMap.get("CR_ACT_NUM").equals("NO_TRANSACTION_TO_SB")){
...
//                executeUpdate("updateTermLoanSecuritySalaryTO", objTermLoanSecuritySalaryTO);
            ...
            editTransaction(transMap);
             //kcc details insertion
            System.out.println("kccTo####"+kccTo);
            if(kccTo!=null && kccTo.getActNum().length()>0){
                sqlMap.executeUpdate("insertKccRenewalDetailsTO", kccTo);
            }
            /*
...
private void updateVehicleTableDetails() throws Exception {
        ...
        if (deletedVehicleTableValues!= null && deletedVehicleTableValues.size() > 0) {
            ...
            for (int i = 0; i < deletedVehicleTableValues.size(); i++) {
                ...
                objVehicleTO.setStatusBy(objTermLoanFacilityTO.getStatusBy());
                objVehicleTO.setStatusDt(curr_dt);
    ...
    private void updateMemberTableDetails() throws Exception {
        ...
        if (deletedMemberTableValues != null && deletedMemberTableValues.size() > 0) {
            ...
            for (int i = 0; i < deletedMemberTableValues.size(); i++) {
                ...
                objMemberTypeTO.setStatusBy(objTermLoanFacilityTO.getStatusBy());
                objMemberTypeTO.setStatusDt(curr_dt);
    ...
    private void updateCollateralTableDetails() throws Exception {
        ...
        if (deletedCollateralTableValues != null && deletedCollateralTableValues.size() > 0) {
            ...
            for (int i = 0; i < deletedCollateralTableValues.size(); i++) {
                ...
                objTermLoanSecurityLandTO.setStatusBy(objTermLoanFacilityTO.getStatusBy());
                objTermLoanSecurityLandTO.setStatusDt(objTermLoanFacilityTO.getStatusDt());
...
//                    objTermLoanDepositTypeTO.setAcctNum(acct_No);
        ...
        if (deletedDepositTableValues != null && deletedDepositTableValues.size() > 0) {
            ...
            for (int i = 0; i < deletedDepositTableValues.size(); i++) {
                ...
                hmap.put("CHITTAL_NO", objTermLoanFacilityTO.getAcctNum());
                ...
                objDepositTypeTO.setStatusDt(curr_dt);
                objDepositTypeTO.setStatusBy(objTermLoanFacilityTO.getStatusBy());
...
//                    objTermLoanDepositTypeTO.setAcctNum(acct_No);
        ...
        if (deletedLosTableValues != null && deletedLosTableValues.size() > 0) {
            ...
            for (int i = 0; i < deletedLosTableValues.size(); i++) {
                ...
                objLosTypeTO.setStatusBy(objTermLoanFacilityTO.getStatusBy());
                objLosTypeTO.setStatusDt(curr_dt);
...
//                logTO.setPrimaryKey(objTermLoanCourtDetailsTO.getKeyData());
    ...
    private boolean checkGoldSecurity(TermLoanFacilityTO objTermLoanFacilityTO) throws Exception {
        HashMap map = new HashMap();
        boolean flag = false;
        map.put("GROSS_WEIGHT", CommonUtil.convertObjToDouble(objTermLoanFacilityTO.getTxtGrossWeight()));
        map.put("NET_WEIGHT", CommonUtil.convertObjToDouble(objTermLoanFacilityTO.getTxtNetWeight()));
            ...
            for (int i = facilityMap.size() - 1, j = 0; i >= 0; --i, ++j) {
                ...
                if (objTermLoanFacilityTO.getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
                    ...
                    if (objTermLoanFacilityTO.getTxtValueOfGold() != null && objTermLoanFacilityTO.getTxtValueOfGold() > 0) {
                        if(objCustomerGoldStockSecurityTO != null && objCustomerGoldStockSecurityTO.getPledgeAmount() > 0){
                            objCustomerGoldStockSecurityTO.setAcctNum(acct_No);                            
                            executeUpdate("insertLoansSecurityGoldStockTO",objCustomerGoldStockSecurityTO);
                        }else{
...
//                    logDAO.addToLog(logTO);
                    ...
                    if (objTermLoanFacilityTO.getAuthorizeStatus1() != null && objTermLoanFacilityTO.getAuthorizeStatus1().length() > 0) {
                        ...
                        if(CommonUtil.convertObjToStr(objTermLoanFacilityTO.getAuthorizeStatus1()).length()<=0){ //Added by Revathi 20-feb-2024 to avoid authorize_status1 column value as ""
                            objTermLoanFacilityTO.setAuthorizeStatus1(null);
                        }
                    ...
                    if (objTermLoanFacilityTO.getTxtValueOfGold() != null && objTermLoanFacilityTO.getTxtValueOfGold() > 0) {
                        ...
                        if(objCustomerGoldStockSecurityTO != null && objCustomerGoldStockSecurityTO.getPledgeAmount() > 0 && checkGoldStockSecurityForLoan(objCustomerGoldStockSecurityTO)){
                            ...
                            releaseMap.put("RELEASE_DT",curr_dt.clone());
                            releaseMap.put("ACCT_NUM",objCustomerGoldStockSecurityTO.getAcctNum());
                            executeUpdate("updateLoansSecurityGoldStockReleaseDt",releaseMap);
                            executeUpdate("insertLoansSecurityGoldStockTO",objCustomerGoldStockSecurityTO);
                        }
                    }
//                    logDAO.addToLog(logTO);
        ...
        try {
            if (installmentMap != null && installmentMap.size() > 0) {
                ...
                for (int i = installmentMap.size() - 1, j = 0; i >= 0; --i, ++j) {
                    ...
                    if(objTermLoanInstallmentTO.getCommand().equals("DELETE")){
                        objTermLoanInstallmentTO.setActiveStatus("N");
                        sqlMap.executeUpdate("upadateActiveStatusForDeletedSchedule", objTermLoanInstallmentTO);
                    }
           ...
           // System.out.println("getdepositliento#####" + list);
            if (list != null && list.size() > 0) {
                for (int i = 0; i < list.size(); i++) {
                    ...
                    lienAuthMap.put("SHADOWLIEN", new Double(depositLienTO.getLienAmount().doubleValue())); // Added by nithya on 17-05-2021 for KD-2851
    ...
    private void authorize(HashMap map) throws Exception {
        try {
            ...
            for (int i = selectedList.size() - 1, j = 0; i >= 0; --i, ++j) {
                ...
                sqlMap.executeUpdate("updateLoanODCompleteStatus", aHmap);
...
//                        mdsUpdateMap.put("CHITTAL_NO", keyValue);
               ...
               getActNoMap.put("STATUS_DT",curr_dt);
                //SECURITY DETAILS
                if (i == 0) {
                    ...
                    sqlMap.executeUpdate("authorizeVehicleDetails", getActNoMap);
                    sqlMap.executeUpdate("authorizeSalarySecurityDetails", getActNoMap);
                }
...
//        cashAuthMap.put("DAILY", "DAILY");
        ...
        //added by rishad 21/oct/2019(status checking)
        if(status!=null && status.equalsIgnoreCase(CommonConstants.STATUS_AUTHORIZED)){
    ...
    public HashMap execute(HashMap map) throws Exception {
        ...
        //  System.out.println("map.get(CHK_UNLIEN)>>>"+map.get("CHK_UNLIEN"));
        
        
        if (map.containsKey("CHK_UNLIEN")) {
            if (map.get("CHK_UNLIEN").equals("Y")) {
                HashMap unlienMap = new HashMap();
                HashMap unlienMap1 = new HashMap();
                unlienMap = (HashMap) map.get("LTD");
            //    System.out.println("unlienMap???>>>>" + unlienMap);
//          unlienMap.get("DEPOSIT_PRODUCT");
                ...
                unlienMap1.put("PROD_ID", CommonUtil.convertObjToStr(unlienMap.get("DEPOSIT_PRODUCT")));
              //  System.out.println("unlienMap1????>>>" + unlienMap1);
                List unlienList = ServerUtil.executeQuery("getDepositUnlienCheck", unlienMap1);
               // System.out.println("unlienList in dao>>>" + unlienList);
                if (unlienList != null && unlienList.size() > 0) {
                    unlienMap1 = (HashMap) unlienList.get(0);
               //     System.out.println("unlienMap1 in dao>>>>" + unlienMap1);
                    String unlien = CommonUtil.convertObjToStr(unlienMap1.get("DEPOSIT_UNLIEN"));
                  //  System.out.println("unlien asss in dao>>>>" + unlien);
                    if (unlien.equals("Y")) {
                        HashMap cancelMap = new HashMap();
                        cancelMap.put("PROD_ID", unlienMap.get("LOAN_PRODUCT").toString());
                        cancelMap.put("DEPOSIT_NO", CommonUtil.convertObjToStr(unlienMap.get("DEPOSIT_NO").toString()));
                        lienCancel(cancelMap);
                    }
                }
            }
        ...
        if (map.containsKey("RECOVERY")) {
            recovery = CommonUtil.convertObjToStr(map.get("RECOVERY"));
          //  System.out.println("recovery in dao111>>>" + recovery);
        }
        if (map.containsKey("BALANCE_SHARE")) {
            String balShareAmt2 = CommonUtil.convertObjToStr(map.get("BALANCE_SHARE"));
            balShareAmt = CommonUtil.convertObjToDouble(balShareAmt2);
           // System.out.println("balShareAmt111>>>>" + balShareAmt);
        }
        if (map.containsKey("LOAN_TYPE")) {
            loanType = CommonUtil.convertObjToStr(map.get("LOAN_TYPE"));
            productCategory = CommonUtil.convertObjToStr(map.get("PRODUCT_CATEGORY"));
            keyValue = CommonUtil.convertObjToStr(map.get("KEYVALUE"));
        } else {
            loanType = "";
        }
        ...
        // added by nithya on 01-04-2016 for 3930 
        if(map.containsKey("REPAY_SCHEDULE_CREATE")){
          sqlMap.startTransaction();
          if(map.containsKey("UPDATE_REPAY_SCHEDULE_TABLE")){
             updateRepaymentSchedule(map);
          } 
          if(map.containsKey("UPDATE_LOANS_INSTALLMENT_TABLE")){
             updateScheduleInstallment(map); 
          }
          if(map.containsKey("TermLoanRepaymentTO")){
            repaymentMap = (HashMap) map.get("TermLoanRepaymentTO");
            insertRepaymentSchedule();
          }          
          if(map.containsKey("TermLoanInstallmentTO")){
             installmentMap = (HashMap) map.get("TermLoanInstallmentTO"); 
             insertInstallmentSchedule();
          }
          sqlMap.commitTransaction();
          destroyObjects();
        } 
       ...
       //    System.out.println("installmentAllmap" + installmentAllMap);
            ...
            //added for kcc by sreekrishnan
            if (map.containsKey("KccRenewalTO")) {
            	kccTo = (KccRenewalTO) map.get("KccRenewalTO");
        	}
            if (map.containsKey("DailyLoanSanctionDetailsTO")) {
                dailyLoanMap = (HashMap) map.get("DailyLoanSanctionDetailsTO");
            }
            if (map.containsKey("OTSCourtDetailsTO")) {
                oTSMap = (HashMap) map.get("OTSCourtDetailsTO");
            }
            ...
            if (map.containsKey("DepositSecurityTableDetails")) {
                ...
                securAmt = CommonUtil.convertObjToDouble(map.get("SECURITY_AMT")).doubleValue();
...
//            objTermLoanExtenFacilityDetailsTO=(TermLoanExtenFacilityDetailsTO)insideMap.get("TermLoanExtenFacilityDetailsTO");
            ...
            if (map.containsKey("LTD")) {
                ...
                multipleDeposit = (LinkedHashMap) depositCustDetMap.get("MULTIPLE_DEPOSIT");
                if (deleteLien == null) {
                    deleteLien = new LinkedHashMap();
                }
                deleteLien = (LinkedHashMap) depositCustDetMap.get("DELETE_LIEN");
                if (updateLien == null) {
                    updateLien = new LinkedHashMap();
                }
                updateLien = (LinkedHashMap) depositCustDetMap.get("UPDATE_DEPOSIT_LIEN");
           ...
           //    System.out.println("@##$#$% transDetailMap #### :" + transDetailMap);
                if(map.containsKey("SUSPENCE_ACCT_NO") && map.containsKey("SUSPENCE_PROD_ID")){
                    suspenceAccountNo = CommonUtil.convertObjToStr(map.get("SUSPENCE_ACCT_NO"));
                    suspenceProdId = CommonUtil.convertObjToStr(map.get("SUSPENCE_PROD_ID"));
                   //s System.out.println("suspenceAccountNo"+suspenceAccountNo+"suspenceProdId"+suspenceProdId);
                }
            ...
            if (map.containsKey("EDIT_TRANS_PART") && map.get("EDIT_TRANS_PART") != null) {
                transMap = (HashMap) map.get("EDIT_TRANS_PART");
            } else {
            ...
            if (map.containsKey(CommonConstants.AUTHORIZEMAP)) {
                ...
                if (authMap != null) {
                    authorize(map);
                    List authList = null;
                    authList = (List) authMap.get("AUTHORIZEDATA");
                    HashMap newMap = new HashMap();
                    newMap = (HashMap) authList.get(0);
                    String accNo = "";
                    accNo = CommonUtil.convertObjToStr(newMap.get("ACCT_NUM"));
                    returnMap.put("ACCTNO", accNo);
                }
            } else {
                if (command.equals(CommonConstants.TOSTATUS_INSERT)) {
                    insertData(map, transDetailMap,generateCashId);
                } else if (command.equals(CommonConstants.TOSTATUS_UPDATE)) {
                    if(map.containsKey("INT_GET_FROM") && map.get("INT_GET_FROM")!=null  ){
                        intGetFromUpdate=CommonUtil.convertObjToStr(map.get("INT_GET_FROM"));
                    }
                    if(map.containsKey("DELETE_INT_FLAG") && map.get("DELETE_INT_FLAG")!=null  ){
                        intDeleteFlag=CommonUtil.convertObjToStr(map.get("DELETE_INT_FLAG"));
                    }
                    updateData();
...
//                    hashMap.put("TODAY_DT", paramMap.get("TODAY_DT"));
    ...
    private void destroyObjects() {
        ...
        objCustomerGoldStockSecurityTO = null;
        chargeLst = null;
        behaves_like = null;
        kccTo = null;
    }
