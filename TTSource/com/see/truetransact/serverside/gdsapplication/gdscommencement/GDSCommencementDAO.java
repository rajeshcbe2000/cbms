 ...
 */
public class GDSCommencementDAO extends TTDAO {
     ...
     */
    public GDSCommencementDAO() throws ServiceLocatorException {
    ...
    private HashMap getData(HashMap map) throws Exception {
        ...
        List list = (List) sqlMap.executeQueryForList("getSelectGroupGLTransNotAuthGDSApplicationTO", map);
    ...
    private void insertDataWithBonus(HashMap map, LogDAO objLogDAO, LogTO objLogTO, double auction_maxamt, HashMap productMap) throws Exception {
        try {
            ...
            HashMap creditHeadMap = new HashMap();// Added by nithya on 16-04-2019 for KD-484
            String isBonusTransfer = "";
            int totalMembers = 0;
            double omitedBonusAmt = 0.0;
            ...
            if (map.containsKey("TransactionTO")) {
                ...
                map.put("GROUP_NO",map.get("SCHEME_NAME"));
                List totalList = sqlMap.executeQueryForList("getSelectGLTransGDSMDSApplicationTO", map);
                if (totalList != null && totalList.size() > 0) {
                    ...
                    List lst = sqlMap.executeQueryForList("getSelectGDSGroupAcctHead", applicationMap);
                     if (lst != null && lst.size() > 0) {                         
                         ...
                         totalSuspendsAmt = (totalTransAmt - totalBonusAmt)/schemeCount;
                         totalBonusAmt = totalBonusAmt/schemeCount;
                         omitedBonusAmt = totalBonusAmt;
                         System.out.println("omitedBonusAmt :: "+ omitedBonusAmt);
                         for (int i = 0; i < lst.size(); i++) {
                             applicationMap = (HashMap) lst.get(i);
                             totalMembers = CommonUtil.convertObjToInt(applicationMap.get("TOTAL_NO_OF_MEMBERS"));
                             creditHeadMap.put(applicationMap.get("SCHEME_NAME"),applicationMap.get("RECEIPT_HEAD"));// Added by nithya on 16-04-2019 for KD-484
                             txMap = new HashMap();
                    ...
                    for (int i = 0; i < totalList.size(); i++) {
                        ...
                        if (transactionTO.getTransType().equals("TRANSFER")) {
                            ...
                            if (CommonUtil.convertObjToDouble(mdsApplicationTO.getInstAmt()).doubleValue() > 0) {
                                ...
                                //txMap.put(TransferTrans.CR_AC_HD, applicationMap.get("RECEIPT_HEAD"));// Added by nithya on 16-04-2019 for KD-484
                                txMap.put(TransferTrans.CR_AC_HD, creditHeadMap.get(mdsApplicationTO.getSchemeName()));// Added by nithya on 16-04-2019 for KD-484
                                txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                                txMap.put(TransferTrans.PARTICULARS, mdsApplicationTO.getSchemeName());
                                ...
                                if(!isBonusTransfer.equalsIgnoreCase("N")){
                                   ...
                                   transferTo = transactionDAO.addTransferCreditLocal(txMap, CommonUtil.convertObjToDouble(mdsApplicationTO.getInstAmt()).doubleValue() - (omitedBonusAmt/totalMembers));
                                } 
                    ...
                    sqlMap.executeUpdate("updateGDSCommencementTransId", transMap);
                    linkBatchMap = null;
                }
            }
                    ...
                    for (int i = 0; i < totalList.size(); i++) {
                        ...
                        if (transactionTO.getTransType().equals("TRANSFER")) {
                            ...
                            if (CommonUtil.convertObjToDouble(mdsApplicationTO.getInstAmt()).doubleValue() > 0) {
                                ...
                                txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                                txMap.put(TransferTrans.PARTICULARS, mdsApplicationTO.getSchemeName() + "-" + mdsApplicationTO.getChitNo());
                                txMap.put(TransferTrans.CR_BRANCH, BRANCH_ID);
                                txMap.put("SCREEN_NAME",CommonUtil.convertObjToStr(map.get(CommonConstants.SCREEN)));
                                System.out.println("txMap : " + txMap + "serviceAmt :" + CommonUtil.convertObjToDouble(mdsApplicationTO.getInstAmt()).doubleValue());
                                transferTo = transactionDAO.addTransferCreditLocal(txMap, CommonUtil.convertObjToDouble(mdsApplicationTO.getInstAmt()).doubleValue());
        ...
        try {
            ...
            if (map.containsKey("TransactionTO")) {
                ...
                if (totalList != null && totalList.size() > 0) {
                    ...
                    if (lst != null && lst.size() > 0) {
                        int schemeCount = lst.size();
                        transactionTO.setTransAmt(transactionTO.getTransAmt()/schemeCount);
                        System.out.println("inside GDSCommencementDAO :: insertData ::" + transactionTO.getTransAmt());
                        for (int i = 0; i < lst.size(); i++) {
                            applicationMap = (HashMap) lst.get(i);
                            creditHeadMap.put(applicationMap.get("SCHEME_NAME"),applicationMap.get("RECEIPT_HEAD"));// Added by nithya on 16-04-2019 for KD-484
                            txMap = new HashMap();
                    ...
                    map.put("COMMAND", map.get("MODE"));
                    map.put("TxTransferTO", TxTransferTO);
                     System.out.println("inside GDSCommencementDAO :: insertData :: final map " + map);
                    transMap = transferDAO.execute(map, false); 
                    System.out.println("transactionDAO map : " + map);
                    transactionDAO.setBatchId(CommonUtil.convertObjToStr(transMap.get("TRANS_ID")));
                    transactionDAO.setBatchDate(currDt);
                    transactionDAO.execute(map); //commented by nithya for testing purpose
                    HashMap linkBatchMap = new HashMap();
                    HashMap remitMap = new HashMap();
//                    lst = sqlMap.executeQueryForList("getSelectMaxTransIdForCash",null);
//                    if(lst!=null && lst.size()>0){
...
//                            mdsApplicationTO.setStatus("DELETED");
   ...
   private void authorizePredefined(HashMap map) throws Exception {
        ...
        if (map.containsKey("TransactionTO")) {
            List productLst = sqlMap.executeQueryForList("getSelectGDSGroupAcctHead", map);
            ...
            if (transactionDetailsMap.size() > 0 && transactionDetailsMap.containsKey("NOT_DELETED_TRANS_TOs")) {
                ...
                List totalList = sqlMap.executeQueryForList("getSelectGroupGLTransNotAuthGDSApplicationTO", map);
                if (totalList != null && totalList.size() > 0) {
                    System.out.println("###### : " + totalList.size());
                        ...
                        if (authorizeStatus.equals("AUTHORIZED")) {                           
                            ...
                            mdsTransMap.put("CHITTAL_NO", mdsApplicationTO.getChittalNo());
                             if(bonusFirstInst){
                                 ...
                                 amtMap.put("TRANS_DT",currDt.clone());
                                 amtMap.put("GL_TRANS_ACT_NUM", mdsApplicationTO.getChittalNo());
                                 amtMap.put("BATCH_ID",mdsApplicationTO.getCommencementTransId());
                                 List amtLst = sqlMap.executeQueryForList("getInstalmmentAmtForBonusFrstInstallment",amtMap);
                                 if(amtLst != null && amtLst.size() > 0){
                                     HashMap amtLstMap = (HashMap)amtLst.get(0);
                                     if(amtLstMap.containsKey("AMOUNT") && amtLstMap.get("AMOUNT") != null && !amtLstMap.get("AMOUNT").equals(""))
                                        mdsApplicationTO.setInstAmt(CommonUtil.convertObjToDouble(amtLstMap.get("AMOUNT")));
                                 }
                             }
            ...
            if (transactionDetailsMap.size() > 0 && transactionDetailsMap.containsKey("NOT_DELETED_TRANS_TOs")) {
                ...
                if (totalList != null && totalList.size() > 0) {
                    ...
                    for (int i = 0; i < totalList.size(); i++) {
                        ...
                        gdsApplicationTO = (GDSApplicationTO) totalList.get(i);
                        if (authorizeStatus.equals("AUTHORIZED")) {
                            gdsApplicationTO.setShadowCredit(gdsApplicationTO.getInstallmentAmount());
                            sqlMap.executeUpdate("updateMDSShadowCreditCrAmt", gdsApplicationTO);
                            gdsApplicationTO.setTotalBalance(gdsApplicationTO.getInstallmentAmount());
                            gdsApplicationTO.setClearBalance(gdsApplicationTO.getInstallmentAmount());
                            gdsApplicationTO.setAvailableBalance(gdsApplicationTO.getInstallmentAmount());
                            System.out.println("########" + gdsApplicationTO);
                            sqlMap.executeUpdate("updateMDSShadowCreditDrAmt", gdsApplicationTO);
                            sqlMap.executeUpdate("updateMDSAvailBalance", gdsApplicationTO);
                            sqlMap.executeUpdate("updateMDSClearBalance", gdsApplicationTO);
                            sqlMap.executeUpdate("updateMDSTotalBalance", gdsApplicationTO);
                            applnAuthMap.put("COMMENCEMENT_AUTHORIZE_STATUS", authorizeStatus);
                            applnAuthMap.put("LAST_TRANS_DT", currDt);
                            applnAuthMap.put("TRANS_ID", gdsApplicationTO.getTransId());
                            applnAuthMap.put("SCHEME_NAME", gdsApplicationTO.getSchemeName());
                            applnAuthMap.put("INST_COUNT", new Double(1));
                            System.out.println("$#@$#@#@$#@applnAuthMap :" + applnAuthMap);
                            sqlMap.executeUpdate("updateMDSCommencementAuthStatus", applnAuthMap);

                            HashMap mdsTransMap = new HashMap();
                            mdsTransMap.put("GDS_NO", gdsApplicationTO.getGds_No());
                            mdsTransMap.put("SCHEME_NAME", gdsApplicationTO.getSchemeName());
                            mdsTransMap.put("CHITTAL_NO", gdsApplicationTO.getChittalNo());
                             if(bonusFirstInst){
                                 HashMap amtMap = new HashMap();
                                 amtMap.put("TRANS_DT",currDt.clone());
                                 amtMap.put("GL_TRANS_ACT_NUM", gdsApplicationTO.getChittalNo());
                                 amtMap.put("BATCH_ID",gdsApplicationTO.getCommencementTransId());
                                 List amtLst = sqlMap.executeQueryForList("getInstalmmentAmtForBonusFrstInstallment",amtMap);
                                 if(amtLst != null && amtLst.size() > 0){
                                     HashMap amtLstMap = (HashMap)amtLst.get(0);
                                     if(amtLstMap.containsKey("AMOUNT") && amtLstMap.get("AMOUNT") != null && !amtLstMap.get("AMOUNT").equals(""))
                                        gdsApplicationTO.setInstallmentAmount(CommonUtil.convertObjToDouble(amtLstMap.get("AMOUNT")));
                                 }
                             }
                            mdsTransMap.put("INST_AMT", gdsApplicationTO.getInstallmentAmount());
                            mdsTransMap.put("NO_OF_INST", new Integer(1));
                            mdsTransMap.put("PENAL_AMT", new Double(0.0));
//                            mdsTransMap.put("BONUS_AMT", 0);
//                            mdsTransMap.put("DISCOUNT_AMT", 0);
                            mdsTransMap.put("BONUS_AMT", new Double(0.0)); // Modified by nithya on 16-01-2020 for KD-1280
                            mdsTransMap.put("DISCOUNT_AMT", new Double(0.0));
                            mdsTransMap.put("MDS_INTEREST", new Double(0.0));                           
                            mdsTransMap.put("NET_AMT", gdsApplicationTO.getInstallmentAmount());
                            mdsTransMap.put("STATUS", CommonConstants.STATUS_CREATED);
                            mdsTransMap.put("STATUS_DT", currDt);
                            mdsTransMap.put("STATUS_BY", map.get("USER_ID"));
                            mdsTransMap.put("AUTHORIZE_STATUS", authorizeStatus);
                            ...
                            mdsTransMap.put("NET_TRANS_ID", gdsApplicationTO.getCommencementTransId());
                            //mdsTransMap.put("SUB_NO", gdsApplicationTO.getSubNo());
                            mdsTransMap.put("SUB_NO", CommonUtil.convertObjToInt(gdsApplicationTO.getSubNo()));
                            Date cmDt=gdsApplicationTO.getCommencementDate();
                            String mnthNdYr ="";
                            if (cmDt != null) {
                                int m = DateUtil.getMonth(cmDt)-1;
                                DateFormatSymbols dfs = new DateFormatSymbols();
                            ...
                            //Added by sreekrishnan for sms alert..
                                HashMap smsAlertMap = new HashMap();
                                SmsConfigDAO smsDAO = new SmsConfigDAO();
                                smsAlertMap.put("SCHEME_NAME", gdsApplicationTO.getSchemeName());
                                smsAlertMap.put("CHITTAL_NO", gdsApplicationTO.getChittalNo());
                                System.out.println("smsAlertMap%#%#%#%#%#%  " + smsAlertMap);
                                List MdsSmsList = sqlMap.executeQueryForList("getMDsDetailsForSMS", smsAlertMap);
                                if (MdsSmsList != null && MdsSmsList.size() > 0) {
                                    HashMap MdsSmsMap = (HashMap) MdsSmsList.get(0);
                                    System.out.println("MdsSmsMap%#%#%#^#^#^#^" + MdsSmsMap);
                                    if (MdsSmsMap != null && !MdsSmsMap.equals("")) {
                                        MdsSmsMap.put(CommonConstants.TRANS_ID, gdsApplicationTO.getCommencementTransId());
                            ...
                            //                            commencementMap.put("COMMENCEMENT_STATUS","");
                            //                            commencementMap.put("COMMENCEMENT_DATE",null);
                            commencementMap.put("TRANS_ID", gdsApplicationTO.getTransId());
                            sqlMap.executeUpdate("updateCommencementStatusNull", commencementMap);
                        }
                    }
                    if (transactionTO.getTransType().equals("TRANSFER")) {
    ...
    public static void main(String str[]) {
        try {
            GDSCommencementDAO dao = new GDSCommencementDAO();
        ...
        if (map.containsKey("MDS_CLOSURE")) {
            ...
            try {
                ...
                if (command.equals(CommonConstants.TOSTATUS_INSERT)) {
                    ...
                    String commonScheme = CommonUtil.convertObjToStr(commonSchemeMap.get("SCHEME_NAME")); HashMap checkMap = new HashMap();
