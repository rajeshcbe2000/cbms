 ...
 */
public class GDSMasterMaintenanceDAO extends TTDAO {

    private final static Logger log = Logger.getLogger(GDSMasterMaintenanceDAO.class);
    ...
    private GDSMasterMaintenanceTO MaintenanceTO;
...
//    private TransactionDAO transactionDAO = null ; //trans details
    private GDSMasterMaintenanceTO objTO;

    /**
     * Creates a new instance of OperativeAcctProductDAO
     */
    public GDSMasterMaintenanceDAO() throws ServiceLocatorException {
    ...
    private HashMap getData(HashMap map) throws Exception {
       ...
       // map.put("SUB_NO", String.valueOf(subNo));
        map.put("SUB_NO", subNo);
        List masterList = (List) sqlMap.executeQueryForList("getSelectGDSMasterMaintenanceTO", map);        
        List getChittalLst = (List) sqlMap.executeQueryForList("getSelectOneChittalNoForGDSNo", map);
        String CommonChittalNo = CommonUtil.convertObjToStr(((HashMap)getChittalLst.get(0)).get("CHITTAL_NO"));
        map.put("CHITTAL_NO",CommonChittalNo);
        ...
        if(masterList != null && masterList.size() >0 ){
        	MaintenanceTO = (GDSMasterMaintenanceTO) masterList.get(0);
        }
...
//            list = sqlMap.executeQueryForList("getSelectRemittanceIssueTransactionTODate", where.get("REP_INT_CLS_NO"));
    ...
    public HashMap execute(HashMap map) throws Exception {
        ...
        logDAO = new LogDAO();
        logTO = new LogTO();
        MaintenanceTO = (GDSMasterMaintenanceTO) map.get("MasterMaintenanceDetailsTOData");
        objTO = (GDSMasterMaintenanceTO) map.get("MasterMaintenanceDetailsTOData");
...
//            insertData(map, objLogDAO, objLogTO);
            ...
            // Need to iterate the transaction----- nithya
    ...
    private void updateData(HashMap map, LogDAO objLogDAO, LogTO objLogTO) throws Exception {
        try {
            ...
            whereMap.put("GROUP_NO", MaintenanceTO.getSchemeName());
            whereMap.put("GDS_NO", MaintenanceTO.getChittalNo());
            ...
            applicationMap.put("GDS_NO",MaintenanceTO.getChittalNo());
            ...
            List existList = (List) sqlMap.executeQueryForList("getSelectGDSMasterData", whereMap);
            if (existList != null && existList.size() > 0) {
                sqlMap.startTransaction();                   
                for (int i = 0; i < allApplnDetails.size(); i++) { 
                    GDSApplicationTO objGDSApplicationTO = (GDSApplicationTO) allApplnDetails.get(i);
                    MaintenanceTO.setChittalNo(objGDSApplicationTO.getChittalNo());
                    MaintenanceTO.setSchemeName(objGDSApplicationTO.getSchemeName());
                    ...
                    updateMemberTableDetails(); // Completed for GDS
                    updateDepositTableDetails();// Completed for GDS
                ...
                updateMdsGoldSecurity(MaintenanceTO);
                sqlMap.commitTransaction();
            } else {
                insertData(map, objLogDAO, objLogTO);
    ...
    private void updateCollateralTableDetails() throws Exception {
        if (collateralTableDetails != null && collateralTableDetails.size() > 0) {
            ...
            for (int i = 0; i < collateralTableDetails.size(); i++) {
                ...
                objMDSMasterSecurityLandTO.setChittalNo(MaintenanceTO.getChittalNo());
    ...
    private void updateMemberTableDetails() throws Exception {
        if (deletedMemberTableValues != null) {
            ...
            for (int i = 0; i < deletedMemberTableValues.size(); i++) {
                ...
                objMemberTypeTO.setChittalNo(MaintenanceTO.getChittalNo());                
        ...
        if (memberTableDetails != null) {
            ...
            for (int i = 0; i < memberTableDetails.size(); i++) {
                ...
                objMemberTypeTO.setStatusDt(getProperDateFormat(CurrDt));
                objMemberTypeTO.setChittalNo(MaintenanceTO.getChittalNo());
//                double networth = CommonUtil.convertObjToDouble(objMemberTypeTO.getNetworth()).doubleValue()/schemeCount; 
        ...
        if (depositTableDetails != null) {
            ...
            for (int i = 0; i < depositTableDetails.size(); i++) {
                ...
                if (objDepositTypeTO.getStatus().equals(CommonConstants.STATUS_CREATED)) {
                    objDepositTypeTO.setChittalNo(MaintenanceTO.getChittalNo());
                    ...
                    hmap.put("CHITTAL_NO", objDepositTypeTO.getChittalNo());
                    hmap.put("GDS_NO", objDepositTypeTO.getChittalNo());
                    //hmap.put("SUB_NO", objDepositTypeTO.getSubNo());
                    hmap.put("SUB_NO", CommonUtil.convertObjToInt(objDepositTypeTO.getSubNo()));// Added by nithya on 16-01-2020 for KD-1279
                    List list = sqlMap.executeQueryForList("getAvailableBalForDep", hmap);
                    if (i == 0) {
                        double noinset = 0.0;
                        double instAmt = 0.0;
                        ...
                        List mdsList1 = sqlMap.executeQueryForList("getGDSSelctApplnReceiptDetails", hmap);
...
//                        }
                    ...
                    if (objDepositTypeTO.getProdType().equals("TD") || objDepositTypeTO.getProdType().equals("Deposits")) {
                        if (prizedAmount > 0.0) {
    ...
    private void authorize(HashMap map,HashMap Actmap) throws Exception {
        ...
        try {
            ...
            // Iterating for authorization
            HashMap applicationMap = new HashMap();
            applicationMap.put("GDS_NO",AuthMap.get("CHITTAL_NO"));
            List allApplnDetails = sqlMap.executeQueryForList("getAllApplcationDetailsForGDS", applicationMap);
            if(allApplnDetails != null && allApplnDetails.size() > 0){
              for(int k=0; k<allApplnDetails.size(); k++){
                  GDSApplicationTO objGDSApplicationTO = (GDSApplicationTO) allApplnDetails.get(k);
                  AuthMap.put("CHITTAL_NO", objGDSApplicationTO.getChittalNo());
                  AuthMap.put("SCHEME_NAME", objGDSApplicationTO.getSchemeName());
                  ...
                  amap.put("SUB_NO", CommonUtil.convertObjToInt(AuthMap.get("SUB_NO")));
...
//                    System.out.println("transactioninvest1111@@@@44444>>>>>>>" + (TransactionTO) allowedTransDetailsTO.get(String.valueOf(1)));
              ...
              aHmap.put("acctNum", AuthMap.get("GDS_NO"));
    ...
    public static void main(String str[]) {
        try {
            GDSMasterMaintenanceDAO dao = new GDSMasterMaintenanceDAO();
    ...
    private void updateMdsSalarySecurity(GDSMasterMaintenanceTO MaintenanceTO) throws Exception {
    ...
    private boolean checkGoldSecurity(GDSMasterMaintenanceTO MaintenanceTO) throws Exception {
        HashMap map = new HashMap();
        boolean flag = false;
        map.put("GROSS_WEIGHT", CommonUtil.convertObjToDouble(MaintenanceTO.getGrossWeight()));
        map.put("NET_WEIGHT", CommonUtil.convertObjToDouble(MaintenanceTO.getNetWeight()));
    ...
    private void updateMdsGoldSecurity(GDSMasterMaintenanceTO MaintenanceTO) throws Exception {
        //To change body of generated methods, choose Tools | Templates.
        if (MaintenanceTO.getGoldValue() != null && CommonUtil.convertObjToDouble(MaintenanceTO.getGoldValue()) > 0) {
                ...
                objTermLoanSecurityTO.setAcctNum(MaintenanceTO.getGdsNo());
                objTermLoanSecurityTO.setStatus(CommonConstants.STATUS_CREATED);
                objTermLoanSecurityTO.setAsOn((Date)CurrDt.clone());
                objTermLoanSecurityTO.setGrossWeight(CommonUtil.convertObjToDouble(MaintenanceTO.getGrossWeight()));
                objTermLoanSecurityTO.setNetWeight(CommonUtil.convertObjToDouble(MaintenanceTO.getNetWeight()));
            ...
            if(checkGoldSecurity(MaintenanceTO)){
    ...
    private void insertMdsSalarySecurity(GDSMasterMaintenanceTO MaintenanceTO) throws Exception {
        for (int i = 0; i < SalarySecDetails.size(); i++) {
            ...
            MaintenanceTO.setPin(CommonUtil.convertObjToInt(docMap.get("PIN")));
            ...
            MaintenanceTO.setTotalSalary(CommonUtil.convertObjToDouble(docMap.get("TOTAL_SALARY")));
