 ...
 *
 * Created on February 15, 2018, 4:14 PM
 */
package com.see.truetransact.serverside.gdsapplication.gdsreceiptentry;

import com.see.truetransact.serverside.mdsapplication.mdsreceiptentry.*;
...
/**
 * This is used for GDSReceiptEntryDAO Data Access.
 *
 * @author Nithya
 *
 */
public class GDSReceiptEntryDAO extends TTDAO {

    private static SqlMap sqlMap = null;
    private TransactionDAO transactionDAO = null;
    private GDSReceiptEntryTO gdsReceiptEntryTO = null;
     ...
     */
    public GDSReceiptEntryDAO() throws ServiceLocatorException {
    ...
    private HashMap getData(HashMap map) throws Exception {
        System.out.println("Inside GDSReceiptEntryDAO :: getdata ::" + map);
        isSplitMDSTransaction = "";
        ...
        List list = (List) sqlMap.executeQueryForList("getGDSReceiptEntryTO", editMap);
        if(list == null || list.size()<=0){
            editMap.put("SINGLE_TRANS_ID", CommonUtil.convertObjToStr(map.get("SINGLE_TRANS_ID")));
             list = (List) sqlMap.executeQueryForList("getGDSReceiptEntryTOForAdv", editMap);
        }
        getReturnMap.put("gdsReceiptEntryTO", list);
        gdsReceiptEntryTO = new GDSReceiptEntryTO();
        if (map.containsKey("VOUCHER_DETAILS")) {
            returnMap = new HashMap();
            gdsReceiptEntryTO = (GDSReceiptEntryTO) ((List) getReturnMap.get("gdsReceiptEntryTO")).get(0);
            String netTransID = "";
            if(map.containsKey("NET_TRANS_ID") && map.get("NET_TRANS_ID") != null){
                netTransID = CommonUtil.convertObjToStr(map.get("NET_TRANS_ID"));
            }            
            String bonusTransID = gdsReceiptEntryTO.getBonusTransId();
            if (netTransID != null && netTransID.indexOf(",") != -1) { // KD-2257
                gdsReceiptEntryTO.setNetTransId(netTransID.substring(0, netTransID.indexOf(",")));
                if (bonusTransID!= null && bonusTransID.indexOf(",") != -1) {
                    gdsReceiptEntryTO.setBonusTransId(bonusTransID.substring(0, bonusTransID.indexOf(",")));
                }
                setTransDetails(gdsReceiptEntryTO);
                gdsReceiptEntryTO.setNetTransId(netTransID.substring(netTransID.indexOf(",") + 1, netTransID.length()));
                if (bonusTransID != null && bonusTransID.indexOf(",") != -1) {
                    gdsReceiptEntryTO.setBonusTransId(bonusTransID.substring(bonusTransID.indexOf(",") + 1, bonusTransID.length()));
                }
            }
            setTransDetails(gdsReceiptEntryTO);
            getReturnMap.put("VOUCHER_DATA", returnMap);
            gdsReceiptEntryTO.setNetTransId(netTransID);
            gdsReceiptEntryTO.setBonusTransId(bonusTransID);
        }
        ...
        String transId = "";
        if(map.containsKey("NET_TRANS_ID") && map.get("NET_TRANS_ID") != null){
            transId = CommonUtil.convertObjToStr(map.get("NET_TRANS_ID"));
        }
        ...
        if (transactionMap != null && transactionMap.containsKey("PENAL_TRANS_ID") && transactionMap.get("PENAL_TRANS_ID") != null && !transactionMap.get("PENAL_TRANS_ID").equals("")) {
        ...
        if (transactionMap != null && transactionMap.containsKey("ARBITRATION_ID") && transactionMap.get("ARBITRATION_ID") != null && !transactionMap.get("ARBITRATION_ID").equals("")) {
        ...
        if (transactionMap != null && transactionMap.containsKey("NOTICE_ID") && transactionMap.get("NOTICE_ID") != null && !transactionMap.get("NOTICE_ID").equals("")) {
        ...
        if (transactionMap != null && transactionMap.containsKey("DISCOUNT_TRANS_ID") && transactionMap.get("DISCOUNT_TRANS_ID") != null && !transactionMap.get("DISCOUNT_TRANS_ID").equals("")) {
    ...
    private void insertData(HashMap map, LogDAO objLogDAO, LogTO objLogTO) throws Exception {
        try {
            System.out.println("Inside GDSReceiptEntryDAO :: insertData :: map :: "+ map);
            ...
            TxTransferTO = new ArrayList(); // Added by nithya on 05-12-2019 for KD-1021
            ...
            instNo = CommonUtil.convertObjToInt(gdsReceiptEntryTO.getCurrInst()) - CommonUtil.convertObjToInt(gdsReceiptEntryTO.getPendingInst());
            bankAdvMap.put("SCHEME_NAME", commonScheme);
            bankAdvMap.put("GROUP_NO",map.get("GROUP_NO"));
            bankAdvMap.put("CHITTAL_NO", gdsReceiptEntryTO.getChittalNo());
            bankAdvMap.put("GDS_NO",map.get("GDS_NO"));
            bankAdvMap.put("SUB_NO", CommonUtil.convertObjToInt(gdsReceiptEntryTO.getSubNo()));
            bankAdvMap.put("INSTALLMENT_NO", instNo);
            GDSReceiptEntryTO mdsBankAdvReceiptEntryTO = null;
            GDSReceiptEntryTO mdsCurrentReceiptEntryTO = null;
...
//                        if (i == 1 && (gdsReceiptEntryTO.getPendingInst() + 1 == gdsReceiptEntryTO.getNoOfInstPay())) {    // //Commented by nithya on 17-10-2018 for KD 285 - Bonus amount not deducting in mds transaction (bank advance enabled bank) Kuttilanji scb                      
//                            bonusFirst = bonusNew;//*pay;
//                        }//                      
                        //if (gdsReceiptEntryTO.getBonusAmtPayable() == 0.0) {
                        if(gdsReceiptEntryTO.getForfeitBonusAmtPayable() > 0){
                            bonusreversal = true;
                    ...
                    if (gdsReceiptEntryTO.getPendingInst() + 1 == gdsReceiptEntryTO.getNoOfInstPay()) {//Added by nithya on 17-10-2018 for KD 285 - Bonus amount not deducting in mds transaction (bank advance enabled bank) Kuttilanji scb
                        bonusFirst = bonusNew;
                    }
                }
                //bankBonusAmt = bankBonusAmt * bankAdvanceLst.size();
               // bankAdvInsAmt = bankAdvInsAmt * bankAdvanceLst.size();
                bankBonusAmt = bankBonusAmt * commonSchemeDetailsLst.size();
                bankAdvInsAmt = bankAdvInsAmt * commonSchemeDetailsLst.size();
                System.out.println("bankBonusAmt receipt:: "+ bankBonusAmt);
                System.out.println("bankAdvInsAmt :: "+ bankAdvInsAmt);
                mdsBankAdvReceiptEntryTO = getgdsReceiptEntryTO(bankBonusAmt, bankAdvInsAmt);
                mdsCurrentReceiptEntryTO = (GDSReceiptEntryTO) map.get("gdsReceiptEntryTO");
            ...
            if (instalmentPayAmt > 0 || (bankAdvanceLst == null || bankAdvanceLst.size() <= 0)) { 
                System.out.println("Executing here  instalmentPayAmt > 0 ");
                if (bankAdvInsAmt > 0) {             
                    if (futureAdvbonusAmt <= 0) {
                        futureAdvbonusAmt = bonusFirst;
                    }
                    mdsCurrentReceiptEntryTO = getgdsReceiptEntryTO(futureAdvbonusAmt, instalmentPayAmt);
            ...
            gdsReceiptEntryTO.setBankAdvanceAmt(bankAdvInsAmt-bankBonusAmt);
             // ------------------------- Main code Ends---------------------------------
            if (instalmentPayAmt == 0 && bankAdvanceLst != null && bankAdvanceLst.size() > 0) {
                System.out.println("Executing here  instalmentPayAmt = 0 if bank advance");
                String tran_type = "";
                ...
                if (map.containsKey("TransactionTO")) {
                  ...
                  mdsCurrentReceiptEntryTO = (GDSReceiptEntryTO) map.get("gdsReceiptEntryTO");
                System.out.println("TxTransferTO :: before ::"+ TxTransferTO); 
                if (tran_type != null && tran_type.equals("TRANSFER")) {
                    ...
                    for(int n=0;n<TxTransferTO.size();n++){
                         ...
                         if(transferTo.getTransType()!=null && transferTo.getTransType().equals("DEBIT")){
                             ...
                             transferTo.setInpAmount(mdsCurrentReceiptEntryTO.getNetAmt());
                             transferTo.setAmount(mdsCurrentReceiptEntryTO.getNetAmt());
                             transferTo.setScreenName((String) map.get(CommonConstants.SCREEN));
                         }
                         templist.add (transferTo);
                    }
                    System.out.println("TxTransferTO :: after ::"+ TxTransferTO); 
                    ...
                    mdsBankAdvReceiptEntryTO.setNoticeId(CommonUtil.convertObjToStr(transMap.get("TRANS_ID")));
                    mdsBankAdvReceiptEntryTO.setArbitrationId(CommonUtil.convertObjToStr(transMap.get("TRANS_ID")));
            ...
            // The below code should be itereted inside loop
            if (gdsReceiptEntryTO != null) { //INSERT RECIEPT TABLE
                if (bankAdvInsAmt > 0) {
                    if (bankAdvInsAmt > 0 && instalmentPayAmt > 0) {
                        //System.out.println("###### INSERT BOTH MDS_RECEIPT TRANS_ID ");
                        if (map.containsKey("BANK_ADVANCE_NET_TRANS_ID") && map.get("BANK_ADVANCE_NET_TRANS_ID") != null && CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_NET_TRANS_ID")).length() > 0) { // Added by nithya on 14-01-2020 for KD-1183
                           gdsReceiptEntryTO.setNetTransId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_NET_TRANS_ID")) + "," + CommonUtil.convertObjToStr(map.get("CURRENT_NET_TRANS_ID"))); 
                        }else{
                           gdsReceiptEntryTO.setNetTransId(CommonUtil.convertObjToStr(map.get("CURRENT_NET_TRANS_ID")));
                        }
                        if (map.containsKey("BANK_ADVANCE_BONUS_TRANS_ID") && map.get("BANK_ADVANCE_BONUS_TRANS_ID") != null && CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_BONUS_TRANS_ID")).length() > 0) { // Added by nithya on 14-01-2020 for KD-1183
                            gdsReceiptEntryTO.setBonusTransId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_BONUS_TRANS_ID")) + "," + CommonUtil.convertObjToStr(map.get("CURRENT_BONUS_TRANS_ID")));
                        }else{
                            gdsReceiptEntryTO.setBonusTransId(CommonUtil.convertObjToStr(map.get("CURRENT_BONUS_TRANS_ID")));
                        }
                        gdsReceiptEntryTO.setPenalTransId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_PENAL_TRANS_ID")));
                        gdsReceiptEntryTO.setNoticeId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_NOTICE_TRANS_ID")));
                        gdsReceiptEntryTO.setArbitrationId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_ARBITRATION_TRANS_ID")));
                        gdsReceiptEntryTO.setDiscountTransId(CommonUtil.convertObjToStr(map.get("CURRENT_DISCOUNT_TRANS_ID")));
                        if(map.containsKey("BANK_ADVANCE_FORFEITBONUS_TRANS_ID") && map.get("BANK_ADVANCE_FORFEITBONUS_TRANS_ID") != null){ //2093
                         gdsReceiptEntryTO.setForfeitBonusTransId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_FORFEITBONUS_TRANS_ID")));
                        }else{
                         gdsReceiptEntryTO.setForfeitBonusTransId("");   
                        }
                    } else {
                        System.out.println("###### INSERT ONLY BANKA_ADVANCE MDS_RECEIPT TRANS_ID : ");
                        gdsReceiptEntryTO.setNetTransId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_NET_TRANS_ID")));
                        gdsReceiptEntryTO.setBonusTransId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_BONUS_TRANS_ID")));
                        gdsReceiptEntryTO.setPenalTransId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_PENAL_TRANS_ID")));
                        gdsReceiptEntryTO.setNoticeId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_NOTICE_TRANS_ID")));
                        gdsReceiptEntryTO.setArbitrationId(CommonUtil.convertObjToStr(map.get("BANK_ADVANCE_ARBITRATION_TRANS_ID")));
                        if(map.containsKey("BANK_ADVANCE_FORFEITBONUS_TRANS_ID") && map.get("BANK_ADVANCE_FORFEITBONUS_TRANS_ID") != null){//2093
                    ...
                    gdsReceiptEntryTO.setNetTransId(CommonUtil.convertObjToStr(map.get("CURRENT_NET_TRANS_ID")));
                    if (gdsReceiptEntryTO.getBonusAmtPayable() > 0) {
                        gdsReceiptEntryTO.setBonusTransId(CommonUtil.convertObjToStr(map.get("CURRENT_BONUS_TRANS_ID")));
                    } else if (gdsReceiptEntryTO.getPenalAmtPayable() > 0)//bbbbb
                    {
                        gdsReceiptEntryTO.setPenalTransId(CommonUtil.convertObjToStr(map.get("CURRENT_PENAL_TRANS_ID")));
                    } else {
                        gdsReceiptEntryTO.setPenalTransId("");
                    }
                    if (gdsReceiptEntryTO.getDiscountAmt() > 0) {
                        gdsReceiptEntryTO.setDiscountTransId(CommonUtil.convertObjToStr(map.get("CURRENT_DISCOUNT_TRANS_ID")));
                    } else {
                        gdsReceiptEntryTO.setDiscountTransId("");
                    }

                    gdsReceiptEntryTO.setNoticeId(CommonUtil.convertObjToStr(map.get("CURRENT_NOTICE_TRANS_ID")));
                    gdsReceiptEntryTO.setArbitrationId(CommonUtil.convertObjToStr(map.get("CURRENT_ARBITRATION_TRANS_ID")));
                    gdsReceiptEntryTO.setForfeitBonusTransId("");
                }
                gdsReceiptEntryTO.setStatusBy((String) map.get(CommonConstants.USER_ID));
                gdsReceiptEntryTO.setStatus(CommonConstants.STATUS_CREATED);
                gdsReceiptEntryTO.setStatusDt((Date)currDt.clone());
                gdsReceiptEntryTO.setSingleTransId(generateSingleTransId);
                double noticeAmount = CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt());//9535
                gdsReceiptEntryTO.setNetAmt(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNetAmt()) - noticeAmount);
               // System.out.println("###### gdsReceiptEntryTO ###### : " + gdsReceiptEntryTO);
                //Added by Nidhin mantis Id :9624
                if (isSplitMDSTransaction != null && isSplitMDSTransaction.equals("Y")) {
                    if (map.containsKey("gdsReceiptEntryTOList") && map.get("gdsReceiptEntryTOList") != null) {
                        List mdssReceiptList = (List) map.get("gdsReceiptEntryTOList");
                     //   System.out.println("Going to Inserting ...."+mdssReceiptList);
                        if (mdssReceiptList != null && mdssReceiptList.size() > 0) {
                            for (int i = 0; i < mdssReceiptList.size(); i++) {
                                GDSReceiptEntryTO MDSTo = (GDSReceiptEntryTO) mdssReceiptList.get(i);
                                ...
                                sqlMap.executeUpdate("insertGDSReceiptEntryTO", MDSTo);
                            }
                        }
                    }
                } else {
                    ...
                    if (allApplnDetails != null && allApplnDetails.size() > 0) {
                        gdsReceiptEntryTO.setBonusAmtAvail(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getBonusAmtAvail()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setBonusAmtPayable(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getBonusAmtPayable()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setCurrInst(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getCurrInst())));
                        gdsReceiptEntryTO.setDiscountAmt(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getDiscountAmt()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setInstAmt(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getInstAmt()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setInstAmtPayable(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getInstAmtPayable()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setMdsInterset(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getMdsInterset()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setNetAmt(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNetAmt()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setPaidAmt(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getPaidAmt()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setPenalAmtPayable(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getPenalAmtPayable()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setPendingInst(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getPendingInst()).doubleValue()));
                        gdsReceiptEntryTO.setServiceTaxAmt(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getServiceTaxAmt()).doubleValue()/schemeCount));
                        gdsReceiptEntryTO.setTotalInstDue(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getTotalInstDue()).doubleValue()));
                        gdsReceiptEntryTO.setNoticeAmt(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt()).doubleValue()/schemeCount)); // Added by nithya on 23-11-2019 for KD-692
                        gdsReceiptEntryTO.setArbitrationAmt(CommonUtil.convertObjToDouble(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getArbitrationAmt()).doubleValue()/schemeCount));
                        if(isBonusTrans.equalsIgnoreCase("N")){
                            gdsReceiptEntryTO.setBonusAmtPayable(0.0);
                            gdsReceiptEntryTO.setDiscountAmt(0.0);
                        }   
                        for (int i = 0; i < allApplnDetails.size(); i++) {
                                GDSApplicationTO objGDSApplicationTO = (GDSApplicationTO)allApplnDetails.get(i);
                                gdsReceiptEntryTO.setSchemeName(CommonUtil.convertObjToStr(objGDSApplicationTO.getSchemeName()));
                                gdsReceiptEntryTO.setChittalNo(CommonUtil.convertObjToStr(objGDSApplicationTO.getChittalNo()));
                                System.out.println("gdsReceiptEntryTO  checking ::" + gdsReceiptEntryTO);
                                sqlMap.executeUpdate("insertGDSReceiptEntryTO", gdsReceiptEntryTO);
                            }
                    }
                            
                                 
                }
                updateChitCloseDate(gdsReceiptEntryTO,null);
                //System.out.println("$#@$#$@# gdsReceiptEntryTO :" + gdsReceiptEntryTO);
                penalTransHappened = false;

                //From Recover Tally List
                if (map.containsKey("FROM_RECOVERY_TALLY") || map.containsKey("FROM_MDS_MEMBER_RECEIPT")) {
                    returnMap.put("BATCH_ID", CommonUtil.convertObjToStr(gdsReceiptEntryTO.getNetTransId()));
                }
              ...
              //  System.out.println("mdsCurrentReceiptEntryTO " + mdsCurrentReceiptEntryTO + "   " + mdsBankAdvReceiptEntryTO);
                if (mdsCurrentReceiptEntryTO != null) {
                    System.out.println("executing inside mdsCurrentReceiptEntryTO");
                    setTransDetails(mdsCurrentReceiptEntryTO);
                }
                if (mdsBankAdvReceiptEntryTO != null) {
                    System.out.println("executing inside mdsBankAdvReceiptEntryTO");
...
//                setTransDetails(gdsReceiptEntryTO);
            ...
            if (map.containsKey("serviceTaxDetailsTO")) {
                ...
                objserTax.setAcct_Num(gdsReceiptEntryTO.getNetTransId());
    ...
    private GDSReceiptEntryTO getgdsReceiptEntryTO(double bonusAmt, double instAmt) {
        double netAmt = 0.0;
        netAmt = instAmt - bonusAmt;
        GDSReceiptEntryTO mdsBankAdvReceiptEntryTO = new GDSReceiptEntryTO();
        mdsBankAdvReceiptEntryTO.setSchemeName(gdsReceiptEntryTO.getSchemeName());
        mdsBankAdvReceiptEntryTO.setChittalNo(gdsReceiptEntryTO.getChittalNo());
        mdsBankAdvReceiptEntryTO.setSubNo(gdsReceiptEntryTO.getSubNo());
        mdsBankAdvReceiptEntryTO.setDivisionNo(gdsReceiptEntryTO.getDivisionNo());
        ...
        mdsBankAdvReceiptEntryTO.setNoticeAmt(gdsReceiptEntryTO.getNoticeAmt());
        mdsBankAdvReceiptEntryTO.setArbitrationAmt(gdsReceiptEntryTO.getArbitrationAmt());
        mdsBankAdvReceiptEntryTO.setPenalAmtPayable(gdsReceiptEntryTO.getPenalAmtPayable());
        mdsBankAdvReceiptEntryTO.setDiscountAmt(gdsReceiptEntryTO.getDiscountAmt());
        mdsBankAdvReceiptEntryTO.setBranchCode(gdsReceiptEntryTO.getBranchCode());
        mdsBankAdvReceiptEntryTO.setForfeitBonusAmtPayable(gdsReceiptEntryTO.getForfeitBonusAmtPayable());
    ...
    private void insertReceiptData(HashMap map, GDSReceiptEntryTO mdsCurrentReceiptEntryTO, LogDAO objLogDAO, LogTO objLogTO, String cash_unique_id) throws Exception {
        try {
            System.out.println("Inside GDSReceiptEntryDAO :: insertReceiptData :: mdsCurrentReceiptEntryTO :: "+mdsCurrentReceiptEntryTO);
            ...
            System.out.println("Inside GDSReceiptEntryDAO :: insertReceiptData ::map ::" + map);
            ...
            if(map.containsKey("MP_MDS_CODE") && map.get("MP_MDS_CODE")!= null){
                mdsCode = CommonUtil.convertObjToStr(map.get("MP_MDS_CODE"));
            }
            ...
            if (map.containsKey("TransactionTO")) {
                ...
                if (transactionTO.getTransType().equals("TRANSFER") || transactionTO.getTransType().equals("CASH")) {
                    ...
                    transactionDAO.setLinkBatchID(CommonUtil.convertObjToStr(mdsCurrentReceiptEntryTO.getChittalNo()));
                     int schemeCount = 0;
                     HashMap applicationMap = new HashMap();
                     applicationMap.put("GDS_NO",gdsReceiptEntryTO.getGds_no() );
                     ...
                     // Code for transfer transactions
                    if (transactionTO.getTransType().equals("TRANSFER")) {
                        ...
                        String gdsGroupName = "";                        
                         if(map.containsKey("GROUP_NO") && map.get("GROUP_NO") != null){
                             ...
                             groupNameMap.put("GROUP_NO",CommonUtil.convertObjToStr(map.get("GROUP_NO")));
                              ...
                              System.out.println("gdsGroupName herev ::" +  gdsGroupName);
                         }
                        //End
                        // Itereating for each chittal in a group for each scheme                        
                        // --- Debit from OA/SA/GL starts----
                         if (!map.containsKey("THALAYAL_CHITTAL") && !map.containsKey("MUNNAL_CHITTAL")&& CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNetAmt()).doubleValue() > 0) {
                        //if (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNetAmt()).doubleValue() > 0) {
                            ...
                            if (map.containsKey("FROM_PARTICULARS_TRANSALL")) {                               
                                ...
                                txMap.put(TransferTrans.PARTICULARS, "Installment Amt :" + mdsCode + "-" + mdsCurrentReceiptEntryTO.getGds_no() + "_" + mdsCurrentReceiptEntryTO.getSubNo()+"-"+gdsGroupName);
                            }
                            if (!transactionTO.getProductType().equals("") && transactionTO.getProductType().equals("OA")) {
                            ...
                            transferTo.setGlTransActNum(mdsCurrentReceiptEntryTO.getGds_no());
                            transferTo.setSingleTransId(generateSingleTransId);
                            transferTo.setScreenName((String) map.get(CommonConstants.SCREEN));
                            TxTransferTO.add(transferTo);
                        }
                        //-----Debit from OA/SA/GL End
                        for (int i = 0; i < allApplnDetails.size(); i++) {
                            // If penal trans not happening starts                            
                            GDSApplicationTO objGDSApplicationTO = (GDSApplicationTO) allApplnDetails.get(i);
                            ...
                            // For thalayal receipt transactions - Multiple debit from thalayal receipt head of each scheme
                            ...
                            //System.out.println("thalayalTransCategory :: " + thalayalTransCategory);
                            if (map.containsKey("THALAYAL_CHITTAL") && !map.get("THALAYAL_CHITTAL").equals("") && map.get("THALAYAL_CHITTAL").equals("Y")) {
                                transferTo.setInstrumentNo2("APPL_GL_TRANS");
                                if (!transactionTO.getProductType().equals("") && transactionTO.getProductType().equals("GL") && transactionTO.getProductId().equals("")) {
                                ...
                                txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                                //For thalayal customization - parameterization starts
                                if (thalayalTransCategory.equalsIgnoreCase("FULLAMT_TO_FULLAMT")) {
                                    transferTo = transactionDAO.addTransferDebitLocal(txMap, (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNetAmt()).doubleValue() + CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getBonusAmtPayable()).doubleValue())/schemeCount);
                                    ...
                                    transferTo = transactionDAO.addTransferDebitLocal(txMap, CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNetAmt()).doubleValue()/schemeCount);
                                } else {
                                    transferTo = transactionDAO.addTransferDebitLocal(txMap, (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNetAmt()).doubleValue() + CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getBonusAmtPayable()).doubleValue())/schemeCount);
                                }
                                // End 
                                ...
                                transferTo.setInstrumentNo2("APPL_GL_TRANS");
                                if (!transactionTO.getProductType().equals("") && transactionTO.getProductType().equals("GL") && transactionTO.getProductId().equals("")) {
                                    txMap.put(TransferTrans.DR_AC_HD, (String) applicationMap.get("MUNNAL_REP_PAY_HEAD"));
                                    txMap.put(TransferTrans.DR_PROD_TYPE, TransactionFactory.GL);
                                }
                                txMap.put("TRANS_MOD_TYPE", "GL");
                                if (map.containsKey("FROM_PARTICULARS_TRANSALL")) {
                                    ...
                                    txMap.put(TransferTrans.PARTICULARS, "Installment Amt :" + mdsCode + "-" + mdsCurrentReceiptEntryTO.getGds_no() + "_" + mdsCurrentReceiptEntryTO.getSubNo());
                                }
                                txMap.put(TransferTrans.DR_BRANCH, BRANCH_ID);
                                txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                            ...
                            if (!penalTransHappened) {
                                if (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getPenalAmtPayable()).doubleValue() > 0) {
                                    System.out.println("penal transaction Started");
                                ...
                                if (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getNoticeAmt()).doubleValue() > 0) {
                                    ...
                                    whereMap.put("ACT_NUM",objGDSApplicationTO.getChittalNo()+"_"+objGDSApplicationTO.getSubNo());
                                    ...
                                    if (chargeList != null && chargeList.size() > 0) {
                                        for (int c = 0; c < chargeList.size(); c++) {
                                            double chargeAmount = 0.0;
                                            String chargeType = "";
                                            whereMap = (HashMap) chargeList.get(c);
                                ...
                                if (serviceTaxMap != null && serviceTaxMap.size() > 0) {
                                    ...
                                    txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                                    txMap.put("TRANS_MOD_TYPE", "MDS");
                                    transferTo = transactionDAO.addTransferCreditLocal(txMap, CommonUtil.convertObjToDouble(serviceTaxMap.get("TOT_TAX_AMT"))/schemeCount);
                                    transferTo.setTransId("-");
                                    transferTo.setBatchId("-");
                                    transferTo.setTransDt(currDt);
                                    transferTo.setInitiatedBranch(BRANCH_ID);
                            ...
                            // If penal trans not happens ends
                            ...
                            if (map.containsKey("THALAYAL_CHITTAL") && !map.get("THALAYAL_CHITTAL").equals("") && map.get("THALAYAL_CHITTAL").equals("Y")) {
                               // finalReceivingAmt = finalReceivingAmt + CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getBonusAmtPayable()).doubleValue();
                                //Credit side thalayal customization
                                if (thalayalTransCategory.equalsIgnoreCase("FULLAMT_TO_FULLAMT")) {
                                finalReceivingAmt = finalReceivingAmt + CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getBonusAmtPayable()).doubleValue();
                                } else if (thalayalTransCategory.equalsIgnoreCase("FULLAMT_TO_SUBAMT")) {
                            ...
                            if (finalReceivingAmt > 0) {
                                ...
                                if (map.containsKey("FROM_PARTICULARS_TRANSALL")) {                                 
                                    ...
                                    txMap.put(TransferTrans.PARTICULARS, "Installment Amt :" + applicationMap.get("MP_MDS_CODE") + "-" + objGDSApplicationTO.getChittalNo() + "_" + mdsCurrentReceiptEntryTO.getSubNo() + " - From : " + transactionTO.getDebitAcctNo());
                                }
                                txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                                txMap.put("TRANS_MOD_TYPE", "MDS");
                                if (postageAmt > 0.0 || renewPostageAmt > 0.0) {
                                    if (postageAmt > 0.0) {
                                        transferTo = transactionDAO.addTransferCreditLocal(txMap, (finalReceivingAmt - postageAmt) / schemeCount);// shany
                                    }
                                    if (renewPostageAmt > 0.0) {
                                        transferTo = transactionDAO.addTransferCreditLocal(txMap, (finalReceivingAmt - renewPostageAmt) / schemeCount);// shany
                                    }
                                } else {
                                    transferTo = transactionDAO.addTransferCreditLocal(txMap, finalReceivingAmt / schemeCount);// shany
                                }
                                transferTo.setScreenName((String) map.get(CommonConstants.SCREEN));
                                ...
                                if (map.containsKey("POSTAGE_AMT_FOR_INT") || map.containsKey("RENEW_POSTAGE_AMT")) {
                                    if (postageAmt > 0.0) {
                                        ...
                                        txMap.put("TRANS_MOD_TYPE", "MDS");
                                        transferTo = transactionDAO.addTransferCreditLocal(txMap, postageAmt / schemeCount);// shany
                                        transferTo.setInitiatedBranch(BRANCH_ID);
                                        transferTo.setTransAllId(CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                                        transferTo.setRec_mode(CommonUtil.convertObjToStr(map.get("REC_MODE")));
                                        transferTo.setGlTransActNum(objGDSApplicationTO.getChittalNo());
                                    ...
                                    if (renewPostageAmt > 0.0) {
                                        ...
                                        txMap.put(CommonConstants.USER_ID, CommonUtil.convertObjToStr(map.get("USER_ID")));
                                        txMap.put("TRANS_MOD_TYPE", "MDS");
                                        transferTo = transactionDAO.addTransferCreditLocal(txMap, renewPostageAmt / schemeCount);// shany
                                        transferTo.setInitiatedBranch(BRANCH_ID);
                                        transferTo.setTransAllId(CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                                        transferTo.setRec_mode(CommonUtil.convertObjToStr(map.get("REC_MODE")));
                                        transferTo.setGlTransActNum(objGDSApplicationTO.getChittalNo());
                            ...
                            System.out.println("thalayalTransCategory here :: " + thalayalTransCategory);
                            if (map.containsKey("THALAYAL_CHITTAL") && !map.get("THALAYAL_CHITTAL").equals("") && map.get("THALAYAL_CHITTAL").equals("Y")) {
                                System.out.println("Here for thalayal");
                                if (thalayalTransCategory.equalsIgnoreCase("FULLAMT_TO_SUBAMT")) {
                                    System.out.println("Execute here inside :: if FULLAMT_TO_SUBAMT");
                                    thalayalMunnalBonusAmt = mdsCurrentReceiptEntryTO.getBonusAmtPayable().doubleValue();
                                    System.out.println("thalayalMunnalBonusAmt :: " + thalayalMunnalBonusAmt);
                            ...
                            if (thalayalMunnalBonusAmt > 0.0) {
                                ...
                                if (map.containsKey("FROM_PARTICULARS_TRANSALL")) {
                                    ...
                                    txMap.put(TransferTrans.PARTICULARS, "Installment Amt :" + applicationMap.get("MP_MDS_CODE") + "-" + objGDSApplicationTO.getChittalNo() + "_" + objGDSApplicationTO.getSubNo());
                                }
                                txMap.put(CommonConstants.USER_ID, CommonUtil.convertObjToStr(map.get("USER_ID")));
                                txMap.put("TRANS_MOD_TYPE", "MDS");
                                transferTo = transactionDAO.addTransferCreditLocal(txMap, thalayalMunnalBonusAmt/schemeCount);
                                transferTo.setInitiatedBranch(BRANCH_ID);
                                transferTo.setTransAllId(CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                                transferTo.setRec_mode(CommonUtil.convertObjToStr(map.get("REC_MODE")));
                                transferTo.setGlTransActNum(objGDSApplicationTO.getChittalNo());
                        ...
                        String command = CommonUtil.convertObjToStr(gdsReceiptEntryTO.getCommand());
                            ...
                            if (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getArbitrationAmt()).doubleValue() > 0) {
                                ...
                                if (allApplnDetails != null && allApplnDetails.size() > 0) {
                                    ...
                                    for (int k = 0; k < allApplnDetails.size(); k++) {
                                        ...
                                        if (chargeList != null && chargeList.size() > 0) {
                                            //applncashList = new ArrayList(); // Commented by nithya on 23-11-2019 for KD-692
                                            for (int i = 0; i < chargeList.size(); i++) {
                                                double chargeAmount = 0.0;
                                                String chargeType = "";
                                                whereMap = (HashMap) chargeList.get(i);
                                            ...
                                            if (serviceTaxMap != null && serviceTaxMap.size() > 0) {
                                                ...
                                                cashTransMap.put("AMOUNT", CommonUtil.convertObjToStr(serviceTaxMap.get("TOT_TAX_AMT")));
                                                cashTransMap.put("MP_MDS_CODE", applicationMap.get("MP_MDS_CODE"));
                                                cashTransMap.put("TRANS_ALL_ID", CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                                                cashTransMap.put("AUTHORIZE_REMARKS", cash_unique_id);
                                                cashTransMap.put("TRANS_MOD_TYPE", "MDS");
...
//                                            cashTransMap.put("DAILYDEPOSITTRANSTO", applncashList);// Commented by nithya on 23-11-2019 for KD-692
//                                            cashTransMap.put("SCREEN_NAME", (String) map.get(CommonConstants.SCREEN));
//                                            cashTransMap.put("TRANS_ALL_ID", CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
//                                            cashTransMap.put("SINGLE_TRANS_ID", generateSingleTransId);
//                                            cashMap = cashTransactionDAO.execute(cashTransMap, false);
...
//                                            sqlMap.executeUpdate("updateLinkBatchIdCash", linkBatchMap);
                                            ...
                                            cashTransMap.put("DAILYDEPOSITTRANSTO", applncashList); // Added by nithya on 23-11-2019 for KD-692
                                            cashTransMap.put("SCREEN_NAME", (String) map.get(CommonConstants.SCREEN));
                                            cashTransMap.put("TRANS_ALL_ID", CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                                            cashTransMap.put("SINGLE_TRANS_ID", generateSingleTransId);
                                            cashMap = cashTransactionDAO.execute(cashTransMap, false);
    ...
    private void insertBankAdvData(HashMap map, GDSReceiptEntryTO mdsBankAdvReceiptEntryTO, LogDAO objLogDAO, LogTO objLogTO, String cash_unique_id) throws Exception {
        try {
            System.out.println("###### MDS BANK ADVANCE ###### : "+map);
            System.out.println("inside insertBankAdvData :: mdsBankAdvReceiptEntryTO :: " + mdsBankAdvReceiptEntryTO);
            String isBonusTransfer = "Y";
            String mdsCode = "";
                ...
                if (transactionTO.getTransType().equals("TRANSFER") || transactionTO.getTransType().equals("CASH")) {
                    ...
                    if (transactionTO.getTransType().equals("TRANSFER")) {
                        ...
                        if (CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getNetAmt()).doubleValue() > 0) {                        
                            ...
                            txMap.put(TransferTrans.PARTICULARS, "Installment Amt :" + mdsCode + "-" + mdsBankAdvReceiptEntryTO.getGds_no() + "_" + mdsBankAdvReceiptEntryTO.getSubNo()+"-"+gdsGroupName);
                            txMap.put(TransferTrans.DR_BRANCH, BRANCH_ID);
                            txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                            txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                            ...
                            transferTo.setTransAllId(CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                            transferTo.setSingleTransId(generateSingleTransId);
                            transferTo.setGlTransActNum(mdsBankAdvReceiptEntryTO.getGds_no());
                            transferTo.setScreenName((String) map.get(CommonConstants.SCREEN));
                        ...
                        for (int i = 0; i < allApplnDetails.size(); i++) {
                            ...
                            if (CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getNetAmt()).doubleValue() > 0) {
                                ...
                                txMap.put(TransferTrans.PARTICULARS, "Installment Amt :" + mdsCode + "-" + objGDSApplicationTO.getChittalNo() + "_" + mdsBankAdvReceiptEntryTO.getSubNo() + ":BANK ADVANCE");
                                txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                                ...
                                if (map.containsKey("POSTAGE_AMT_FOR_INT") || map.containsKey("RENEW_POSTAGE_AMT")) {
                                    if (postageAmt > 0.0) {
                                        ...
                                        txMap.put(TransferTrans.CR_BRANCH, _branchCode);
                                        txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                                        txMap.put(TransferTrans.PARTICULARS, "Installment Amt :" + applicationMap.get("MP_MDS_CODE") + "-" + objGDSApplicationTO.getChittalNo() + "_" + mdsBankAdvReceiptEntryTO.getSubNo() + ":BANK ADVANCE");
                                        txMap.put(CommonConstants.USER_ID, CommonUtil.convertObjToStr(map.get("USER_ID")));
                                        txMap.put("TRANS_MOD_TYPE_MMBS", "MDS");
                                        txMap.put("TRANS_MOD_TYPE", "MDS");
                            ...
                            if (!penalTransHappened) {
                                if (CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getPenalAmtPayable()).doubleValue() > 0) {
                                    ...
                                    txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                                    txMap.put(TransferTrans.CR_BRANCH, mdsBankAdvReceiptEntryTO.getBranchCode());//BRANCH_ID);
                                    txMap.put(TransferTrans.PARTICULARS, "Penal :" + mdsCode + "-" + objGDSApplicationTO.getChittalNo() + "_" + mdsBankAdvReceiptEntryTO.getSubNo());
                                    txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                                    txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                                    txMap.put("TRANS_MOD_TYPE_MMBS", "MDS");
                                    txMap.put("TRANS_MOD_TYPE", "MDS");
                                    transferTo = transactionDAO.addTransferCreditLocal(txMap, CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getPenalAmtPayable()).doubleValue()/schemeCount);
                                    transferTo.setTransId("-");
                                    transferTo.setBatchId("-");
                                    transferTo.setTransDt(currDt);
                                    transferTo.setInitiatedBranch(BRANCH_ID);
                                ...
                                if (CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getNoticeAmt()).doubleValue() > 0) {
                                    ...
                                    whereMap.put("ACT_NUM",objGDSApplicationTO.getChittalNo()+"_"+objGDSApplicationTO.getSubNo());
                                    if (map.containsKey("INT_CALC_UPTO_DT")) {       
                                        whereMap.put("INT_CALC_UPTO_DT", map.get("INT_CALC_UPTO_DT"));
                                        chargeList = (List) sqlMap.executeQueryForList("getMDSRecoveryNoticeChargeDetails", whereMap);
                                    } else {
                                        chargeList = (List) sqlMap.executeQueryForList("getMDSNoticeChargeDetails", whereMap);
                                    ...
                                    if (chargeList != null && chargeList.size() > 0) {
                                        for (int t = 0; t < chargeList.size(); t++) {
                                            ...
                                            if (chargeAmount > 0) {
                                                ...
                                                txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                                                txMap.put(TransferTrans.CR_BRANCH, mdsBankAdvReceiptEntryTO.getBranchCode());//BRANCH_ID);
                                                txMap.put(TransferTrans.PARTICULARS, chargeType + mdsCode + "-" + objGDSApplicationTO.getChittalNo() + "_" + mdsBankAdvReceiptEntryTO.getSubNo());
                                                txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                                                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                                                txMap.put("TRANS_MOD_TYPE_MMBS", "MDS");
                                                txMap.put("TRANS_MOD_TYPE", "MDS");
...
//                                            txMap.put(TransferTrans.CR_AC_HD, applicationMap.get("CASE_EXPENSE_HEAD"));
                            ...
                            if (bonusreversal) {
                                ...
                                txMap.put(TransferTrans.CR_PROD_TYPE, TransactionFactory.GL);
                                txMap.put(TransferTrans.CR_BRANCH, mdsBankAdvReceiptEntryTO.getBranchCode());//BRANCH_ID);
                                txMap.put(TransferTrans.PARTICULARS, "Bonus Reversal :" + applicationMap.get("MP_MDS_CODE") + "-" + objGDSApplicationTO.getChittalNo() + "_" + mdsBankAdvReceiptEntryTO.getSubNo() + ":BANK ADVANCE");
                                txMap.put(TransferTrans.AUTHORIZE_STATUS_2, "ENTERED_AMOUNT");
                                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                                txMap.put("TRANS_MOD_TYPE_MMBS", "MDS");
                                txMap.put("TRANS_MOD_TYPE", "MDS");
...
//                            System.out.println("bankBonusReverseAmt" + bankBonusReverseAmt);
                                ...
                                //transferTo = transactionDAO.addTransferCreditLocal(txMap, (CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getBonusAmtPayable()).doubleValue())/schemeCount);
                                 transferTo = transactionDAO.addTransferCreditLocal(txMap, (CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getForfeitBonusAmtPayable()).doubleValue())/schemeCount);
                                transferTo.setTransId("-");
                                transferTo.setBatchId("-");
                                transferTo.setTransDt(currDt);
                                ...
                                transferTo.setRec_mode(CommonUtil.convertObjToStr(map.get("REC_MODE")));
                                transferTo.setGlTransActNum(objGDSApplicationTO.getChittalNo());
                                transferTo.setSingleTransId(generateSingleTransId);//nithya added for bonus reverasal -KD-3787
                                transactionDAO.setLinkBatchID(CommonUtil.convertObjToStr(mdsBankAdvReceiptEntryTO.getChittalNo()));
                                transferTo.setScreenName((String) map.get(CommonConstants.SCREEN));
                                TxTransferTO.add(transferTo);
                            }
                        ...
                        System.out.println("mdsBankAdvReceiptEntryTO inside cash :: "+ mdsBankAdvReceiptEntryTO);
...
//                                            //applncashList.add(commonApplicationListForCashTrans(map,allApplnDetails,mdsBankAdvReceiptEntryTO,cash_unique_id,chargeAmount,cashTransMap));
//                                            applncashList = commonApplicationListForCashTrans(map,allApplnDetails,mdsBankAdvReceiptEntryTO,cash_unique_id,chargeAmount,cashTransMap);
//                                        }
//                                    }
//                                    cashTransMap.put("DAILYDEPOSITTRANSTO", applncashList);
//                                    cashTransMap.put("SCREEN_NAME", (String) map.get(CommonConstants.SCREEN));
...
//                                            sqlMap.executeUpdate("updateLinkBatchIdCash", linkBatchMap);
                        ...
                        if (bonusreversal) {
                                System.out.println("bonus reversal in cash trans");
                                //Added By Suresh
                                if (map.containsKey("MDS_CLOSURE")) {
                                  ...
                                  cashTransMap.put("ACC_HEAD", applicationMap.get("SUNDRY_RECEIPT_HEAD"));
                                   ...
                                   cashTransMap.put("ACC_HEAD", applicationMap.get("MDS_FORFEITED_ACCT_HEAD"));
                                }
                                ...
                                cashTransMap.put("AMOUNT", CommonUtil.convertObjToDouble(mdsBankAdvReceiptEntryTO.getForfeitBonusAmtPayable()).doubleValue()/schemeCount);
                                cashTransMap.put("TRANS_MOD_TYPE", "MDS");
                                cashTransMap.put("SCREEN_NAME", (String) map.get(CommonConstants.SCREEN));                              
                                cashTransMap.put("GDS_TRANS_FOR","BONUS_REVERSAL");
                                cashTransMap.put("INSTRUMENT_NO2","BONUS");// Added by nithya on 31-10-2019 for KD-680
                                applncashList = commonApplicationListForCashTrans(map,allApplnDetails,mdsBankAdvReceiptEntryTO,cash_unique_id,mdsBankAdvReceiptEntryTO.getForfeitBonusAmtPayable(),cashTransMap);//nithya---- 
                                cashTransMap.put("DAILYDEPOSITTRANSTO", applncashList);
                                cashTransMap.put("SCREEN_NAME", (String) map.get(CommonConstants.SCREEN));
                                cashTransMap.put("AUTHORIZE_REMARKS", cash_unique_id);
                                cashTransMap.put("TRANS_ALL_ID", CommonUtil.convertObjToStr(map.get("TRANS_ALL_ID")));
                    ...
                    map.put("BANK_ADVANCE_NOTICE_TRANS_ID", mdsBankAdvReceiptEntryTO.getNoticeId());
                    map.put("BANK_ADVANCE_ARBITRATION_TRANS_ID", mdsBankAdvReceiptEntryTO.getArbitrationId());
                    map.put("BANK_ADVANCE_FORFEITBONUS_TRANS_ID", mdsBankAdvReceiptEntryTO.getForfeitBonusTransId());
    ...
    private void setTransDetails(GDSReceiptEntryTO gdsReceiptEntryTO) throws Exception {

        System.out.println("inside gdsReceiptEntryTO :: "+ gdsReceiptEntryTO);
        transIdMap = new HashMap();
        if (gdsReceiptEntryTO.getArbitrationAmt().doubleValue() > 0) {
            getTransDetails(gdsReceiptEntryTO.getArbitrationId(), "ARBITRATION");

            transIdMap.put(gdsReceiptEntryTO.getArbitrationId(), null);
            System.out.println("transIdMap 1" + transIdMap);
        }
        if (gdsReceiptEntryTO.getNoticeAmt().doubleValue() > 0) {
            getTransDetails(gdsReceiptEntryTO.getNoticeId(), "NOTICE");
            transIdMap.put(gdsReceiptEntryTO.getNoticeId(), null);
            System.out.println("transIdMap 2" + transIdMap);
        }
        if (gdsReceiptEntryTO.getPenalAmtPayable().doubleValue() > 0) {
            getTransDetails(gdsReceiptEntryTO.getPenalTransId(), "PENAL");
            transIdMap.put(gdsReceiptEntryTO.getPenalTransId(), null);
            System.out.println("transIdMap 3" + transIdMap);
        }

        if (gdsReceiptEntryTO.getBonusAmtPayable().doubleValue() > 0) {
            getTransDetails(gdsReceiptEntryTO.getBonusTransId(), "BONUS");
            transIdMap.put(gdsReceiptEntryTO.getBonusTransId(), null);
            System.out.println("transIdMap 4" + transIdMap);
        } else {
            getTransDetails(gdsReceiptEntryTO.getBonusTransId(), "BONUS");
            transIdMap.put(gdsReceiptEntryTO.getBonusTransId(), null);
        }
        if (gdsReceiptEntryTO.getDiscountAmt().doubleValue() > 0) {
            getTransDetails(gdsReceiptEntryTO.getDiscountTransId(), "DISCOUNT");
            transIdMap.put(gdsReceiptEntryTO.getDiscountTransId(), null);
            System.out.println("transIdMap 5" + transIdMap);
        }
        if (gdsReceiptEntryTO.getNetAmt().doubleValue() > 0) {
            getTransDetails(gdsReceiptEntryTO.getNetTransId(), "NET");
            transIdMap.put(gdsReceiptEntryTO.getNetTransId(), null);
            System.out.println("transIdMap 6" + transIdMap);
        }
        
        if(gdsReceiptEntryTO.getForfeitBonusAmtPayable().doubleValue() > 0){
            getTransDetails(gdsReceiptEntryTO.getForfeitBonusTransId(), "FORFEIT_BONUS");
            transIdMap.put(gdsReceiptEntryTO.getForfeitBonusTransId(), null);
            System.out.println("transIdMap 7" + transIdMap);
        }
    ...
    private void getTransDetails(String batchId, String mapKey) throws Exception {
        System.out.println("@@@ transIdMap:" + transIdMap + " / @@@ batchId:" + batchId + " sinle " + gdsReceiptEntryTO.getSingleTransId());
    ...
    //        System.out.println("objCashTransactionTO :"+gdsReceiptEntryTO);
    //        double finalReceivingAmt = CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNetAmt()).doubleValue() -
    //        (CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getPenalAmtPayable()).doubleValue() +
    //        CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt()).doubleValue() +
    //        CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getArbitrationAmt()).doubleValue());
    //
    //        if(gdsReceiptEntryTO.getCommand().equals(CommonConstants.TOSTATUS_INSERT)){
    ...
    //            objCashTO.setStatusDt(gdsReceiptEntryTO.getStatusDt());
    //            objCashTO.setInstrumentNo2("ENTERED_AMOUNT");
    //            objCashTO.setParticulars(gdsReceiptEntryTO.getSchemeName()+"-" + gdsReceiptEntryTO.getChittalNo()+":Penal");
    ...
    //            objCashTO.setCommand(gdsReceiptEntryTO.getCommand());
    ...
    //            objCashTO.setInpAmount(new Double(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getPenalAmtPayable()).doubleValue()));
    //            objCashTO.setAmount(new Double(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getPenalAmtPayable()).doubleValue()));
    ...
    //            objCashTO.setStatusDt(gdsReceiptEntryTO.getStatusDt());
    //            objCashTO.setInstrumentNo2("ENTERED_AMOUNT");
    //            objCashTO.setParticulars(gdsReceiptEntryTO.getSchemeName()+"-" + gdsReceiptEntryTO.getChittalNo()+":Notice");
    //            objCashTO.setInitiatedBranch(CommonUtil.convertObjToStr(applnMap.get("BRANCH_CODE")));
    //            objCashTO.setInitChannType(CommonConstants.CASHIER);
    //            objCashTO.setCommand(gdsReceiptEntryTO.getCommand());
    //            System.out.println("objCashTO 1st one:" + objCashTO);
    //            objCashTO.setAcHdId(CommonUtil.convertObjToStr(applnMap.get("NOTICE_CHARGES_HEAD")));
    //            objCashTO.setInpAmount(new Double(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt()).doubleValue()));
    //            objCashTO.setAmount(new Double(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt()).doubleValue()));
    //            cashList.add(objCashTO);
    //            objCashTO = null;
    //        }
    //        return cashList;
    ...
    //            objCashTO.setStatusDt(gdsReceiptEntryTO.getStatusDt());
    //            objCashTO.setInstrumentNo2("ENTERED_AMOUNT");
    //            objCashTO.setParticulars(gdsReceiptEntryTO.getSchemeName()+"-" + gdsReceiptEntryTO.getChittalNo()+":Arbitration");
    //            objCashTO.setInitiatedBranch(CommonUtil.convertObjToStr(applnMap.get("BRANCH_CODE")));
    //            objCashTO.setInitChannType(CommonConstants.CASHIER);
    //            objCashTO.setCommand(gdsReceiptEntryTO.getCommand());
    //            System.out.println("objCashTO 1st one:" + objCashTO);
    //            objCashTO.setAcHdId(CommonUtil.convertObjToStr(applnMap.get("CASE_EXPENSE_HEAD")));
    //            objCashTO.setInpAmount(new Double(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getArbitrationAmt()).doubleValue()));
    //            objCashTO.setAmount(new Double(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getArbitrationAmt()).doubleValue()));
    //            cashList.add(objCashTO);
    //            objCashTO = null;
    //        }
    //        return cashList;
    ...
    //            objCashTO.setStatusDt(gdsReceiptEntryTO.getStatusDt());
    //            objCashTO.setInstrumentNo2("ENTERED_AMOUNT");
    //            objCashTO.setParticulars(gdsReceiptEntryTO.getSchemeName()+"-" + gdsReceiptEntryTO.getChittalNo());
    //            objCashTO.setInitiatedBranch(CommonUtil.convertObjToStr(applnMap.get("BRANCH_CODE")));
    //            objCashTO.setInitChannType(CommonConstants.CASHIER);
    //            objCashTO.setCommand(gdsReceiptEntryTO.getCommand());
    //            System.out.println("objCashTO 1st one:" + objCashTO);
    ...
    private ArrayList commonApplicationList(HashMap applnMap) throws Exception {
        ...
        if (gdsReceiptEntryTO.getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
            ...
            objCashTO.setParticulars(applnMap.get("PARTICULARS") + ":" + applnMap.get("MP_MDS_CODE") + "-" + gdsReceiptEntryTO.getChittalNo() + "_" + gdsReceiptEntryTO.getSubNo());
//            objCashTO.setInitiatedBranch(CommonUtil.convertObjToStr(applnMap.get("BRANCH_CODE")));
            objCashTO.setInitChannType(CommonConstants.CASHIER);
            objCashTO.setCommand(gdsReceiptEntryTO.getCommand());
        ...
        private ArrayList commonApplicationListForCashTrans(HashMap map, List allApplnDetails,GDSReceiptEntryTO mdsCurrentReceiptEntryTO, String cash_unique_id,double transactionAmount,HashMap cashTransMap) throws Exception {
                ...
                for (int i = 0; i < schemeCount; i++) {
                    ...
                    if(CommonUtil.convertObjToStr(cashTransMap.get("GDS_TRANS_FOR")).equalsIgnoreCase("BONUS_REVERSAL")){
                       if(map.containsKey("MDS_CLOSURE")){
                           ...
                           cashTransMap.put("ACC_HEAD",applnMap.get("MDS_FORFEITED_ACCT_HEAD"));
                       }
                    }
                    ...
                    if (gdsReceiptEntryTO.getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
                        ...
                        objCashTO.setStatusDt(gdsReceiptEntryTO.getStatusDt());
                        objCashTO.setAuthorizeStatus_2("ENTERED_AMOUNT");
                        objCashTO.setParticulars(cashTransMap.get("PARTICULARS") + ":" + applnMap.get("MP_MDS_CODE") + "-" + objGDSApplicationTO.getChittalNo() + "_" + gdsReceiptEntryTO.getSubNo());
                        objCashTO.setInitChannType(CommonConstants.CASHIER);
                        objCashTO.setCommand(gdsReceiptEntryTO.getCommand());
                        objCashTO.setTransAllId(CommonUtil.convertObjToStr(applnMap.get("TRANS_ALL_ID")));
                        ...
                        if (cashTransMap.containsKey("INSTRUMENT_NO2") && cashTransMap.get("INSTRUMENT_NO2") != null) {// Added by nithya on 31-10-2019 for KD-680
                            objCashTO.setInstrumentNo2(CommonUtil.convertObjToStr(cashTransMap.get("INSTRUMENT_NO2")));
                        }
                        System.out.println("objCashTO 1st one:" + objCashTO);
                        cashList.add(objCashTO);
                        objCashTO = null;
    ...
    private CashTransactionTO createCashTransactionTO(HashMap applnMap, String generateSingleTransId) throws Exception { //Case Expense Transaction
        ...
        if (gdsReceiptEntryTO.getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
            ...
            if (applnMap.get("PARTICULARS") != null && (applnMap.get("PARTICULARS").toString().startsWith("Penal Chittal No")
                ...
                objCashTO.setParticulars(applnMap.get("PARTICULARS") + ":" + applnMap.get("MP_MDS_CODE") + "-" + applnMap.get("GL_TRANS_ACT_NUM") + "_" + gdsReceiptEntryTO.getSubNo());
            }
            objCashTO.setInitiatedBranch(CommonUtil.convertObjToStr(applnMap.get("BRANCH_CODE")));
            objCashTO.setInitChannType(CommonConstants.CASHIER);
            objCashTO.setCommand(gdsReceiptEntryTO.getCommand());
    ...
    public void commonTransactionCashandTransfer(HashMap map, GDSReceiptEntryTO mdsCurrentReceiptEntryTO, HashMap debitMap, HashMap applicationMap, int schemeCount,List allApplnDetails) throws Exception {
        String BRANCH_ID = CommonUtil.convertObjToStr(map.get("BRANCH_CODE"));
        ...
        for (int i = 0; i < allApplnDetails.size(); i++) {
            ...
            if (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getBonusAmtPayable()).doubleValue() > 0) {
                ...
                transferTo = transactionDAO.addTransferDebitLocal(txMap, CommonUtil.convertObjToDouble(totalAmount / schemeCount));// shany
                transferTo.setTransId("-");
                transferTo.setBatchId("-");
            ...
            if (forfeiteBonusAmount > 0 && mdsCurrentReceiptEntryTO.getBonusAmtPayable().doubleValue() <= 0) {
                ...
                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                txMap.put("TRANS_MOD_TYPE", "MDS");
                transferTo = transactionDAO.addTransferDebitLocal(txMap, CommonUtil.convertObjToDouble(forfeiteBonusAmount) / schemeCount);// shany
                transferTo.setTransId("-");
                transferTo.setBatchId("-");
                transferTo.setTransType(CommonConstants.DEBIT);
                transferTo.setProdType(TransactionFactory.GL);
            ...
            if (CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getDiscountAmt()).doubleValue() > 0) {
                ...
                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                txMap.put("TRANS_MOD_TYPE", "MDS");
                transferTo = transactionDAO.addTransferDebitLocal(txMap, CommonUtil.convertObjToDouble(mdsCurrentReceiptEntryTO.getDiscountAmt() / schemeCount).doubleValue());// shany
                transferTo.setTransId("-");
                transferTo.setBatchId("-");
                transferTo.setTransType(CommonConstants.DEBIT);
                transferTo.setProdType(TransactionFactory.GL);
            ...
            if (receivingGLAmt > 0 || receivingGLAmt == 0) {
                ...
                txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                txMap.put("TRANS_MOD_TYPE", "MDS");
                transferTo = transactionDAO.addTransferCreditLocal(txMap, receivingGLAmt / schemeCount);// shany           
                transferTo.setTransId("-");
                transferTo.setBatchId("-");
                ...
                if (forfeiteBonusList != null && forfeiteBonusList.size() > 0) {
                    ...
                    if (applicationMap.get("FORFEITE_HD_Y_N") != null && applicationMap.get("FORFEITE_HD_Y_N").equals("Y")) {
                        ...
                        txMap.put(TransferTrans.DR_INST_TYPE, "VOUCHER");
                        txMap.put("TRANS_MOD_TYPE", "MDS");
                        transferTo = transactionDAO.addTransferCreditLocal(txMap, forfeiteBonusAmount / schemeCount);// shany                 
                        transferTo.setTransId("-");
                        transferTo.setBatchId("-");
                        transferTo.setTransType(CommonConstants.CREDIT);
                        transferTo.setProdType(TransactionFactory.GL);
    ...
    public void commonTransactionCashandTransferBankAdv(HashMap map, GDSReceiptEntryTO mdsBankAdvReceiptEntryTO, HashMap debitMap, HashMap applicationMap) throws Exception {
        String BRANCH_ID = CommonUtil.convertObjToStr(map.get("BRANCH_CODE"));
        ArrayList transferList = new ArrayList();
        TransferTrans transferTrans = new TransferTrans();
        TxTransferTO transferTo = new TxTransferTO();
...
//            System.out.println("transferDAO List transMap : " + transMap);
    ...
    public void commonTransactionTransferBankAdvBonus(HashMap map, GDSReceiptEntryTO mdsBankAdvReceiptEntryTO, HashMap debitMap, HashMap applicationMap) throws Exception {
        String BRANCH_ID = CommonUtil.convertObjToStr(map.get("BRANCH_CODE"));
        ArrayList transferList = new ArrayList();
        TransferTrans transferTrans = new TransferTrans();
        TxTransferTO transferTo = new TxTransferTO();
...
//            System.out.println("transferDAO List transMap : " + transMap);
    ...
    private void updateData(HashMap chargesMap, LogDAO objLogDAO, LogTO objLogTO) throws Exception {
        ...
        System.out.println("gdsReceiptEntryTO :" + gdsReceiptEntryTO);
        sqlMap.executeUpdate("updateReceiptEntryTO", gdsReceiptEntryTO);
    }
    ...
    private ArrayList applicationUpdate(HashMap chargesMap) throws Exception {//updating Transaction
        ...
        ArrayList cashList = new ArrayList();
        if (gdsReceiptEntryTO.getCommand().equals(CommonConstants.TOSTATUS_UPDATE)) {
            HashMap tempMap = (HashMap) chargesMap.get("SERVICE_TAX_AMT_TRANSACTION");
            String trans_id = CommonUtil.convertObjToStr(gdsReceiptEntryTO.getNetTransId());
            CashTransactionTO objCashTO = new CashTransactionTO();
            objCashTO = new CashTransactionTO();
            objCashTO.setInpAmount(gdsReceiptEntryTO.getNetAmt());
            objCashTO.setAmount(gdsReceiptEntryTO.getNetAmt());
            objCashTO.setCommand(gdsReceiptEntryTO.getCommand());
            cashList.add(objCashTO);
            objCashTO = null;
        }
    ...
    private void deleteData(HashMap chargesMap, LogDAO objLogDAO, LogTO objLogTO) throws Exception {
        ...
        if (chargesMap.containsKey("TransactionTO")) {
            ...
            gdsReceiptEntryTO.setStatus(CommonConstants.STATUS_DELETED);
            ...
            if (transactionTO.getTransType().equals("TRANSFER") && transactionMap != null && transactionMap.get("NET_TRANS_ID") != null && !transactionMap.get("NET_TRANS_ID").equals("")
                    && chargesMap.containsKey("NET_TRANSACTION_TRANSFER") && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNetAmt()).doubleValue() > 0) {
                ...
                gdsReceiptEntryTO.setNetTransId(CommonUtil.convertObjToStr(tempMap.get("BATCH_ID")));
                double newAmount = CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNetAmt()).doubleValue();
                ArrayList batchList = new ArrayList();
                TxTransferTO txTransferTO = null;
                HashMap oldAmountMap = new HashMap();
                String commandMode = CommonUtil.convertObjToStr(gdsReceiptEntryTO.getCommand());
                if (gdsReceiptEntryTO.getCommand().equals(CommonConstants.TOSTATUS_DELETE)) {
            ...
            if (transactionMap != null && transactionMap.get("BONUS_TRANS_ID") != null && !transactionMap.get("BONUS_TRANS_ID").equals("")
                    && chargesMap.containsKey("BONUS_TRANSACTION_TRANSFER") && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getBonusAmtPayable()).doubleValue() > 0) {
            ...
            if (transactionMap != null && transactionMap.get("DISCOUNT_TRANS_ID") != null && !transactionMap.get("DISCOUNT_TRANS_ID").equals("")
                    && chargesMap.containsKey("DISCOUNT_TRANSACTION_TRANSFER") && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getDiscountAmt()).doubleValue() > 0) {
            ...
            if (transactionTO.getTransType().equals("CASH") && transactionMap != null && transactionMap.get("NET_TRANS_ID") != null && !transactionMap.get("NET_TRANS_ID").equals("")
                    && chargesMap.containsKey("NET_TRANSACTION_CASH") && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNetAmt()).doubleValue() > 0) {
            ...
            if (transactionTO.getTransType().equals("CASH") && transactionMap != null && transactionMap.get("PENAL_TRANS_ID") != null && !transactionMap.get("PENAL_TRANS_ID").equals("")
                    && chargesMap.containsKey("PENAL_TRANSACTION_CASH") && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getPenalAmtPayable()).doubleValue() > 0) {
            ...
            if (transactionTO.getTransType().equals("CASH") && transactionMap != null && transactionMap.get("ARBITRATION_ID") != null && !transactionMap.get("ARBITRATION_ID").equals("")
                    && chargesMap.containsKey("ARBITRATION_TRANSACTION_CASH") && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getArbitrationAmt()).doubleValue() > 0) {
            ...
            if (transactionTO.getTransType().equals("CASH") && transactionMap != null && transactionMap.get("NOTICE_ID") != null && !transactionMap.get("NOTICE_ID").equals("")
                    && chargesMap.containsKey("NOTICE_TRANSACTION_CASH") && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt()).doubleValue() > 0) {
            ...
            deleteMap = null;
            tempMap = null;
            gdsReceiptEntryTO.setNetTransId(CommonUtil.convertObjToStr(transactionMap.get("NET_TRANS_ID")));
            gdsReceiptEntryTO.setStatusDt((Date)currDt.clone());
            System.out.println("gdsReceiptEntryTO :" + gdsReceiptEntryTO);
            sqlMap.executeUpdate("deleteReceiptEntryTO", gdsReceiptEntryTO);
        }
    }
    ...
    private void authorize(HashMap map, LogDAO objLogDAO, LogTO objLogTO) throws Exception {
        ...
        List allApplnDetails = new ArrayList();
        ...
        if (map.containsKey("TransactionTO")) {
            ...
            if (transactionTO.getTransType().equals("TRANSFER") || transactionTO.getTransType().equals("CASH")) {
                ...
                if (transactionMap != null && transactionMap.containsKey("NET_TRANS_ID") && transactionMap.get("NET_TRANS_ID") != null && !transactionMap.get("NET_TRANS_ID").equals("")) {
                    ...
                    if (linkBatchId != null && linkBatchId.indexOf(",") != -1) {
                ...
                if (transactionTO.getTransType().equals("CASH")) {                    
                    if (transactionMap != null && transactionMap.containsKey("NET_TRANS_ID") && transactionMap.get("NET_TRANS_ID") != null && !transactionMap.get("NET_TRANS_ID").equals("")) {
                        ...
                        System.out.println("transList.. cash.. "+ transList);
                        for (int i = 0; i < transList.size(); i++) {
                            ArrayList arrList = new ArrayList();
                            linkBatchId = CommonUtil.convertObjToStr(transList.get(i));
                            //singleAuthorizeMap.put("TRANS_ID", transactionMap.get("NET_TRANS_ID"));
                            singleAuthorizeMap.put("TRANS_ID", linkBatchId);
                ...
                if (transactionMap != null && transactionMap.containsKey("BONUS_TRANS_ID") && transactionMap.get("BONUS_TRANS_ID") != null && !transactionMap.get("BONUS_TRANS_ID").equals("")) {
                    ...
                    System.out.println("gdsReceiptEntryTO.getBranchCode() :" + gdsReceiptEntryTO.getBranchCode());
                    System.out.println("bbbbbbbbb.getBranchCode() :" + map.get("CHITTAL_NO"));
                    // System.out.println("bbbbbbbbb.chitNochitNo() :" +chitNo);
                    String branchCode = CommonUtil.convertObjToStr(map.get("BRANCH_CODE"));
                ...
                if (gdsReceiptEntryTO.getBankPay().equals("N")) {
                    HashMap transactionaApplnMap = new HashMap();
                    transactionaApplnMap.put("TOTAL_BALANCE", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getInstAmt()));
                    transactionaApplnMap.put("CLEAR_BALANCE", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getInstAmt()));
                    transactionaApplnMap.put("AVAILABLE_BALANCE", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getInstAmt()));
                    transactionaApplnMap.put("SHADOW_CREDIT", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getInstAmt()));
                    transactionaApplnMap.put("SHADOW_DEBIT", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getInstAmt()));
                    ...
                    allApplnDetails = sqlMap.executeQueryForList("getAllApplcationDetailsForGDS", map);                   
                    for (int i = 0; i < allApplnDetails.size(); i++) {
                        GDSApplicationTO objGDSApplicationTO = (GDSApplicationTO)allApplnDetails.get(i);                       
                    ...
                    System.out.println("isSplitMDSTransaction^$^$^$^$"+isSplitMDSTransaction);
                    System.out.println("map.get(WHOLE_SPLIT_MAP)^$^$^$^$"+map.get("WHOLE_SPLIT_MAP"));
                    ...
                    if (isSplitMDSTransaction != null && isSplitMDSTransaction.equals("Y")) {
                        ...
                        if (map != null && map.containsKey("WHOLE_SPLIT_MAP") && map.get("WHOLE_SPLIT_MAP") != null) {
                            ...
                            System.out.println("Nithus closed split transaction " + maps);
                            if (maps.containsKey("gdsReceiptEntryTO") && maps.get("gdsReceiptEntryTO") != null) {
                                lsts = (List) maps.get("gdsReceiptEntryTO");
                            }
                        }
                        System.out.println("lsts^$^$^$^$"+lsts);
                        if (lsts != null && lsts.size() > 0) {
                            for (int i = 0; i < lsts.size(); i++) {
                                GDSReceiptEntryTO MDSTo = (GDSReceiptEntryTO) lsts.get(i);
                                HashMap mdsTransMap = new HashMap();
                                System.out.println("11111|" + MDSTo.getBonusAmtPayable());
                                System.out.println("222222|" + MDSTo.getPenalAmtPayable());
                                mdsTransMap.put("SCHEME_NAME", gdsReceiptEntryTO.getSchemeName());
                                mdsTransMap.put("CHITTAL_NO", gdsReceiptEntryTO.getChittalNo());
                                mdsTransMap.put("SUB_NO", CommonUtil.convertObjToInt(gdsReceiptEntryTO.getSubNo()));
                                ...
                                mdsTransMap.put("MDS_INTEREST", gdsReceiptEntryTO.getMdsInterset());
                                ...
                                System.out.println("setNetAmtsetNetAmt" + gdsReceiptEntryTO.getNetAmt());
                                gdsReceiptEntryTO.setNetAmt(CommonUtil.convertObjToDouble(MDSTo.getNetAmt()) - noticeAmount - arbitrationAmount - setvideTaxAmount);
                                System.out.println("noticeAmountnoticeAmount" + noticeAmount + "  " + gdsReceiptEntryTO.getNetAmt());
                                ...
                                maxListMap.put("SCHEME_NAME", gdsReceiptEntryTO.getSchemeName());
                                maxListMap.put("CHITTAL_NO", gdsReceiptEntryTO.getChittalNo());
                                maxListMap.put("SUB_NO", CommonUtil.convertObjToInt(gdsReceiptEntryTO.getSubNo()));
                        ...
                        for (int i = 0; i < allApplnDetails.size(); i++) {
                            ...
                            HashMap mdsTransMap = new HashMap();
                            mdsTransMap.put("SUB_NO", CommonUtil.convertObjToInt(gdsReceiptEntryTO.getSubNo()));
                            mdsTransMap.put("NO_OF_INST", CommonUtil.convertObjToInt(gdsReceiptEntryTO.getNoOfInstPay()));
                            mdsTransMap.put("INST_AMT", gdsReceiptEntryTO.getInstAmt()/schemeCount);
                            mdsTransMap.put("PENAL_AMT", gdsReceiptEntryTO.getPenalAmtPayable()/schemeCount);  
                            mdsTransMap.put("BANK_ADV_AMTCR",gdsReceiptEntryTO.getBankAdvanceAmt()/schemeCount);
                            mdsTransMap.put("FORFEITED_AMT",gdsReceiptEntryTO.getForfeitBonusAmtPayable()/schemeCount);
                            mdsTransMap.put("DISCOUNT_AMT", gdsReceiptEntryTO.getDiscountAmt()/schemeCount);
...
//                                mdsTransMap.put("DISCOUNT_AMT", 0);
                                ...
                                mdsTransMap.put("BONUS_AMT", gdsReceiptEntryTO.getBonusAmtPayable()/schemeCount);
                                mdsTransMap.put("DISCOUNT_AMT", gdsReceiptEntryTO.getDiscountAmt()/schemeCount);
                            }
                            mdsTransMap.put("MDS_INTEREST", gdsReceiptEntryTO.getMdsInterset()/schemeCount);
                            mdsTransMap.put("NOTICE_AMT", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt()/schemeCount));
                            mdsTransMap.put("ARBITRATION_AMT", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getArbitrationAmt()/schemeCount));
                            mdsTransMap.put("SERVICE_TAX_AMT", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getServiceTaxAmt()/schemeCount));
                            mdsTransMap.put("NARRATION", CommonUtil.convertObjToStr(gdsReceiptEntryTO.getNarration()));
                            double noticeAmount = CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt());//9535
                            System.out.println("setNetAmtsetNetAmt" + gdsReceiptEntryTO.getNetAmt());
                            //gdsReceiptEntryTO.setNetAmt(CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNetAmt()) - noticeAmount);
                            System.out.println("noticeAmountnoticeAmount" + noticeAmount + "  " + gdsReceiptEntryTO.getNetAmt());
                            mdsTransMap.put("NET_AMT", (CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNetAmt()) - noticeAmount)/schemeCount);
                            //mdsTransMap.put("SUBSCRIPTION_AMT", CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getInstAmtPayable()/schemeCount)); // 01-11-2019
                            ...
                            mdsTransMap.put("AUTHORIZE_DT", setProperDtFormat(currDt));
                            mdsTransMap.put("TRANS_DT", setProperDtFormat(currDt));
                            mdsTransMap.put("GDS_NO",gdsReceiptEntryTO.getGds_no());
                            mdsTransMap.put("NET_TRANS_ID", transactionMap.get("NET_TRANS_ID"));
                            HashMap maxListMap = new HashMap();
                            maxListMap.put("SCHEME_NAME", gdsReceiptEntryTO.getSchemeName());
                            maxListMap.put("CHITTAL_NO", gdsReceiptEntryTO.getChittalNo());
                ...
                if (authorizeStatus != null && authorizeStatus.equals(CommonConstants.STATUS_REJECTED)) {
                    //updateChitCloseDate(gdsReceiptEntryTO, CommonConstants.STATUS_REJECTED);
                }
                serviceAuthMap.put("AUTHORIZE_BY", map.get("USER_ID"));
...
//                authorizeReceiptMap.put("NET_TRANS_ID",transactionTO.getBatchId());
                ...
                HashMap whereMap = new HashMap();
                double instNo = CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getCurrInst()).doubleValue()
                        - CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getPendingInst()).doubleValue()
                        + CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoOfInstPay()).doubleValue();
                whereMap.put("INSTALLMENT_NO", CommonUtil.convertObjToInt(instNo));
                whereMap.put("CHITTAL_NO", gdsReceiptEntryTO.getChittalNo());
                whereMap.put("SUB_NO", CommonUtil.convertObjToInt(gdsReceiptEntryTO.getSubNo()));
                ...
                whereMap.put("REPAID_DT",currDt.clone());//Added by nithya on 07-11-2018 for KD 296 : 0016388: mds bank advance modification-Kuttilanji scb(GDS module)
                double number = CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoOfInstPay()).doubleValue();
                ...
                System.out.println("authorizeStatus"+authorizeStatus+"gdsReceiptEntryTO:"+gdsReceiptEntryTO);
                ...
                if (authorizeStatus.equals("AUTHORIZED")) {
                    if (transactionMap != null && transactionMap.get("ARBITRATION_ID") != null && !transactionMap.get("ARBITRATION_ID").equals("")
                            && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getArbitrationAmt()).doubleValue() > 0) {
                        //Update Paid Charge Amount
                        HashMap chargeMap = new HashMap();
                        chargeMap.put("CASE_CHARGE_DETAILS", "CASE_CHARGE_DETAILS");
                        for (int i = 0; i < allApplnDetails.size(); i++) {
                            GDSApplicationTO objGDSApplicationTO = (GDSApplicationTO) allApplnDetails.get(i);
                            String actNo = CommonUtil.convertObjToStr(objGDSApplicationTO.getChittalNo()) + "_" + CommonUtil.convertObjToStr(objGDSApplicationTO.getSubNo());
                    ...
                    if (transactionMap != null && transactionMap.get("NOTICE_ID") != null && !transactionMap.get("NOTICE_ID").equals("")
                            && CommonUtil.convertObjToDouble(gdsReceiptEntryTO.getNoticeAmt()).doubleValue() > 0) {
                        //Update Paid Charge Amount
                        HashMap chargeMap = new HashMap();
            ...
            if (authorizeStatus.equals(CommonConstants.STATUS_AUTHORIZED)) {
                ...
                smsAlertMap.put("SCHEME_NAME", gdsReceiptEntryTO.getSchemeName());
                smsAlertMap.put("CHITTAL_NO", gdsReceiptEntryTO.getChittalNo());
...
private void chargesCollected(String act_num, HashMap inputMap) throws Exception {
    ...
    private void doUnmarkLean() throws Exception {
        String chittalNo = CommonUtil.convertObjToStr(gdsReceiptEntryTO.getChittalNo());
        String subNo = CommonUtil.convertObjToStr(gdsReceiptEntryTO.getSubNo());
    ...
    public HashMap execute(HashMap map, boolean isTransaction) throws Exception {
        ...
        ibrHierarchy = 1;
        if (map.containsKey("gdsReceiptEntryTO") || map.containsKey("GDSReceiptEntryTO")) {
            gdsReceiptEntryTO = new GDSReceiptEntryTO();
            if(map.containsKey("gdsReceiptEntryTO")){
                gdsReceiptEntryTO = (GDSReceiptEntryTO) map.get("gdsReceiptEntryTO");
            }else if (map.containsKey("MDS_TRANS_ALL_AUTHORIZATION") && map.containsKey("gdsReceiptEntryTO")){
                gdsReceiptEntryTO = (GDSReceiptEntryTO) ((List) map.get("gdsReceiptEntryTO")).get(0);
            ...
            final String command = gdsReceiptEntryTO.getCommand();
            try {
    ...
    private void destroyObjects() {
        gdsReceiptEntryTO = null;
        TxTransferTO = null; // Added by nithya on 05-12-2019 for KD-1021
    }
		//added by chithra for service Tax
 private void insertServiceTaxDetails(ServiceTaxDetailsTO objserviceTaxDetailsTO) {
    ...
    public static void main(String str[]) {
        try {
            GDSReceiptEntryDAO dao = new GDSReceiptEntryDAO();
    ...
    private void updateChitCloseDate(GDSReceiptEntryTO gdsReceiptEntryTO, String status) throws Exception {
        ...
        chitCloseMap.put("SCHEME_NAME", CommonUtil.convertObjToStr(gdsReceiptEntryTO.getSchemeName()));
        chitCloseMap.put("CHITTAL_NO", CommonUtil.convertObjToStr(gdsReceiptEntryTO.getChittalNo()));
        chitCloseMap.put("SUB_NO", CommonUtil.convertObjToInt(gdsReceiptEntryTO.getSubNo()));
        chitCloseMap.put("DIV_NO", CommonUtil.convertObjToInt(gdsReceiptEntryTO.getDivisionNo()));
        chitCloseMap.put("GDS_NO", CommonUtil.convertObjToStr(gdsReceiptEntryTO.getGds_no()));
        List instLst = sqlMap.executeQueryForList("getGDSNoOfInstalmentsPaid", chitCloseMap);
        ...
        int noOfInst = CommonUtil.convertObjToInt(gdsReceiptEntryTO.getNoOfInst());
        if (status != null && !status.equals("") && status.equals(CommonConstants.STATUS_REJECTED)) {
            ...
            noOfInstToPay = CommonUtil.convertObjToInt(gdsReceiptEntryTO.getNoOfInstPay());
            chkNoOfInst = noOfInstPaid + noOfInstToPay;
        }
