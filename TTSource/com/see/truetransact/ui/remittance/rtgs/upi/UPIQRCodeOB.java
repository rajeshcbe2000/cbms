 ...
 * @author  bala
 */
public class UPIQRCodeOB extends CObservable {
    ...
    private ComboBoxModel cbmUpiBank;
    ...
    private static UPIQRCodeOB upiQRCodeOB;
    ...
    static {
        try {
            ...
            upiQRCodeOB = new UPIQRCodeOB();
        } catch(Exception e) {
        }
    }
    
    public static UPIQRCodeOB getInstance() {
        return upiQRCodeOB;
    }
    
    /** Creates a new instance of ViewAllOB */
    public UPIQRCodeOB() throws Exception {
        initiate();
    }
    ...
    public void fillDropdown() throws Exception {
        try {
            ...
            lookup_keys.add("UPI_PRODUCT_TYPE");
            lookup_keys.add("APP_CATEGORY");
            lookup_keys.add("UPI_BANK");
            lookUpHash.put(CommonConstants.PARAMFORQUERY, lookup_keys);
            keyValue = ClientUtil.populateLookupData(lookUpHash);

            // Added by nithya for sicombobox
            fillData((HashMap) keyValue.get("UPI_PRODUCT_TYPE"));
            cbmProdType = new ComboBoxModel(key, value);
            fillData((HashMap) keyValue.get("APP_CATEGORY"));
            cbmCategory = new ComboBoxModel(key, value);
            fillData((HashMap) keyValue.get("UPI_BANK"));
            cbmUpiBank = new ComboBoxModel(key, value);
    ...
    private void fillDropDown_old() throws Exception{
        ...
        System.out.println("lookupValues 1 :: " + lookupValues);
        lookupValues = ClientUtil.populateLookupData(param);
        fillData((HashMap) lookupValues.get("APP_CATEGORY"));
        cbmCategory = new ComboBoxModel(key, value);
        
        System.out.println("lookupValues 2 :: " + lookupValues);
        lookupValues = ClientUtil.populateLookupData(param);
        fillData((HashMap) lookupValues.get("UPI_BANK"));
        cbmUpiBank = new ComboBoxModel(key, value);
    ...
    private void fillData_old(HashMap keyValue)  throws Exception{
...
//                    Date currDt = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(ClientUtil.getCurrentDate()));
    ...
    public ComboBoxModel getCbmUpiBank() {
        return cbmUpiBank;
    }

    public void setCbmUpiBank(ComboBoxModel cbmUpiBank) {
        this.cbmUpiBank = cbmUpiBank;
    }
