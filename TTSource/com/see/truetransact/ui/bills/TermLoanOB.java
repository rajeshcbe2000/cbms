...
import com.see.truetransact.transferobject.bills.TermLoanJointAcctTO;
import com.see.truetransact.transferobject.bills.TermLoanBorrowerTO;
import com.see.truetransact.transferobject.bills.TermLoanClassificationTO;
import com.see.truetransact.transferobject.bills.TermLoanCompanyTO;
import com.see.truetransact.transferobject.bills.TermLoanFacilityTO;
import com.see.truetransact.transferobject.bills.TermLoanOtherDetailsTO;
import com.see.truetransact.transferobject.bills.TermLoanSanctionFacilityTO;
import com.see.truetransact.transferobject.bills.TermLoanSanctionTO;
...
//        getKeyValue((HashMap)keyValue.get("LOANPRODUCT.OPERATESLIKE"));
    ...
    private void setOperationMap() throws Exception{
        ...
        operationMap.put(CommonConstants.JNDI, "BillsAccountJNDI");
        operationMap.put(CommonConstants.HOME, "BillsAccountHome");
        operationMap.put(CommonConstants.REMOTE, "BillsAccount");
    }
    ...
    public void populateData(HashMap whereMap, AuthorizedSignatoryOB authOB, PowerOfAttorneyOB poaOB) {
        ...
        try {
            whereMap.put("UI_PRODUCT_TYPE", "BILLS");
...
//                setShareTab(mapData);
        ...
        if ((mapData.get("KEY_VALUE").equals("ACCOUNT_NUMBER")) || (mapData.get("KEY_VALUE").equals("AUTHORIZE"))){
            ...
            if (mapData.get("KEY_VALUE").equals("AUTHORIZE")){
                ...
                List resultList = (List) ClientUtil.executeQuery("getBillsSanctionDetails", transactionMap);
    ...
    private void setTermLoanSanctionTO(ArrayList objSanctionTO, ArrayList objSanctionFacilityTO){
        try{
            ...
            for (int i = objSanctionTO.size() - 1,j = 0;i >= 0;--i,++j){
                ...
                List resultList = ClientUtil.executeQuery("getSelectBillsSanctionFacilityMaxSLNO", transactionMap);
                if (resultList.size() > 0){
    ...
    private void setMax_Del_San_Details_No(String borrowNo){
        try{
            ...
            List resultList = ClientUtil.executeQuery("getSelectBillsSanctionMaxSLNO", transactionMap);
            if (resultList.size() > 0){
                retrieve = (HashMap) resultList.get(0);
    ...
    private void doActionPerform(int saveMode) throws Exception{
        ...
        data.put("UI_PRODUCT_TYPE", "BILLS");
        ...
        if (proxyResultMap!=null) {
            if(proxyResultMap.containsKey("ACCTNO") && loanType.equals("OTHERS"))
                ClientUtil.showMessageWindow("Bill Account No. : "+proxyResultMap.get("ACCTNO"));
...
//                objTermLoanFacilityTO.setAuthorizeDt2((Date)oneRecord.get(AUTHORIZE_DT_2));
    ...
    public void setBorrowerNumber(){
        try{
            if (termLoanBorrowerOB.getLblBorrowerNo_2().length() <= 0){
                HashMap transactionMap = new HashMap();
                HashMap retrieve = new HashMap();
                String mapName = "getBillsBorrowerNumber";
                if(loanType.equals("LTD")) {
    ...
    public void updateProd_ID(int sanctionNo, int slno){
        try{
            ...
            ClientUtil.execute("updateProdID_BillsGuarantorTO", transactionMap);
            ClientUtil.execute("updateProdID_BillsInterestTO", transactionMap);
            ClientUtil.execute("updateProdID_BillsClassificationTO", transactionMap);
    ...
    private void populateAccountNumber(String strSanctionNo, int slno){
        try{
            ...
            transactionMap.put("sanctionNo", strSanctionNo);
            transactionMap.put("slNo", String.valueOf(slno));
            List resultList = (List) ClientUtil.executeQuery("getBillsAccountNumber", transactionMap);
            if (resultList.size() > 0){
    ...
    public boolean checkSanctionNoDublication(String strSanctionNo){
        ...
        try{
            ...
            HashMap retrieve;
            transactionMap.put("sanctionNo", strSanctionNo);
            List resultList = (List) ClientUtil.executeQuery("getBillsCountOfSanctionNo", transactionMap);
