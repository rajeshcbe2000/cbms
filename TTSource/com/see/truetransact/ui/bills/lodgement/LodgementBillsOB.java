...
public class LodgementBillsOB extends CObservable{
    ...
    private static LodgementBillsOB objLodgementBillsOB;//Singleton Object Reference
    private LodgementBillsRB resourceBundle = new LodgementBillsRB();
    private final static Logger log = Logger.getLogger(LodgementBillsOB.class);//Creating Instace of Log
    ...
    private String excludeOtherBankCharge = ""; // Added by nithya on 30-06-2016 for 0004748

    public String getExcludeOtherBankCharge() {
        return excludeOtherBankCharge;
    }

    public void setExcludeOtherBankCharge(String excludeOtherBankCharge) {
        this.excludeOtherBankCharge = excludeOtherBankCharge;
    }
...
//        tblHeadings1.add(resourceBundle.getString("tblHeading4"));
    ...
    public static LodgementBillsOB getInstance()throws Exception{
...
//            getKeyValue((HashMap)keyValue.get("LODGEMENT.INSTRUCTIONS"));
    ...
    private LodgementMasterTO getLodgementMasterTO(String command){
        ...
        objLodgementMasterTO.setProdType(CommonUtil.convertObjToStr(getCbmProductType().getKeyForSelected()));
        objLodgementMasterTO.setBorrowProdId(CommonUtil.convertObjToStr(getCbmProductId().getKeyForSelected()));
        objLodgementMasterTO.setBorrowAcctNum(getTxtAccountNo());
        ...
        objLodgementMasterTO.setIsMultipleLodgement("N"); // Added by nithya on 27-06-2016 for multiple lodgement 
...
//        getCbmProductID().setSelectedItem(objLodgementMasterTO.getCbpProdID());
    ...
    private LodgementChequeTO getLodgementChequeTO(String command){
        ...
        objLodgementChequeTO.setInstrumentAmount(CommonUtil.convertObjToDouble(getTxtInstrumentAmount()));
...
//        }
      ...
      private LodgementRemitTO getLodgementRemitTO(String command){//babu1
...
//        if((getCbmActivities().getKeyForSelected().equals("PROCEEDS_RECEIVED"))){
//        if((getCboActivities().equalsIgnoreCase("PROCEEDS RECEIVED")) && (!getCboReceivedFrom().equalsIgnoreCase("BRANCH"))){
//             objLodgementRemitTO.setRemitCity(getCboRemitCity());
...
//            objLodgementRemitTO.setRemitDraweeBank(CommonUtil.convertObjToStr(getCbmRemitDraweeBank().getKeyForSelected()));
//            objLodgementRemitTO.setRemitDraweeBranchCode(CommonUtil.convertObjToStr(getCbmRemitBranchCode().getKeyForSelected()));
//        }
        objLodgementRemitTO.setRemitInstDt(DateUtil.getDateMMDDYYYY(getTdtRemitInstDate()));
        objLodgementRemitTO.setRemitFavouring(getTxtRemitFavour());
        objLodgementRemitTO.setRemitFavouringIn(getTxtRemitFavour1());
        objLodgementRemitTO.setRemitInstAmt(CommonUtil.convertObjToDouble(getTxtInstAmt()));
...
//            objLodgementBillRatesTO.setNoOfIntDays(new Double(time1 * 1));
    ...
    public void resetForm(){
        ...
        setExcludeOtherBankCharge(""); // Added by nithya on 30-06-2016 for 4748
   }
...
//        }
        ...
        for (int i = 0; i < alLData.size(); i++) {
            ...
            if (temp != null && temp.size() > 1) {
                String chargeType = CommonUtil.convertObjToStr(temp.get(1));
                if (chargeType != null && chargeType.equalsIgnoreCase("BANK_CHARGES")) {
                    checkForTaxMap = checkServiceTaxApplicable();
                    if (checkForTaxMap.containsKey("SERVICE_TAX_APPLICABLE") && checkForTaxMap.get("SERVICE_TAX_APPLICABLE") != null && CommonUtil.convertObjToStr(checkForTaxMap.get("SERVICE_TAX_APPLICABLE")).equalsIgnoreCase("Y")) {
                        if (checkForTaxMap.containsKey("SERVICE_TAX_ID") && checkForTaxMap.get("SERVICE_TAX_ID") != null && CommonUtil.convertObjToStr(checkForTaxMap.get("SERVICE_TAX_ID")).length() > 0) {
                            chargeAmt = CommonUtil.convertObjToDouble(temp.get(2));
                            if (chargeAmt > 0) {
                                taxMap = new HashMap();
                                taxMap.put("SETTINGS_ID", checkForTaxMap.get("SERVICE_TAX_ID"));
                                taxMap.put(ServiceTaxCalculation.TOT_AMOUNT, chargeAmt);
...
//                    setServiceTax_Map(serviceTax_Maptemp);
                    setLblServiceTaxval(amt);
                    serviceTax_Maptemp.put(ServiceTaxCalculation.TOT_TAX_AMT, amt);
                    setServiceTax_Map(serviceTax_Maptemp);
...
//            }
    ...
    public void execute(String command) {
		...
		crBranchId = CommonUtil.convertObjToStr(interBranchCodeMap.get("BRANCH_CODE"));
            ...
            term.put("LodgementMasterTO", getLodgementMasterTO(command));
            ...
            (getCbmInstrumentType().getKeyForSelected().equals("DEMAND DRAFT"))){
                term.put("LodgementChequeTO", getLodgementChequeTO(command));
            ...
            if (lblServiceTaxval != null && CommonUtil.convertObjToDouble(lblServiceTaxval) > 0 && serviceTax_Map != null) {
                ...
                term.put("serviceTaxDetailsTO", setServiceTaxDetails(command));
            }
            ...
            (getCbmActivities().getKeyForSelected().equals("CLOSURE")))
                term.put("LodgementRemitTO", getLodgementRemitTO(command));
...
//                 if((CommonUtil.convertObjToStr(term.get("SUB_REG_TYPE")).equals("CPD")) && (getOperatesLikeValue().equals("OUTWARD"))){
            ...
            // Added by nithya on 30-06-2016 for 4748    
                if ((getCbmActivities().getKeyForSelected().equals("REALIZE")) && (getOperatesLikeValue().equals("OUTWARD"))) {
                if (getExcludeOtherBankCharge().equalsIgnoreCase("Y")) {
                    term.put("OMIT_OTHER_BANK_CHARGE", "OMIT_OTHER_BANK_CHARGE");
                }
            }
...
//                }
        ...
        if(insertData != null && insertData.size()>0){
            for(int i=0; i<insertData.size();i++){
                ...
                objLodgementInstructionsTO.setSlNo(CommonUtil.convertObjToStr(((ArrayList)insertData.get(i)).get(0)));
                objLodgementInstructionsTO.setInstruction(CommonUtil.convertObjToStr(((ArrayList)insertData.get(i)).get(1)));
                objLodgementInstructionsTO.setAmount(CommonUtil.convertObjToDouble(((ArrayList)insertData.get(i)).get(2)));
                objLodgementInstructionsTO.setServiceTx(CommonUtil.convertObjToDouble(((ArrayList)insertData.get(i)).get(3)));
                objLodgementInstructionsTO.setStatus(CommonConstants.STATUS_CREATED);
    ...
    private  void updateData(){
        ...
        if(existingData!=null && existingData.size()!=0){
            for(int i=0;i<existingData.size();i++){
                LodgementInstructionsTO  objLodgementInstructionsTO;
                objLodgementInstructionsTO = new LodgementInstructionsTO();
                objLodgementInstructionsTO.setSlNo(CommonUtil.convertObjToStr(((ArrayList)existingData.get(i)).get(0)));
                objLodgementInstructionsTO.setInstruction(CommonUtil.convertObjToStr(((ArrayList)existingData.get(i)).get(1)));
                objLodgementInstructionsTO.setAmount(CommonUtil.convertObjToDouble(((ArrayList)existingData.get(i)).get(2)));
                objLodgementInstructionsTO.setServiceTx(CommonUtil.convertObjToDouble(((ArrayList)existingData.get(i)).get(3)));
                objLodgementInstructionsTO.setLodgementId(txtLodgementId);
                objLodgementInstructionsTO.setStatus(CommonConstants.STATUS_MODIFIED);
                objLodgementInstructionsTO.setStatusBy(TrueTransactMain.USER_ID);
                objLodgementInstructionsTO.setStatusDt(currDt);
        ...
        if(deletingExists==true && deletedData!=null){
            if(deletedData.size()!=0){
                for(int i=0;i<deletedData.size();i++){
                    LodgementInstructionsTO  objLodgementInstructionsTO;
                    objLodgementInstructionsTO = new LodgementInstructionsTO();
                    objLodgementInstructionsTO.setSlNo(CommonUtil.convertObjToStr(deletedData.get(i)));
                    objLodgementInstructionsTO.setInstruction(CommonUtil.convertObjToStr(deletedData.get(i)));
                    objLodgementInstructionsTO.setAmount(CommonUtil.convertObjToDouble(deletedData.get(i)));
                    objLodgementInstructionsTO.setServiceTx(CommonUtil.convertObjToDouble(deletedData.get(i)));
                    objLodgementInstructionsTO.setLodgementId(txtLodgementId);
...
//            }
//        }
        System.out.println("updated tblInstructionData===="+tblInstructionData);
    }
    ...
    private void deletedData(){
        if(removedRow != null && removedRow.size()>0){
            for(int i=0; i<removedRow.size();i++){
                ...
                objLodgementInstructionsTO = new LodgementInstructionsTO();
                objLodgementInstructionsTO.setLodgementId(txtLodgementId);
                objLodgementInstructionsTO.setSlNo(CommonUtil.convertObjToStr(((ArrayList)removedRow.get(i)).get(0)));
                objLodgementInstructionsTO.setInstruction(CommonUtil.convertObjToStr(((ArrayList)removedRow.get(i)).get(1)));
                objLodgementInstructionsTO.setAmount(CommonUtil.convertObjToDouble(((ArrayList)removedRow.get(i)).get(2)));
                objLodgementInstructionsTO.setServiceTx(CommonUtil.convertObjToDouble(((ArrayList)removedRow.get(i)).get(3)));
                objLodgementInstructionsTO.setStatus(CommonConstants.STATUS_DELETED);
                objLodgementInstructionsTO.setStatusBy(TrueTransactMain.USER_ID);
                objLodgementInstructionsTO.setStatusDt(currDt);
...
//    }
    ...
    private HashMap checkServiceTaxApplicable() {// function rewritten by nithya for service tax to gst conversion
        HashMap whereMap = new HashMap();
        whereMap.put("value", CommonUtil.convertObjToStr(getCbmBillsType().getKeyForSelected()));
        String checkFlag = "N";
        HashMap checkForTaxMap = new HashMap();
    ...
    public ServiceTaxDetailsTO setServiceTaxDetails(String command) {
