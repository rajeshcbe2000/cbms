 ...
 * @author  amathan
 */
public class EmployeeMasterUISupport {
    private final static ClientParseException parseException = ClientParseException.getInstance();
    private final EmployeeMasterRB objEmployeeRB = new EmployeeMasterRB();
    EmployeeMasterOB observable;
    ...
    /** Creates a new instance of EmployeeUI */
    public EmployeeMasterUISupport() {
    }
    
    public EmployeeMasterUISupport(EmployeeMasterOB observable) {
...
//                else
    ...
    public boolean chkMinAddrTypeCommAddr(CTable tblContactList){
        ...
        if (tblContactList.getRowCount() == 0){
            minAddrType = false;
            displayAlert(objEmployeeRB.getString("minAddrType"));
...
//            commAddr = false;
//            displayAlert(objEmployeeRB.getString("commAddrAlert"));
//        }
     ...
     public boolean chkIncomeParticulars(CTable tblIncParticulars){
        ...
        if (tblIncParticulars.getRowCount() == 0){
            displayAlert(objEmployeeRB.getString("IncomePar"));
            inc = true;
    }
          return inc;
     }
     
      public boolean chkLandDetails(CTable tblEmployeeLandDetails){
          boolean land = false;
        if (tblEmployeeLandDetails.getRowCount() == 0){
            displayAlert(objEmployeeRB.getString("LandPar"));
            land = true;
    }
          return land;
     }
...
//        if (observable.getSignFile() != null && observable.getSignFile().length() > 0 ){
    ...
    public void fillImage(CLabel lblImage,CButton btnRemove){
        ...
        if (lblImage.getName().equals(PHOTO)){
            if (getPhotoAction()==0) {
                fileName.append("Employee/photos/").append(observable.getPhotoFile());
...
//                fileName = new StringBuffer(observable.getPhotoFile());
            ...
            if (getSignAction()==0) {
                fileName.append("Employee/signatures/").append(observable.getSignFile());
