...
public class DeathMarkingOB extends CObservable{
    ...
    private DeathMarkingOB() throws Exception {
        ...
        map.put(CommonConstants.HOME, "deposit.deathmarking.DeathMarkingHome");
        map.put(CommonConstants.REMOTE,"deposit.deathmarking.DeathMarking");
        try {
   ...
   /* Getter for tblDeathMarkingModel **/
    ...
    public void setTblDeathMarkingTitle() throws Exception{
        tblDeathMarkingTitle.add(objDeathMarkingRB.getString("lblDepositNo"));
        tblDeathMarkingTitle.add(objDeathMarkingRB.getString("lblTotalBalance"));
...
//        objDeathMarkingTO.setReportedOn(DateUtil.getDateMMDDYYYY(getDtdReportedOn()));
        Date dtDate = DateUtil.getDateMMDDYYYY(getDtdDtOfDeath());
        if(dtDate != null){
        Date Dt = (Date) curDate.clone();
        Dt.setDate(dtDate.getDate());
        Dt.setMonth(dtDate.getMonth());
        Dt.setYear(dtDate.getYear());
        objDeathMarkingTO.setDeathDt(Dt);
        }else{
           objDeathMarkingTO.setDeathDt(DateUtil.getDateMMDDYYYY(getDtdDtOfDeath()));
        }
        
        Date rtDate = DateUtil.getDateMMDDYYYY(getDtdReportedOn());
        if(rtDate != null){
        Date Rt = (Date) curDate.clone();
        Rt.setDate(rtDate.getDate());
        Rt.setMonth(rtDate.getMonth());
        Rt.setYear(rtDate.getYear());
        objDeathMarkingTO.setReportedOn(Rt);
        }else{
           objDeathMarkingTO.setReportedOn(DateUtil.getDateMMDDYYYY(getDtdReportedOn())); 
        }
        ...
        objDeathMarkingTO.setStatus(CommonConstants.STATUS_CREATED);
    ...
    public  void setDeathMarkingInfoList(ArrayList arrayDeathMarkingInfo) {
        ...
        for (int i=0;i<toSize;i++){
            ...
            tblDeathMarkingInfoRow.add(columnData.get("Deposit No."));
            tblDeathMarkingInfoRow.add(columnData.get("Total Balance"));
