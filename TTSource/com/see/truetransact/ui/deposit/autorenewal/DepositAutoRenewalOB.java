 ...
 * 
 * SBInterestApplicationOB.java
 *
 * Created on March 1, 2019, 11:01 AM
 */

package com.see.truetransact.ui.deposit.autorenewal;

import com.see.truetransact.ui.operativeaccount.interestapplication.*;
 ...
 * @author  Lohith R.
 */
public class DepositAutoRenewalOB extends Observable{
    private final static ClientParseException parseException = ClientParseException.getInstance();
    private String lblStatus = ClientConstants.ACTION_STATUS[ClientConstants.ACTIONTYPE_CANCEL];
    private static DepositAutoRenewalOB objLookupMasterOB;
//    private static LookupMasterRB objLookupMasterRB = new LookupMasterRB();
    ...
    private ComboBoxModel cbmExecutionType ;
    private ComboBoxModel cbmProduct;
    
    static {
        try {
            objLookupMasterOB = new DepositAutoRenewalOB();
        } catch(Exception e) {
    ...
    public DepositAutoRenewalOB() throws Exception {
...
//        setTxtLookupID("");
    ...
    private void setOperationMap() throws Exception {
        ...
        operationMap.put(CommonConstants.JNDI, "DepositAutoRenewalJNDI");
        operationMap.put(CommonConstants.HOME, "DepositAutoRenewalHome");
        operationMap.put(CommonConstants.REMOTE, "DepositAutoRenewal");
    }

    
    
    public static DepositAutoRenewalOB getInstance() {
...
//            populateOB(mapData);
    ...
    public void doAction(HashMap paramMap) {
        try {
            System.out.println("paramMap in doAction ::"+ paramMap);
            doActionPerform(paramMap);
        } catch (Exception e) {
    ...
    private void doActionPerform(HashMap dataMap) throws Exception{
        System.out.println("data in OB: " + dataMap);    
        System.out.println("operationMap :: " + operationMap);
        HashMap proxyResultMap = proxy.execute(dataMap, operationMap);
        System.out.println("proxyResultMap :: " + proxyResultMap);
        if(proxyResultMap != null && proxyResultMap.get("STATUS") != null){
            ClientUtil.showMessageWindow(CommonUtil.convertObjToStr(proxyResultMap.get("STATUS")));
        }else{
            ClientUtil.showMessageWindow("Failed !!!");
        }
    ...
    private void fillDropdown() throws Exception {
        ...
        lookupKey.add("APP.DEPOSIT_AUTORENEWAL");
        //lookupKey.add("PRODUCTTYPE");
        cbmProduct = new ComboBoxModel();
        param.put(CommonConstants.PARAMFORQUERY, lookupKey);
        final HashMap lookupValues = ClientUtil.populateLookupData(param);
        fillData((HashMap) lookupValues.get("APP.DEPOSIT_AUTORENEWAL"));
        cbmExecutionType = new ComboBoxModel(key, value);
    }
    
    public void populateProductId() {      
        System.out.println("execution type :: " + CommonUtil.convertObjToStr(cbmExecutionType.getKeyForSelected()));
        HashMap where = new HashMap();
        where.put("EXCTYPE", CommonUtil.convertObjToStr(cbmExecutionType.getKeyForSelected()));
        List productList = ClientUtil.executeQuery("getAllProductsForSBInterestApplication", where);       
        key = new ArrayList();
        value = new ArrayList();
        if (productList != null && productList.size() > 0) {
            for (int i = 0; i < productList.size(); i++) {
                where = (HashMap) productList.get(i);
                String key1 = CommonUtil.convertObjToStr(where.get("KEY"));
                String val1 = CommonUtil.convertObjToStr(where.get("VALUE"));
                cbmProduct = new ComboBoxModel(key, value);
                if (i == 0) {
                    cbmProduct.addKeyAndElement("", "");
                }
                cbmProduct.addKeyAndElement(key1, val1);
            }
        }
    }   
    ...
    public ComboBoxModel getCbmExecutionType() {
        return cbmExecutionType;
    }

    public void setCbmExecutionType(ComboBoxModel cbmExecutionType) {
        this.cbmExecutionType = cbmExecutionType;
    }

    public ComboBoxModel getCbmProduct() {
        return cbmProduct;
    }

    public void setCbmProduct(ComboBoxModel cbmProduct) {
        this.cbmProduct = cbmProduct;
    }
