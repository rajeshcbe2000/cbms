 ...
 *
 */
public class IndendClosingOB extends CObservable {
    ...
    private final static Logger log = Logger.getLogger(IndendClosingOB.class);
    ...
    final ArrayList tblIndendDamageDetailsTitle = new ArrayList();
    private List finalCloseList = null;
    private HashMap editableStopMap = new HashMap();

    /**
     * Creates a new instance of TDS MiantenanceOB
     */
    public IndendClosingOB() {
        try {
            proxy = ProxyFactory.createProxy();
            map = new HashMap();
            map.put(CommonConstants.JNDI, "IndendClosingJNDI");
            map.put(CommonConstants.HOME, "IndendClosingHome");
            map.put(CommonConstants.REMOTE, "IndendClosing");
            fillDropdown();
            setTblIndendCloseDetails();
            tblIndendCloseDetails = new TableModel(null, tblIndendCloseDetailsTitle);
            setTblIndendDamageDetails();
        } catch (Exception e) {
    ...
    public void setTblIndendCloseDetails() {
        ...
        tblIndendCloseDetailsTitle.add("DEPO_ID");
        tblIndendCloseDetailsTitle.add("DESCRIPTION");
        tblIndendCloseDetailsTitle.add("CLOSING_AMT");
        tblIndendCloseDetailsTitle.add("% DEDUCTED_AMT");
        //tblIndendCloseDetailsTitle.add("CLOSING_STOCK_TYPE");
    }

    public void setTblIndendDamageDetails() {
        tblIndendDamageDetailsTitle.add("BRANCH_ID");
        tblIndendDamageDetailsTitle.add("DEPO_ID");
        tblIndendDamageDetailsTitle.add("DESCRIPTION");
        tblIndendDamageDetailsTitle.add("CLOSING_AMT");
    }

    //public void cellEditableColumnTrue() {
        //tblIndendCloseDetails.setEditColoumnNo(3);
        //tblIndendCloseDetails.setEditColoumnNo(4);
    //}
    ...
    private void fillDropdown() throws Exception {
        ...
        param.put(CommonConstants.MAP_NAME, null);
        lookupKeys.add("INDEND_STOCK_TYPE");
        lookupKeys.add("INDEND_CLOSE_STOCK");
        param.put(CommonConstants.PARAMFORQUERY, lookupKeys);
        lookupValues = populateData(param);

        fillData((HashMap) lookupValues.get("INDEND_STOCK_TYPE"));
    ...
    private void doActionPerform() throws Exception {
        ...
        data.put("STOCK_TYPE", getCboStockType());
        data.put("CLOSE_DATE", getTdtClosingDt());
        data.put("INDEND_CLOSING_LIST", getFinalCloseList());
    ...
    public void insertTableData(HashMap whereMap) throws Exception {
        try {
            ...
            editableStopMap = new HashMap();
            finalCloseList = null;
            String mapName = "";
            if (CommonUtil.convertObjToStr(whereMap.get("STOCK_TYPE")).equals("CLOSING")) {
                mapName = "getIndendClosingDetails";
            } else {
                mapName = "getIndendDamageDetails";
            }
            ...
            if (closeList != null && closeList.size() > 0) {
                for (int i = 0; i < closeList.size(); i++) {
                    ...
                    rowList.add(CommonUtil.convertObjToStr(dataMap.get("DEPID")));
                    rowList.add(CommonUtil.convertObjToStr(dataMap.get("DEPO_NAME")));
                    rowList.add(CommonUtil.convertObjToDouble(dataMap.get("CLOSING_AMT")));
                    if (CommonUtil.convertObjToStr(whereMap.get("STOCK_TYPE")).equals("CLOSING")) {
                        rowList.add(CommonUtil.convertObjToDouble(dataMap.get("CLOS_PERC_LESS_AMT")));
                    }
                    if (CommonUtil.convertObjToStr(dataMap.get("CLOSE_STOCK_TYPE")).equals("PERMANENT")) {
                       editableStopMap.put(CommonUtil.convertObjToStr(dataMap.get("BRANCH_CODE"))+ CommonUtil.convertObjToStr(dataMap.get("DEPID")),"PERMANENT");
                    }
                    //rowList.add(new ClosingStockType(CommonUtil.convertObjToStr(dataMap.get("CLOSE_STOCK_TYPE"))));
                ...
                if (CommonUtil.convertObjToStr(whereMap.get("STOCK_TYPE")).equals("CLOSING")) {
                    ...
                    tblIndendCloseDetails = new TableModel((ArrayList) tableList, tblIndendDamageDetailsTitle);
                }
//                cellEditableColumnTrue();
                ...
                if (CommonUtil.convertObjToStr(whereMap.get("STOCK_TYPE")).equals("CLOSING")) {
                    ...
                    tblIndendCloseDetails.setDataArrayList(null, tblIndendDamageDetailsTitle);
                }
            }
        } catch (Exception e) {
            parseException.logException(e, true);
    ...
    public HashMap getEditableStopMap() {
        return editableStopMap;
    }

    public void setEditableStopMap(HashMap editableStopMap) {
        this.editableStopMap = editableStopMap;
    }
}
