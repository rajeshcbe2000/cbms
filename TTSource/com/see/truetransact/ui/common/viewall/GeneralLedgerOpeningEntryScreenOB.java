 ...
 * @author bala
 */
public class GeneralLedgerOpeningEntryScreenOB extends CObservable {
    ...
    private static GeneralLedgerOpeningEntryScreenOB genLedgerOB;
    ...
    static {
        try {
            ...
            genLedgerOB = new GeneralLedgerOpeningEntryScreenOB();
        } catch (Exception e) {
        }
    }

    public static GeneralLedgerOpeningEntryScreenOB getInstance() {
        return genLedgerOB;
    }
     ...
     */
    public GeneralLedgerOpeningEntryScreenOB() throws Exception {
        initiate();
    }
    ...
    public void getGrandTotalAmount(HashMap map){
        HashMap assetMap = new HashMap();
        List assetBal = null;
        assetBal = ClientUtil.executeQuery("getAssetBalance", map);
        assetMap = (HashMap) assetBal.get(0); 
        setAssetBal(CommonUtil.convertObjToDouble(assetMap.get("ASSETSBAL")));
        assetBal.clear();
        assetBal = null;
        assetMap.clear();
        assetMap = null; 
        HashMap liableMap = new HashMap();
        List liableBal = null;
        liableBal = ClientUtil.executeQuery("getLiableBalance", map);
        liableMap = (HashMap) liableBal.get(0); 
        setLiableBal(CommonUtil.convertObjToDouble(liableMap.get("LIABBAL")));
        liableBal.clear();
        liableBal = null;
        liableMap.clear();
        liableMap = null;
...
//    }
    ...
    public ArrayList populateData(HashMap mapID, CTable tblData) {
        ...
        _heading.add("MAJORHEAD");
        _heading.add("ACCOUNT");
        _heading.add("CLOSE_BAL");
        _heading.add("SIDE");
        _heading.add("BRCODE");
        _heading.add("AC_HD_ID");
        _heading.add("OLD_CLOSE_BAL");
        //data = (ArrayList) dataHash.get(CommonConstants.TABLEDATA);
        data = new ArrayList();
        //ArrayList arrList = new ArrayList();
        System.out.println("##$#$#$#$#$##$# Gl list : "+glList);
        if (glList != null && glList.size() > 0) {
            for (int i = 0; i < glList.size(); i++) {
                ArrayList arrList = new ArrayList();
                //arrList = (ArrayList) data.get(i);
                HashMap map = (HashMap)glList.get(i);
                arrList.add(CommonUtil.convertObjToStr(map.get("MAJORHEAD")));
                arrList.add(CommonUtil.convertObjToStr(map.get("ACCOUNT")));
                arrList.add(CommonUtil.convertObjToStr(map.get("CLOSE_BAL")));
                arrList.add(CommonUtil.convertObjToStr(map.get("SIDE")));
                arrList.add(CommonUtil.convertObjToStr(map.get("BRCODE")));
                arrList.add(CommonUtil.convertObjToStr(map.get("AC_HD_ID")));
                arrList.add(CommonUtil.convertObjToStr(map.get("OLD_CLOSE_BAL")));
//                if (!getProdType().equals("TD")) {
//                    arrList.add(0, new Boolean(false));
//                }
                data.add(arrList);
        ...
        if(data==null || data!=null && data.size()==0){
            ...
            populateTable();
            _tblData.setAutoCreateRowSorter(true);//KD-3505
       // }
        whereMap = null;
        return _heading;
    }
    ...
    public void populateTable() {
        ...
        boolean dataExist;
        if (_heading != null && (data!=null && data.size()>0)) {
            _isAvailable = true;
            dataExist = true;
