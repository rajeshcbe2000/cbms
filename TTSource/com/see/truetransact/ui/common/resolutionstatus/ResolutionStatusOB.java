...
/** Resolution Observable is the supporting class for Resolution UI
 *
 * @author bala
 */
public class ResolutionStatusOB extends Observable {
    ...
    public ResolutionStatusOB() {
    }
    
    /** updateStatus method used to update the database field based on the UI button
     * pressed
     *
     * @param map       HashMap from UI which is passed as a argument to Resolution UI constructor
     * @param status    Passed by UI. (Resolution - status)
     */    
    public void updateStatus(HashMap map, String status) {
        ...
        HashMap updateMap = (HashMap) map.get(CommonConstants.UPDATE_MAP_WHERE);
        HashMap dataMap;
        for (int i=0, j=selectedList.size(); i < j; i++) {
            ...
            dataMap.put ("AUTHORIZEDT", ClientUtil.getCurrentDate());
            if (updateMap != null) {
