...
public class QueryReportOB extends CObservable{
    ...
    private final static Logger log = Logger.getLogger(QueryReportOB.class);
    ...
    private EnhancedTableModel tblParameterImportData;
    private CTable _tblData;
    private HashMap dataHash;
    private ArrayList data;
    private ArrayList _heading;
    private int dataSize;
    private EnhancedTableModel clearTable,clearParamTable;
    ...
    private HashMap reportMap = new HashMap();
    
    /** Creates a new instance of TDS MiantenanceOB */
    public QueryReportOB() {
        try {
   ...
   // To Fill the Combo boxes in the UI
    private void fillDropDown() throws Exception {
        ...
        List lst = (List)ClientUtil.executeQuery("getQueryReportNames", where);
        getMap(lst);
    ...
    public HashMap populateQuery(String queryName) throws Exception{
        if(queryName.length()>0){
            ...
            rset = stmt.executeQuery(queryName);  
            System.out.println("$#@$@$@$"+rset);
                     
        }
        return getHashMap(rset);  
    }
    
    public  HashMap getHashMap(ResultSet rowData) throws SQLException {
        ...
        if(colCount>0){
            ...
            if (!rowData.next() ) {
                ClientUtil.showAlertWindow("No Results Found Or Please check the Query!!!");
            } else {
                do {
                    ...
                    for (int i = 1; i <= colCount; i++) {                
                        tableHead.add(metaData.getColumnLabel(i));  
                        tmp.add(rowData.getObject(i));
                    }
                    tableData.add(tmp); 
                } while (rowData.next());
            }            
  ...
  public void populateParameterData(HashMap mapID, CTable tblData) {
        _tblData = tblData;
...
//        _isAvailable = ClientUtil.setTableModel(mapID, tblData);
        ...
        tblParameterImportData = new EnhancedTableModel((ArrayList) tblDatanew, tableTitle);
        for(int i=0;i<tblDatanew.size();i++){
                tblParameterImportData.isCellEditable(i,1);
        }
        setTblParameterImportData(tblParameterImportData);
        tblParameterImportData.fireTableDataChanged();
        setDataSize(data.size());        
    } 
      
   
      
    public void populateData(String mapID, CTable tblData) throws Exception {
        _tblData = tblData; 
        dataHash = (HashMap)populateQuery(mapID);
        if(dataHash!=null && dataHash.get("TABLEHEAD")!=null && !dataHash.get("TABLEHEAD").equals("")){
    ...
    public void clearTable() {
        ...
        tblImportData = new EnhancedTableModel((ArrayList) null, null); 
    }
    
    public void clearParamTable() {
        clearParamTable = new EnhancedTableModel((ArrayList) null, null);  
        tblParameterImportData = new EnhancedTableModel((ArrayList) null, null); 
    }
    ...
    public EnhancedTableModel getTblParameterImportData() {
        return tblParameterImportData;
    }

    public void setTblParameterImportData(EnhancedTableModel tblParameterImportData) {
        this.tblParameterImportData = tblParameterImportData;
    }

    public EnhancedTableModel getClearParamTable() {
        return clearParamTable;
    }

    public void setClearParamTable(EnhancedTableModel clearParamTable) {
        this.clearParamTable = clearParamTable;
    }
