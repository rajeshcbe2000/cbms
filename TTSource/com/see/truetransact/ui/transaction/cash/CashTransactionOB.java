 ...
 * @author rahul
 */
public class CashTransactionOB extends CObservable {
    ...
    private final static Logger log = Logger.getLogger(CashTransactionUI.class);
    ...
    private static CashTransactionOB cashTransactionOB;
    ...
    public boolean debitWithdrwalCharge = false;
    private HashMap otherBankMap;
    public String displayWaive = "";
    private String specialRD = "N"; // Added by nithya on 28-03-2020 for KD-1535
    ...
    public boolean isDebitWithdrwalCharge() {
        return debitWithdrwalCharge;
    }

    public void setDebitWithdrwalCharge(boolean debitWithdrwalCharge) {
        this.debitWithdrwalCharge = debitWithdrwalCharge;
    }
   ...
   // For overdue interest -- End
    ...
    public void setProdCash(String prodCash) {
        this.prodCash = prodCash;
    }
    static {
        try {
            //log.info("In CashTransactionOB Declaration");
            cashTransactionOB = new CashTransactionOB();
        } catch (Exception e) {
            log.info("Error in CashTransactionOB Declaration");
        }
    }

    public static CashTransactionOB getInstance(CInternalFrame frm) {
        frame = frm;
     ...
     */
    public CashTransactionOB() throws Exception {
    ...
    public void fillDropdown() throws Exception {
        ...
        if(cbmProdType.containsElement("MDS")){
            cbmProdType.removeKeyAndElement(CommonUtil.convertObjToStr("MDS"));
        }
    }
    ...
    public CashTransactionTO setCashTransaction() {
        ...
        try {
            ...
            //System.out.println("bhdahd666");
            //System.out.println("bhdahd666 amount>>>>" + getTxtAmount());
            ...
            objCashTransactionTO.setParticulars(CommonUtil.convertObjToStr(getTxtParticulars())+" "+CommonUtil.convertObjToStr(getTxtInstrumentNo1())
                    +CommonUtil.convertObjToStr(getTxtInstrumentNo2()));
            ...
            if (objCashTransactionTO.getProdType().equals("GL") || objCashTransactionTO.getProdType().equals("TL") || objCashTransactionTO.getProdType().equals("ATL") || 
                    objCashTransactionTO.getProdType().equals("AD") || objCashTransactionTO.getProdType().equals("AAD") ) {
                objCashTransactionTO.setLinkBatchId(objCashTransactionTO.getActNum());
            }
            if (getRdoTransactionType_Credit() && objCashTransactionTO.getProdType().equals("TD")) {
                ...
                if (lst != null && lst.size() > 0) {
                    ...
                    if (prodMap != null && prodMap.get("BEHAVES_LIKE").equals("RECURRING")) {
                        objCashTransactionTO.setLinkBatchId(objCashTransactionTO.getActNum());
                        //added by rishad for mantis 0003093: Reccuring deposit deposit instalments updation
                        prodMap.put("DEPOSIT_NO_SUB",objCashTransactionTO.getActNum());
                        prodMap.put("NO_OF_INSTALLMENT",getInstallMentNo());
                        List listInstall = ClientUtil.executeQuery("getMinMaxInstalmentNo", prodMap);
                       ...
                       objCashTransactionTO.setParticulars(CommonUtil.convertObjToStr(getTxtParticulars())+" Insatllment from"+fromInstallment
                    +"TO "+toInstallment);
                        }
                    }
                }
    ...
    public void populateData(HashMap whereMap) {
        ...
        try {
            ...
            if (objCashTransactionTO.getSingleTransId() != null && TrueTransactMain.SERVICE_TAX_REQ.equals("Y")) {
                HashMap whrMap = new HashMap();
                whrMap.put("SINGLETRANS_ID", objCashTransactionTO.getSingleTransId());
                whrMap.put("TRANS_DT",curDate);
                whrMap.put("BRANCH_ID",ProxyParameters.BRANCH_ID);
                
//                List lst = ClientUtil.executeQuery("getServiceTaxDetailsForCash", whrMap);
//                if (lst != null && lst.size() > 0) {
...
//                        //setSingleTranId( objCashTransactionTO.getSingleTransId());
//                    }
//                }
            }
        } catch (Exception e) {
    ...
    private void populateOB(HashMap mapData) throws Exception {
        ...
        if (objCashTransactionTO.getLinkBatchId() != null && objCashTransactionTO.getLinkBatchId().length() > 0) {
            ...
            setProdType(objCashTransactionTO.getProdType());
            setTransModType(objCashTransactionTO.getTransModType());
...
//        objCashTransactionTO = (CashTransactionTO) ((List) mapData.get("CashTransactionTO")).get(0);
        ...
        if(mapData.containsKey("WAIVE_OFF_LIST")){
           List waiveOfflist = (ArrayList) mapData.get("WAIVE_OFF_LIST");
            displayWaive = "";
            if(waiveOfflist.size()>0){
                HashMap waiveMap = new HashMap();
                displayWaive += ("<html> <b> Waive Off details: </b>\n");
                for(int i=0; i<waiveOfflist.size();i++){
                    waiveMap = (HashMap) waiveOfflist.get(i);
                    displayWaive += waiveMap.get("REMARKS") + " : "+ waiveMap.get("WAIVE_AMT") + "\n";
                }
            }
        }
        ttNotifyObservers();
    ...
    public HashMap asAnWhenCustomerComesYesNO(String acct_no) {
        ...
        map.put("INITIATED_BRANCH", ProxyParameters.BRANCH_ID);
        List lst = null;
        if (CommonUtil.convertObjToStr(getProdType()).equalsIgnoreCase("TL") || 
                (CommonUtil.convertObjToStr(getProdType()).equalsIgnoreCase("GL") && 
                CommonUtil.convertObjToStr(getTransModType()).equalsIgnoreCase("TL")) ) {
    ...
    private void setCashTransactionTO(CashTransactionTO objCashTransactionTO) throws Exception {
        ...
        if (CommonUtil.convertObjToStr(objCashTransactionTO.getTransType()).equals(DEBIT)) {
        ...
        setLinkBatchIdForGl(CommonUtil.convertObjToStr(objCashTransactionTO.getLinkBatchId()));
    ...
    public boolean calcRecurringDates(String prodId) {
        ...
        if (lst.size() > 0) {
            ...
            if (recurringMap.get("BEHAVES_LIKE").equals("RECURRING") && !getSpecialRD().equalsIgnoreCase("Y")) {
                String depositNo = getTxtAccNo();
                if (depositNo.lastIndexOf("_") != -1) {
                    depositNo = depositNo.substring(0, depositNo.lastIndexOf("_"));
                }
                recurrMap.put("DEPOSIT_NO", depositNo.trim());
...
//        }
        if (ALL_LOAN_AMOUNT != null && getRdoTransactionType_Credit() && (data.get("PRODUCTTYPE").equals("TL") || data.get("PRODUCTTYPE").equals("ATL") || data.get("PRODUCTTYPE").equals("AD"))) {
            ...
            if (isPenalWaiveOff()) {
                ...
                //System.out.println("inside penalwaive" + penalWaiveAmount);
                ALL_LOAN_AMOUNT.put("PENAL_WAIVE_AMT", penalWaiveAmount);
            ...
            //added by rishad 14/03/2014 for setting waiveamount to all_loan_amount map for waive transaction
            if (isInterestWaiveoff()) {
                ALL_LOAN_AMOUNT.put("WAIVE_OFF_INTEREST", "Y");
                ALL_LOAN_AMOUNT.put("INTEREST_WAIVE_AMT", interestWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("INTEREST_WAIVE_OFF", "N");
            }
            if (isNoticeWaiveoff()) {
                ALL_LOAN_AMOUNT.put("NOTICE_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("NOTICE_WAIVE_AMT", noticeWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("NOTICE_WAIVE_OFF", "N");
            }
            if (isPrincipalwaiveoff()) {
                ALL_LOAN_AMOUNT.put("PRINCIPAL_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("PRINCIPAL_WAIVE_AMT", principalWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("PRINCIPAL_WAIVE_OFF", "N");
            }
             if (isArcWaiveOff()) {
                ALL_LOAN_AMOUNT.put("ARC_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("ARC_WAIVE_AMT", arcWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("ARC_WAIVE_OFF", "N");
            }
               if (isArbitraryWaiveOff()) {
                ALL_LOAN_AMOUNT.put("ARBITRAY_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("ARBITRAY_WAIVE_AMT", arbitarayWaivwAmount);
            } else {
                ALL_LOAN_AMOUNT.put("ARBITRAY_WAIVE_OFF", "N");
            }
               
            if (isRecoveryWaiveOff()) {
                ALL_LOAN_AMOUNT.put("RECOVERY_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("RECOVERY_WAIVE_AMT", recoveryWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("RECOVERY_WAIVE_OFF", "N");
            }
            
            if (isMeasurementWaiveOff()) {
                ALL_LOAN_AMOUNT.put("MEASUREMENT_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("MEASUREMENT_WAIVE_AMT", measurementWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("MEASUREMENT_WAIVE_OFF", "N");
            }
           ...
           if (isKoleFieldOperationWaiveOff()) {
            ...
            //End
             ...
             if (isDecreeWaiveOff()) {
                ALL_LOAN_AMOUNT.put("DECREE_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("DECREE_WAIVE_AMT", decreeWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("DECREE_WAIVE_OFF", "N");
            }

            if (isEpCostWaiveOff()) {
                ALL_LOAN_AMOUNT.put("EP_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("EP_WAIVE_AMT", epCostWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("EP_WAIVE_OFF", "N");
            }
            if (isAdvertiseWaiveOff()) {
                ALL_LOAN_AMOUNT.put("ADVERTISE_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("ADVERTISE_WAIVE_AMT", advertiseWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("ADVERTISE_WAIVE_OFF", "N");
            }
            if (isInsuranceWaiveOff()) {
                ALL_LOAN_AMOUNT.put("INSURENCE_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("INSURENCE_WAIVE_AMT", insuranceWaiveAmont);
            } else {
                ALL_LOAN_AMOUNT.put("INSURENCE_WAIVE_OFF", "N");
            }
            if (isLegalWaiveOff()) {
                ALL_LOAN_AMOUNT.put("LEGAL_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("LEGAL_WAIVE_AMT", legalWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("LEGAL_WAIVE_OFF", "N");
            }
             if (isMiscellaneousWaiveOff()) {
                ALL_LOAN_AMOUNT.put("MISCELLANEOUS_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("MISCELLANEOUS_WAIVE_AMT",miscellaneousWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("MISCELLANEOUS_WAIVE_OFF", "N");
            }
          if (isPostageWaiveOff()) {
                ALL_LOAN_AMOUNT.put("POSTAGE_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("POSTAGE_WAIVE_AMT",postageWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("POSTAGE_WAIVE_OFF", "N");
            }
          
          // For overdue interest -- start
            if (isOverDueIntWaiveOff()) {
                ALL_LOAN_AMOUNT.put("OVERDUEINT_WAIVE_OFF", "Y");
                ALL_LOAN_AMOUNT.put("OVERDUEINT_WAIVE_AMT", overDueIntWaiveAmount);
            } else {
                ALL_LOAN_AMOUNT.put("OVERDUEINT_WAIVE_OFF", "N");
            }
          // For overdue interest -- end
            if(CommonUtil.convertObjToDouble(getLblServiceTaxval())>0){
                ALL_LOAN_AMOUNT.put("TOT_SER_TAX_AMT", getLblServiceTaxval());
                ALL_LOAN_AMOUNT.put("SER_TAX_HEAD", serviceTax_Map.get("TAX_HEAD_ID"));
                ALL_LOAN_AMOUNT.put("SER_TAX_MAP", serviceTax_Map);
                data.put("SERVICE_TAX_THR_CHARGES", "SERVICE_TAX_THR_CHARGES");
            }
        ...
        if (getRdoTransactionType_Credit() && data.get("PRODUCTTYPE").equals("TD")) {
            ...
            //rdPenalMap.put("DEPOSIT_NO", CommonUtil.convertObjToStr(getTxtAccNo()));//Added by Ajay Sharma for Mantis ID - 7952 dated 21-May-2014
            ...
            rdPenalMap.put("DEPOSIT_NO", act_Num);
            if (prodMap.get("BEHAVES_LIKE").equals("RECURRING")) {
                ...
                rdPenalMap.put("DELAYED_AMOUNT", penalAmt);
                rdPenalMap.put("TEMP_DELAYED_AMOUNT", penalAmt);
                rdPenalMap.put("DELAYED_MONTH", penalMonth);
                rdPenalMap.put("TEMP_DELAYED_MONTH", penalMonth);
                //System.out.println("*** rdPenalMap map 1: " + rdPenalMap);
                if (actionType != ClientConstants.ACTIONTYPE_AUTHORIZE) {
                    ClientUtil.execute("updateCurrentsDelayAmtDelayMonth", rdPenalMap);
                }
            }
        }
            ...
            if (lstFDpay != null && lstFDpay.size() > 0) {
                ...
                if (lstBehave != null && lstBehave.size() > 0) {
                    ...
                    if (behavesMap.get("BEHAVES_LIKE").equals("FIXED") && fdPay != null && fdPay.equals("Y") && getActionType() == ClientConstants.ACTIONTYPE_NEW) {
                        ...
                        //babu mod for dep int application
                        ...
                        if (prodMap.get("BEHAVES_LIKE").equals("FIXED")) {
                            ...
                            if (lst != null && lst.size() > 0) {
                                ...
                                data.put("INTERESTMAP", interestMap);//1
        ...
        if (objCashTransactionTO.getProdType().equals("TD") && getAuthorizeMap() != null) {
            ...
            if (lst != null && lst.size() > 0) {
                ...
                interestMap.put("PAYABLE_INTEREST", "PAYABLE_INTEREST");
                //System.out.println("Excpetion interestMap :" + interestMap);
                data.put("INTERESTMAP", interestMap);//2
                //System.out.println("Excpetion data :" + data);
            }
        }
        ...
        if (data.get("PRODUCTTYPE").equals("AB") && getOtherBankMap()!=null && !getOtherBankMap().equals("") && getOtherBankMap().size()>0) {
            data.put("OTHER_BANK_MAP", getOtherBankMap());
        }
...
//        }
        ...
        if (getAuthorizeMap() != null) {
            ...
            //added by sreekrishnan for cashier authorization for identifying whether the transaction is loan opening time or not.
            if(TrueTransactMain.CASHIER_AUTH_ALLOWED != null && TrueTransactMain.CASHIER_AUTH_ALLOWED.toString().equals("Y")){
                   HashMap map2 = new HashMap();
                   HashMap screenMap = new HashMap();
                   screenMap.put("LINK_BATCH_ID",getTxtAccNo());
                   screenMap.put("TRANS_DT",curDate.clone());
                   ...
                   if (listData1 != null && listData1.size() > 0) {
                           ...
                           if(map2.get("SCREEN_NAME") != null && map2.get("SCREEN_NAME").toString().equals("Loans/Advances Account Opening")) {
                                data.put("LOAN_CHARGES_CASH_TXN","LOAN_CHARGES_CASH_TXN");
                           }
                   }
            }
//            if(getServiceTaxAccNo()!=null && getServiceTaxAccNo().length()>0){
//             data.put("ServiceTax_AccNo",getServiceTaxAccNo());
//             data.put("ServiceTax_SingleTranId",getSingleTranId());
//            }
           //Added by sreekrishnan for 3866
            //System.out.println("getRdoTransactionType_Debit@#$@#$#@");
            if (getRdoTransactionType_Debit()){
                if(isDebitWithdrwalCharge()){
                    data.put("DEBIT_WITHDRAWAL_CHARGE_AUTH", "DEBIT_WITHDRAWAL_CHARGE_AUTH");
                }
            }
            ...
            //System.out.println("getAccNumGl()111>>>>" + getAccNumGl());
            if (getAccNumGl() != null) {
                //System.out.println("getAccNumGl()222>>>>" + getAccNumGl());
                data.put("ACC_NO_GL", getAccNumGl());
                data.put("ACC_NO_GL_PROD_TYPE",getAccNumGlProdType());
            }
            ...
            data.put("CASH_TRANSACTION",getProdCash());
             if (getActionType() == ClientConstants.ACTIONTYPE_NEW) {
                if (CommonUtil.convertObjToDouble(getLblServiceTaxval())>0 && 
                    ...
                    data.put("serviceTaxDetailsTO", setServiceTaxDetails(null));
                }
            }
              if (TrueTransactMain.SERVICE_TAX_REQ.equals("Y") && getRdoTransactionType_Debit() == true) {
                ServiceTaxCalculation serviceTaxObj = new ServiceTaxCalculation();
                HashMap resltmap = serviceTaxObj.getServiceTaxRates(new HashMap());
                if (resltmap != null && resltmap.containsKey("TAX_HEAD_ID")) {
                    String headId = CommonUtil.convertObjToStr(resltmap.get("TAX_HEAD_ID"));
                    if (headId.equals(getTxtAccHd())) {
                        data.put("UPDATE_SER_TAX_HEAD", setServiceTaxDetails("D"));
                    }
                }
            }
            //Added by sreekrishnan for 3866
            if (getRdoTransactionType_Debit() && data.get("PRODUCTTYPE").equals("OA")){
                if(isDebitWithdrwalCharge()){
                    data.put("DEBIT_WITHDRAWAL_CHARGE", "DEBIT_WITHDRAWAL_CHARGE");
            ...
            data.put("PROD_ID",(String) cbmProdId.getKeyForSelected()); // Added by nithya on 13-01-2020 for KD-978
            proxyReturnMap = proxy.execute(data, operationMap);
        ...
        try {
            ...
            if (lst != null && lst.size() > 0) {
                ...
                if (procAmount <= oaAmt) {
                    ...
                    if (result1 == 0) //                 procChargeHash=null;
                    {
                        return true;
                    }
                }
...
//                value = null;
    ...
    public boolean isAuthorizationPending(String actNum) {
        ...
        transMap.put("TRANS_DT", curDate.clone());
    ...
    public boolean isAuthorizationPendingTL(String actNum) {        
        boolean isPending = false;
        HashMap transMap = new HashMap();
        transMap.put("LINK_BATCH_ID", actNum);
        transMap.put("TRANS_DT", curDate.clone());
        ...
        if (pendingList != null && pendingList.size() > 0) {
            ...
            //System.out.println("hereeeeeeeeeeeeeee"+trans_actnum);
            if (trans_actnum.equals(actNum)) {
                //ClientUtil.showMessageWindow(" There is Pending Transaction Plz Authorize OR Reject first  ");
                isPending = true;
                hashTrans = null;
    ...
    public void setAccountName(String AccountNo) {
        ...
        HashMap jointResultMap = new HashMap();
        String jointCustNames = "";
        ...
        try {
            if (!prodType.equals("")) {
                ...
                if (prodType.equals("GL") && getTxtAccNo().length() > 0 && (getLinkBatchIdForGl() == null || getLinkBatchIdForGl().equals(""))) {
                    //System.out.println("uuiii111");
                    resultList = ClientUtil.executeQuery("getAccountNumberNameTL", accountNameMap);
                } else {
                    //System.out.println("uuiii222");
                    ...
                    List jointCustNameList = ClientUtil.executeQuery("getJointAccountName", accountNameMap);
                    if (jointCustNameList != null && jointCustNameList.size() > 0) {
                        jointResultMap = (HashMap) jointCustNameList.get(0);
                        if(jointResultMap.containsKey("JOINT_CUST_NAME") && jointResultMap.get("JOINT_CUST_NAME") != null){
                         jointCustNames = CommonUtil.convertObjToStr(jointResultMap.get("JOINT_CUST_NAME"));
                        }
                    }
        ...
        if (resultMap.containsKey("CUSTOMER_NAME")) //Added By Suresh (Member No + Customer Name) 20-Aug-2013
        {
            setLblAccName(CommonUtil.convertObjToStr(resultMap.get("MEMBERSHIP_NO")) + ' ' + ' ' + resultMap.get("CUSTOMER_NAME").toString()+" \n "+jointCustNames);
        } else {
            setLblAccName("");
        }
    }
    ...
    public void setLblServiceTaxval(String lblServiceTaxval) {
     ...
     public ServiceTaxDetailsTO setServiceTaxDetails(String transType) {
        ...
        try {
            ...
            if (serviceTax_Map != null && serviceTax_Map.containsKey("SERVICE_TAX")) {
                ...
                objservicetaxDetTo.setServiceTaxAmt(0.0);
            }
            if (serviceTax_Map != null && serviceTax_Map.containsKey("EDUCATION_CESS")) {
                ...
                objservicetaxDetTo.setEducationCess(0.0);
            }
            if (serviceTax_Map != null && serviceTax_Map.containsKey("HIGHER_EDU_CESS")) {
                ...
                objservicetaxDetTo.setHigherCess(0.0);
            }
            ...
            if (serviceTax_Map != null && serviceTax_Map.containsKey("TOT_TAX_AMT")) {
                ...
                objservicetaxDetTo.setTotalTaxAmt(0.0);
            }
            ...
            if(transType!=null && transType.equals("D")){
                objservicetaxDetTo.setTrans_type("D");
                objservicetaxDetTo.setTotalTaxAmt(CommonUtil.convertObjToDouble(getTxtAmount()));
            }
...
//    }
   ...
   public String getEMIinSimpleInterest() {
    ...
    public void setRecoveryWaiveAmount(double recoveryWaivwAmount) {
        this.recoveryWaiveAmount = recoveryWaivwAmount;
    }
