...
public class CashierApprovalOB extends CObservable{
  ...
  private HashMap dataHash;
    ...
    private final static Logger log = Logger.getLogger(CashierApprovalOB.class);//Creating Instace of Log
    private ProxyFactory proxy;
    private int _actionType;//This vairable is used to type of action 1-New 2-Edit 3-Delete
    private int _result;
    private static CashierApprovalOB objCashierApprovalOB;//Singleton Object Reference
...
private EnhancedTableModel tblPayment;
    ...
    public CashierApprovalOB() {
     ...
     static {
        try {
            ...
            objCashierApprovalOB = new CashierApprovalOB();
        } catch(Exception e) {
           // parseException.logException(e,true);
   ...
   //     tableTitle.add("Payment/Receipt");
        tableTitle.add("TransId");
        tableTitle.add("Product");
        tableTitle.add("Name");
        tableTitle.add("Amount");
        tableTitle.add("Account No");
        tableTitle.add("Status By");
...
//        _isAvailable = ClientUtil.setTableModel(mapID, tblData);
        ...
        for (int i = 0; i <=data.size()-1; i++) {
            ...
            newList.add(false);
            newList.add(tmpList.get(0));//3
            newList.add(tmpList.get(1));//4
            newList.add(tmpList.get(2));//1
            newList.add(tmpList.get(3));//5
            newList.add(tmpList.get(4));//2
            newList.add(tmpList.get(5));//0
...
tblPayment = new EnhancedTableModel((ArrayList) tblDatanew, tableTitle);
        setTblReciept(tblPayment);
        setDataSize(data.size());
...
//        System.out.println("### Data : "+data);
     ...
     public static CashierApprovalOB getInstance()throws Exception{
