 ...
 *
 */
public class GDSChangeofMemberUI extends com.see.truetransact.uicomponent.CInternalFrame implements java.util.Observer, UIMandatoryField {
    ...
    GDSChangeofMemberOB observable = null;
    ...
    private void initForm() {
        setFieldNames();
        internationalize();
        observable = new GDSChangeofMemberOB();
        setMaxLengths();
        initComponentData();
        setMandatoryHashMap();
        setHelpMessage();
...
//        }
    ...
    private void updateAuthorizeStatus(String authorizeStatus) {
        ...
        if (viewType == AUTHORIZE && isFilled) {
            ...
            singleAuthorizeMap.put("GDS_NO",txtChittalNo.getText());
            ...
            mapParam.put(CommonConstants.MAP_NAME, "getGDSChangeOfMemberAuthorize");
            isFilled = false;
    ...
    private void popUp(String currAction) {
        ...
        if (currAction.equalsIgnoreCase("Edit") || currAction.equalsIgnoreCase("Delete")) {
        ...
        }else if (currAction.equals("GROUP_DETAILS")) {
            HashMap map = new HashMap();
            map.put("BRANCH_CODE", TrueTransactMain.BRANCH_ID);
            viewMap.put(CommonConstants.MAP_WHERE, map);
            ...
            HashMap map = new HashMap();
            map.put("BRANCH_CODE", TrueTransactMain.BRANCH_ID);
            map.put("GROUP_NO", txtSchemeName.getText());
            viewMap.put(CommonConstants.MAP_WHERE, map);
            //viewMap.put(CommonConstants.MAP_NAME, "getSelectRecordForEnquiryDetails");
            viewMap.put(CommonConstants.MAP_NAME, "getGDSSelectRecordForEnquiryDetails");            
        }
...
//            }else
            if (observable.getActionType() == ClientConstants.ACTIONTYPE_EDIT
                ...
                this.setButtonEnableDisable();
                observable.setTxtExistingNo(CommonUtil.convertObjToStr(hashMap.get("GDS_NO")));
                ...
                System.out.println("inside GROUP_DETAILS hashMap :: "+ hashMap);
                ...
                hashMap.put("SCHEME_NAME",commonScheme);
                List schemeLst = (List) ClientUtil.executeQuery("getSchemeDetailsList", hashMap);
                if (schemeLst != null && schemeLst.size() > 0) {
                    HashMap productMap = new HashMap();
                    productMap = (HashMap) schemeLst.get(0);
                ...
                System.out.println("inside CHITTAL_NO :: hashMap :: " + hashMap);
                ...
                checkMap.put("SUB_NO", CommonUtil.convertObjToInt(hashMap.get("SUB_NO")));                
                checkMap.put("GDS_NO", hashMap.get("GDS_NO"));
                checkMap.put("GROUP_NO",txtSchemeName.getText());
                List prizedLst = (List) ClientUtil.executeQuery("getSelectGDSPaymentDetails", checkMap);
                if (prizedLst != null && prizedLst.size() > 0) {
                    ...
                    observable.setTxtChitNo(CommonUtil.convertObjToStr(hashMap.get("GDS_NO")));
                    ...
                    //whereMap.put("SUB_NO", CommonUtil.convertObjToStr(observable.getTxtSubNo()));
                    whereMap.put("SUB_NO", CommonUtil.convertObjToInt(observable.getTxtSubNo())); //27-11-2020
                    List list = ClientUtil.executeQuery("getTotalInstAmount", whereMap);
                    if (list != null && list.size() > 0) {
                        whereMap = (HashMap) list.get(0);
                        double paidAmt = CommonUtil.convertObjToDouble(whereMap.get("PAID_AMT")) * observable.getSchemeCount();
                        ...
                        txtTotalAmount.setText(CommonUtil.convertObjToStr(paidAmt));
                        txtInstallmentNo.setEnabled(false);
                    }
                }
                viewType = "";
    ...
    public static void main(String[] arg) {
        ...
        GDSChangeofMemberUI gui = new GDSChangeofMemberUI();
