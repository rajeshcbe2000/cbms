 ...
 * 
 * StandingInstructionDailyOB.java
 *
...
public class StandingInstructionDailyOB extends CObservable {
    ...
    //private final static Logger log = Logger.getLogger(StandingInstructionOB.class);
    ...
    private String tdtIntCalcDt = "" ; // Added by nithya on 14-02-2017

    public String getTdtIntCalcDt() {
        return tdtIntCalcDt;
    }

    public void setTdtIntCalcDt(String tdtIntCalcDt) {
        this.tdtIntCalcDt = tdtIntCalcDt;
    }
    ...
    public void setTblData(com.see.truetransact.clientutil.EnhancedTableModel tblData) {
    ...
    private static StandingInstructionDailyOB standingInstructionDailyOB;

    static {
        try {
            //log.info("Creating AccountCreationOB...");
            standingInstructionDailyOB = new StandingInstructionDailyOB();
        } catch (Exception e) {
            parseException.logException(e, true);
        }
    }

    public StandingInstructionDailyOB() throws Exception {
        curDate = ClientUtil.getCurrentDate();
        map = new HashMap();
        map.put(CommonConstants.JNDI, "StandingInstructionDailyJNDI");
        map.put(CommonConstants.HOME, "supporting.StandingInstructionDaily.StandingInstructionDailyHome");
        map.put(CommonConstants.REMOTE, "supporting.StandingInstructionDaily.StandingInstructionDaily");
        /*
         * 
         */
    ...
    public void setStandingTableTile() {
        ...
        tableTitle.add("SI ID");
        tableTitle.add("LODGE DT");
        tableTitle.add("SI START DT");
        tableTitle.add("SI END DT");
        tableTitle.add("DEBIT ACT");
        tableTitle.add("DEBIT ACT BAL");
        tableTitle.add("CREDIT ACT");
        tableTitle.add("DEBIT AMT");
        tableTitle.add("LAST RUN DT");
        tableTitle.add("NEXT RUN DT");
    ...
    //        };
    //
    //        tbmCreditSI = new EnhUancedTableModel(
    //        new Object [][] {
    //        },
    //        new String [] {
    //            siRB.getString("tblColounHeading1"), siRB.getString("tblColounHeading2"), siRB.getString("tblColounHeading3"), siRB.getString("tblColounHeading4"), siRB.getString("tblColounHeading5")
    ...
    public static StandingInstructionDailyOB getInstance() {
        return standingInstructionDailyOB;
    }
     ...
     *
     * @param rdoHolidayExecution_Yes New value of property
     * rdoHolidayExecution_Yes.
     */
     ...
     *
     * @param rdoHolidayExecution_No New value of property
     * rdoHolidayExecution_No.
     */
...
//            where.put("BRANCH_CODE", branchCode);
    ...
    /*
     * This method Fires a Query to the Database and 'll returm a HashMap which
     * 'll contain the result Lists.
    /*
     */
...
//                resetForm();
    ...
    void doExecute(ArrayList exeList, String typeOfSI) {
        TTException exception = null;
        try {
            HashMap dataMap = new HashMap();
            if (getCboSIType().equals("Variable")) {
                dataMap.put("EXECUTE_VARIABLE_LIST", exeList);
            } else {
                dataMap.put("EXECUTE_LIST", exeList);
            }
            dataMap.put("TYPE_OF_SI", typeOfSI); //Added By SUresh R  14-Jul-2017
            // Added by nithya on 03-03-2017 for 5763 [ RBI ]
            if (CommonUtil.convertObjToStr(CommonConstants.SAL_REC_MODULE).equals("Y")) {
                if (getStandType().equalsIgnoreCase("TL")) {
                    dataMap.put("LAST_CALC_DT_FOR_SI", getTdtIntCalcDt());
                }else{
                    dataMap.put("LAST_CALC_DT_FOR_SI", curDate.clone());
                }
            } else {
                dataMap.put("LAST_CALC_DT_FOR_SI", curDate.clone());
            }
            ...
            dataMap.put("SERVIC_TAX_REQ",TrueTransactMain.SERVICE_TAX_REQ);// Added by nithya on 20-12-2018 for KD 364 - Gst amount calculation issue at Rd standing
            HashMap proxyResultMap = proxy.execute(dataMap, map);
            setProxyReturnMap(proxyResultMap);
            System.out.println("###### proxy : " + proxyResultMap);
...
//                }
    ...
    /*
     * When an Account no is selected. It'll check if it is already exist in the
     * listDebitSI. If exist it'll set Debit details components with the
     * StandintInstructionDebitTO object.
     *
     */
    ...
    /*
     * When an Account no is selected. It'll check if it is already exist in the
     * listCreditSI. If exist it'll set Credit details components with the
     * StandintInstructionCreditTO object.
     *
     */
    ...
    /*
     * When an Account Head is selected. It'll check if it is already exist in
     * the listDebitSI. If exist it'll set Debit details components with the
     * StandintInstructionDebitTO object.
     *
     */
    ...
    /*
     * When an Account Head is selected. It'll check if it is already exist in
     * the listCreditSI. If exist it'll set Credit details components with the
     * StandintInstructionCreditTO object.
      *
     */
...
//                    setCrdActLst(crdActLst);
    ...
    public ArrayList populateData(HashMap mapID, CTable tblData, String typeOfSI) {
        _tblData = tblData;
        ...
        if (data != null && data.size() > 0) {
            if (getCboSIType().equals("Varible")) {
                for (int i = 0; i < data.size(); i++) {
                    arrList = (ArrayList) data.get(i);
                    arrList.add(0, new Boolean(false));
                    if (CommonUtil.convertObjToStr(typeOfSI).equals("RECURRING_VARIABLE")) { //Added By Suresh R  19-Jul-2017
                        newDataList.add(arrList);
                    } else {
                        HashMap whrmap = new HashMap();
                        HashMap hash = null;
                        whrmap.put("SI_ID", CommonUtil.convertObjToStr(arrList.get(1)));
                        whrmap.put("BRANCH_CODE", ProxyParameters.BRANCH_ID);
                        List lst = ClientUtil.executeQuery("getStandingInstalmentNumber", whrmap);
                        if (lst != null && lst.size() > 0) {
                            hash = (HashMap) lst.get(0);
                            if (hash != null && hash.containsKey("PENDING_INST_YN") && CommonUtil.convertObjToStr(hash.get("PENDING_INST_YN")).equals("Y")) {
                                HashMap whereMap1 = new HashMap();
                                whereMap1.put("ACT_NUM", CommonUtil.convertObjToStr(arrList.get(6)));
                                Date currntDate = DateUtil.addDays(curDate, -1);
                                whereMap1.put("CURR_DATE", CommonUtil.getProperDate(curDate, currntDate));
                                List totLoanBalList = ClientUtil.executeQuery("getPrincipalDueDetails", whereMap1);
    ...
    public void setSelectAll(Boolean selected, int count) {
        //System.out.println("Selected@@@2" + selected);
        //System.out.println("tblData.getRowCount()" + count);
        for (int i = 0, j = count; i < j; i++) {
            tblData.setValueAt(selected, i, 0);
        }
    }
    ...
    public void insertTableData(HashMap whereMap) {
        try {
            ...
            List standInsList = ClientUtil.executeQuery("getNormalSiForReExecution", whereMap);
            if (standInsList != null && standInsList.size() > 0) {
                for (int i = 0; i < standInsList.size(); i++) {
                    ...
                    rowList.add(dataMap.get("SI_ID"));
                    rowList.add(dataMap.get("LODGE_DT"));
                    rowList.add(dataMap.get("SI_START_DT"));
                    rowList.add(dataMap.get("SI_END_DT"));
                    rowList.add(dataMap.get("DEBIT_ACT"));
                    rowList.add(CurrencyValidation.formatCrore(CommonUtil.convertObjToStr(dataMap.get("DEBIT_ACT_BAL"))));
                    rowList.add(dataMap.get("CREDIT_ACT"));
                    rowList.add(CurrencyValidation.formatCrore(CommonUtil.convertObjToStr(dataMap.get("DEBIT_AMT"))));
                    rowList.add(dataMap.get("LAST_RUN_DT"));
                    rowList.add(dataMap.get("NEXT_RUN_DT"));
                    tableList.add(rowList);
                    //setFinalList(standInsList);
                    tblData = new EnhancedTableModel((ArrayList) tableList, tableTitle);
                    tblData.isCellEditable(i, 0);
                }
...
//        setCboExecConfig("");
    ...
    public void getDebitProductIdByType(String prodType) throws Exception {//String productType)
        /**
         * The data to be show in Combo Box other than LOOKUP_MASTER table Show
         * Product Id
         */
     ...
     *
     * @param lbl1HolidayExecutonDate New value of property
     * lbl1HolidayExecutonDate.
     */
