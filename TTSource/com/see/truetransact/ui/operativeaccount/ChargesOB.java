...
public class ChargesOB extends CObservable {
    ...
    private static ChargesOB chargesOB;
    
    static {
        try {
            chargesOB = new ChargesOB();
        } catch(Exception e) {
    ...
    public ChargesOB() throws Exception {
    ...
    private void setTabTitle() throws Exception{
        ...
        tabTitle.add(objChargesRB.getString("tblColumn1"));
        tabTitle.add(objChargesRB.getString("tblColumn2"));
        tabTitle.add(objChargesRB.getString("tblColumn3"));
        tabTitle.add(objChargesRB.getString("tblColumn4"));
    }
...
//        cbmProductId = new ComboBoxModel(key,value);
    ...
    public void setupInit() {
        taskMap = new HashMap();
        taskMap.put("chkChequeBookIssueCharges", "");
        taskMap.put("chkChequeReturnCharges", "");
        taskMap.put("chkExcessTransactionCharges", "ExcessTransChrgesTask");
        taskMap.put("chkFolioCharges", "");
        taskMap.put("chkInoperativeCharges", "InOperativeChargesTask");
        taskMap.put("chkMiscCharges", "");
        taskMap.put("chkNonMaintenanceOFMinBalCharges", "MinBalanceChargesTask");
        taskMap.put("chkStatementCharges", "");
        taskMap.put("chkStopPaymentCharges", "");
    ...
    /** Called from UI, Set the tasks selected in to the arrayList, which r sent tom thread pool...*/
    public boolean process(){
        if (!taskRunning){
            ...
            if (chkChequeBookIssueCharges){
                setTabTask("chkChequeBookIssueCharges");
            }
            if (chkChequeReturnCharges){
                setTabTask("chkChequeReturnCharges");
            }
            if (chkExcessTransactionCharges){
                startEachTask("chkExcessTransactionCharges");
            }
            if (chkFolioCharges){
                setTabTask("chkFolioCharges");
            }
            if (chkInoperativeCharges){
                startEachTask("chkInoperativeCharges");
            }
            if (chkMiscCharges){
                setTabTask("chkMiscCharges");
            }
            if (chkNonMaintenanceOFMinBalCharges){
                startEachTask("chkNonMaintenanceOFMinBalCharges");
            }
            if (chkStatementCharges){
                setTabTask("chkStatementCharges");
            }
            if (chkStopPaymentCharges){
                setTabTask("chkStopPaymentCharges");
            }
            
            threadPool.setTaskList(dataList);
    ...
    /** For the mock processes...
     * Just updates the Status in the Table
     */
    private void setTabTask(String chkBox){
        ArrayList exeTask = new ArrayList();
        exeTask.add(objChargesRB.getString(chkBox));   //Task
        exeTask.add("0");               //Actual count
        exeTask.add("0");               //Execution count
        exeTask.add("COMPLETED");         //Status
        tblLog.addRow(exeTask);
    }
    
    /** The Methosd to start the execution of all the task, which r selected */
    private void startEachTask(String chkBox){
        ArrayList exeTask = new ArrayList();
        exeTask.add(objChargesRB.getString(chkBox));
        exeTask.add("0");               //Actual count
        exeTask.add("0");               //Execution count
        exeTask.add("START");             //Status
        
        tblLog.addRow(exeTask);
        dataList.add(getTaskFromMap(chkBox));
        ++exeTaskCountUI;
    }
    
    /** To get the HEader from the Tash HashMap which contain all the
     * Task to be performed in Key-Value Format...*/
    private TaskHeader getTaskFromMap(String value) {
        System.out.println ("value" + value);
        System.out.println ("taskMap" + CommonUtil.convertObjToStr(taskMap.get(value)));
    ...
    public void updateChkBoxTableBasedOnStatus(){
        if (tskHeader.getTaskClass().equals("SelectSBInoperativeBalance")){
            updateTaskStatusInLogTable("chkInoperativeCharges");
        }
    }
    
    private void updateTaskStatusInLogTable(String chkBox){
        ArrayList exeTask = new ArrayList();
        exeTask.add(CommonUtil.convertObjToStr(String.valueOf(tskStatus.getActualCount())));
        exeTask.add(CommonUtil.convertObjToStr(String.valueOf(tskStatus.getExecutionCount())));
        if (tskStatus.getStatus() == 1){
            exeTask.add("ACCEPTED");
        }else if (tskStatus.getStatus() == 2){
            exeTask.add("REJECTED");
        ...
        ArrayList exeAllTasks = tblLog.getDataArrayList();
        ArrayList task;
        for (int i = exeAllTasks.size() - 1,j = 0;i >= 0;--i,++j){
            task = (ArrayList) exeAllTasks.get(j);
            // Update the data for the particular Task in the log table
            if (task.get(0).equals(objChargesRB.getString(chkBox))){
                tblLog.setValueAt(exeTask.get(0), j, 1);
                tblLog.setValueAt(exeTask.get(1), j, 2);
                tblLog.setValueAt(exeTask.get(2), j, 3);
                // If all the tasks are finished, Stop the Process....
                if (getExeTaskCount() == exeTaskCountUI){
                    taskRunning = false;
                    threadPool.stop();
                }
                break;
