 ...
 * @author balachandar
 */
public class KoleFieldKCCChargePostingUI extends com.see.truetransact.uicomponent.CInternalFrame implements Observer, ListSelectionListener {
//    private final AuthorizeRB resourceBundle = new AuthorizeRB();

    private KoleFieldKCCChargePostingOB observable;
     ...
     */
    public KoleFieldKCCChargePostingUI() {
        setupInit();
        setupScreen();
    }
     ...
     * Creates new form AuthorizeUI
     */
    public KoleFieldKCCChargePostingUI(CInternalFrame parent, HashMap paramMap) {
        this.parent = parent;
...
//        setModal(true);
    ...
    private void setObservable() {
        try {
            observable = new KoleFieldKCCChargePostingOB();
...
//            }
        ...
        if (tblData.getRowCount() > 0) {
            for (int i = 0; i < tblData.getRowCount(); i++) {
                ...
                if (CommonUtil.convertObjToStr(tblData.getValueAt(i, 0)).equals("true")) {
                    ...
                    totInvAmt = totInvAmt + CommonUtil.convertObjToDouble(tblData.getValueAt(i, 8).toString()).doubleValue();
    ...
    private void initComponents() {
        ...
        chkDebitLoanType = new com.see.truetransact.uicomponent.CCheckBox();
        chkPostCharges = new com.see.truetransact.uicomponent.CCheckBox();
        ...
        chkDebitLoanType.setText("Debit Loan Type");
        chkDebitLoanType.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                chkDebitLoanTypeActionPerformed(evt);
            }
        });
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 1;
        gridBagConstraints.gridy = 6;
        panMultiSearch1.add(chkDebitLoanType, gridBagConstraints);

        chkPostCharges.setText("Post Charges");
        chkPostCharges.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                chkPostChargesActionPerformed(evt);
            }
        });
        gridBagConstraints = new java.awt.GridBagConstraints();
        gridBagConstraints.gridx = 0;
        gridBagConstraints.gridy = 6;
        panMultiSearch1.add(chkPostCharges, gridBagConstraints);
...
//        String prodType = String.valueOf(cboProdType.getSelectedItem());
        try {
            ...
            if(!chkDebitLoanType.isSelected()){
             ...
             for (int i = 0; i < tblData.getRowCount(); i++) {
                    if (((Boolean) tblData.getValueAt(i, 0)).booleanValue()) {
                        System.out.println("CommonUtil.convertObjToDouble(tblData.getValueAt(i, 8))^#^#^#^#^"+CommonUtil.convertObjToDouble(tblData.getValueAt(i, 8)));
                      if(CommonUtil.convertObjToDouble(tblData.getValueAt(i, 8))<=0){
                         chk = true; 
                      } 
                   }
             }             
            ...
            if (tblData.getRowCount() > 0) {
                for (int i = 0; i < tblData.getRowCount(); i++) {
                    ...
                    if (((Boolean) tblData.getValueAt(i, 0)).booleanValue()) {
                        ...
                        riskFindTot = riskFindTot + CommonUtil.convertObjToDouble(tblData.getValueAt(i, 8));
    ...
    public void updateOBFields() {
        ...
        if (!chkDebitLoanType.isSelected() && !(((ComboBoxModel) cboProdTypeCr.getModel()).getKeyForSelected().toString()).equals("GL")) {
        ...
        if(chkDebitLoanType.isSelected()){
            observable.setChkDebitLoanType("Y");
        }else{
            observable.setChkDebitLoanType("N");
        }
        
        if(chkPostCharges.isSelected()){
            observable.setChkPostCharges("Y");
        }else{
            observable.setChkPostCharges("N");
        }
...
//        }
    ...
    public void LoanChargeTableData(HashMap map) {
        ...
        tblData.setModel(new javax.swing.table.DefaultTableModel(
                ...
                new String[]{
                    "Select", "Account No", "Loan Type", "Membership No", "Name", "Principle Due", "Balance","Land(in Cent)", "Charge Amount","Narration"}) {
...
//                }
    ...
    private Object[][] setLoanChargeTableData(HashMap map) {
        ...
        if (whereMap.containsKey("BEHAVES_LIKE")) {
            observable.setBehavesLike(CommonUtil.convertObjToStr(whereMap.get("BEHAVES_LIKE")));
            if (whereMap.get("BEHAVES_LIKE").equals("OD")) {
              loanList = ClientUtil.executeQuery("getAccountsForKoleFieldKCC", whereMap); 
            } else {
                loanList = ClientUtil.executeQuery("getAccountsForKoleFieldLoans", whereMap);
            }
        }           
        if (loanList != null && loanList.size() > 0) {
            ...
            for (int j = 0; j < loanList.size(); j++) {
                ...
                totalList[j][7] = CommonUtil.convertObjToStr(CommonUtil.convertObjToDouble(dataMap.get("LAND_AREA")));
                totalList[j][8] = CommonUtil.convertObjToStr("0");
            }
            observable.setFinalList(loanList);
            return totalList;
        } else {
    ...
    private void btnProcessActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnProcessActionPerformed
        ...
        try {
            ...
            if(cboProdId.getSelectedItem().equals("")) {
             ...
             if(!chkPostCharges.isSelected() &&  !chkDebitLoanType.isSelected()) {
                ClientUtil.showMessageWindow("Please select posting type!!!");
                return;
            }
...
//            System.out.println("mapRisk "+mapRisk);
            ...
            if (CommonUtil.convertObjToInt(txtNoOfInstallments.getText()) > 0) {
                mapRisk.put("NO_OF_INSTALLMENTS", txtNoOfInstallments.getText());
            }
            if (!observable.getCbmbranch().getKeyForSelected().equals("") && observable.getCbmbranch().getKeyForSelected()!=null) {
...
// TODO add your handling code here:
    ...
    private void txtChrgAmtFocusLost(java.awt.event.FocusEvent evt) {//GEN-FIRST:event_txtChrgAmtFocusLost
        ...
        for(int i=0; i<tblData.getRowCount(); i++){
          double landArea = CommonUtil.convertObjToDouble(tblData.getValueAt(i,7));
          double landcharge = (landArea/100)*chargeAmt; 
          tblData.setValueAt(landcharge, i, 8);  
        }
    ...
    private void chkDebitLoanTypeActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_chkDebitLoanTypeActionPerformed
        // TODO add your handling code here:
        if(chkDebitLoanType.isSelected()){
            chkPostCharges.setSelected(false);
        }else{
            chkPostCharges.setSelected(true);
        }
    }//GEN-LAST:event_chkDebitLoanTypeActionPerformed

    private void chkPostChargesActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_chkPostChargesActionPerformed
        // TODO add your handling code here:
        if(chkPostCharges.isSelected()){
            chkDebitLoanType.setSelected(false);
        }else{
            chkDebitLoanType.setSelected(true);
        }
    }//GEN-LAST:event_chkPostChargesActionPerformed
...
//        authorizeUI.show();
    ...
    private com.see.truetransact.uicomponent.CCheckBox chkDebitLoanType;
    private com.see.truetransact.uicomponent.CCheckBox chkFulldue;
    private com.see.truetransact.uicomponent.CCheckBox chkPostCharges;
