...
public class MDSBankAdvanceUI extends com.see.truetransact.uicomponent.CInternalFrame implements Observer, UIMandatoryField{
    private final static ClientParseException parseException = ClientParseException.getInstance();
    MDSBankAdvanceOB observable = null;
    ...
    private void settingupUI(){
        setFieldNames();
        internationalize();
        observable = new MDSBankAdvanceOB();
        setMandatoryHashMap();
        initTableData();
        setMaximumLength();
        setHelpMessage();
   ...
   object. Help display Label name should be lblMsg. */
    ...
    private void updateAuthorizeStatus(String authorizeStatus) {
        if (viewType == AUTHORIZE && isFilled){
            ...
            whereMap.put("BRANCH_CODE",ProxyParameters.BRANCH_ID);//Added by nithya on 18-08-2020 for KD-2162
            mapParam.put(CommonConstants.MAP_WHERE, whereMap);
            whereMap = null;
            mapParam.put(CommonConstants.MAP_NAME, "getBankAdvanceAuthorize");
            isFilled = true;
    ...
    public void authorize(HashMap map,String id) {
        ...
        if (map.get(CommonConstants.AUTHORIZEDATA) != null) {
            ...
            if(observable.getResult() != ClientConstants.ACTIONTYPE_FAILED){
               if (fromAuthorizeUI) {// Added by nithya on 18-08-2020 for KD-2162
                    authorizeListUI.removeSelectedRow();
                    this.dispose();
                    authorizeListUI.setFocusToTable();
                    authorizeListUI.displayDetails("MDS Bank Advance");
                }
               if (fromNewAuthorizeUI) {// Added by nithya on 18-08-2020 for KD-2162
                    newauthorizeListUI.removeSelectedRow();
                    this.dispose();
                    newauthorizeListUI.setFocusToTable();
                    newauthorizeListUI.displayDetails("MDS Bank Advance");
                } 
            }
    ...
    public void fillData(Object  map) {
        try {
            ...
            if (hash.containsKey("FROM_AUTHORIZE_LIST_UI")) { // Added by nithya on 18-08-2020 for KD-2162
                isFilled = true;
            ...
            if (hash.containsKey("NEW_FROM_AUTHORIZE_LIST_UI")) { // Added by nithya on 18-08-2020 for KD-2162
                isFilled = true;
    ...
    public static void main(String[] arg){
        ...
        MDSBankAdvanceUI gui = new MDSBankAdvanceUI();
