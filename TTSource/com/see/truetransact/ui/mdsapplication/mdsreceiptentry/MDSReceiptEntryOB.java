...
public class MDSReceiptEntryOB extends CObservable {
    ...
    private static MDSReceiptEntryOB MDSReceiptEntryOB;
    private ArrayList bonusList = null;

    static {
        try {
            MDSReceiptEntryOB = new MDSReceiptEntryOB();
        } catch (Exception e) {
    ...
    public MDSReceiptEntryOB() throws Exception {
        operationMap = new HashMap();
        operationMap.put(CommonConstants.JNDI, "MDSReceiptEntryJNDI");
    ...
    public static MDSReceiptEntryOB getInstance() {
    ...
    public void doAction() {
        try {
            ...
            e.printStackTrace();
            if (getActionType() == ClientConstants.ACTIONTYPE_AUTHORIZE && e instanceof TTException) { // Added by nithya on 14-10-2020 for KD-2348
                parseException.logException(e, true);          
            }
        }
    }
    ...
    private MDSReceiptEntryTO setMDSReceiptEntryTO() {
        ...
        mdsReceiptEntryTO.setChitStartDt(getProperDateFormat(getTdtChitStartDt()));
        mdsReceiptEntryTO.setChitEndDt(getProperDateFormat(getTdtChitEndDt()));
        ...
        mdsReceiptEntryTO.setPaidDate(getProperDateFormat(getTdtPaidDate()));
...
//        }
        ...
        mdsReceiptEntryTO.setChangedDt(getProperDateFormat(getTdtChangedDate()));
        ...
        mdsReceiptEntryTO.setForfeitBonusAmtPayable(CommonUtil.convertObjToDouble(getTxtForfietbonus())); //16-07-2020
...
//            int instDay = 0; //Added by Rajesh
            ...
            if (instLst != null && instLst.size() > 0) {
                ...
                //System.out.println("@#@#@#@#@#@#@ paid installment : "+getTxtPaidNoOfInst());
                // Added by Rajesh
...
//                setTdtInsUptoPaid(CommonUtil.convertObjToStr(startDate));
            ...
            prizedMap.put("SCHEME_NAME", getTxtSchemeName());//FROM_AUCTION_ENTRY=N, AFTER_CASH_PAYMENT=Y
            prizedMap.put("DIVISION_NO", chittalMap.get("DIVISION_NO"));
...
//            System.out.println("#@#@#@#@#@# stYear : "+stYear);
//            System.out.println("#@#@#@#@#@# currYear : "+currYear);
//            System.out.println("#@#@#@#@#@# stMonth : "+stMonth);
...
//                    setTxtCurrentInstNo(String.valueOf(value+1));
    ...
    public boolean populateData(HashMap whereMap) {
        isSplitMDSTransaction = "";
        String chitlNo = CommonUtil.convertObjToStr(getTxtChittalNo());
        whereMap.put("CHITTAL_NO", chitlNo);
        whereMap.put("SCHEME_NAME", CommonUtil.convertObjToStr(whereMap.get("SCHEME_NAME")));
...
//                        mdsReceiptEntryTO = new MDSReceiptEntryTO();
                        for (int i = 0; i < lst.size(); i++) {
                            mdsReceiptEntryTO = (MDSReceiptEntryTO) lst.get(i);
            ...
            mdsReceiptEntryTO = (MDSReceiptEntryTO) ((List) mapData.get("MDSReceiptEntryTO")).get(0);
            getMDSReceiptEntryTO(mdsReceiptEntryTO);    
            }
    ...
    private MDSReceiptEntryTO getMDSReceiptEntryTO(MDSReceiptEntryTO mdsReceiptEntryTO) {
        //        MDSReceiptEntryTO mdsReceiptEntryTO = new MDSReceiptEntryTO();
        setTxtSchemeName(mdsReceiptEntryTO.getSchemeName());
        ...
        setLblServiceTaxval(CommonUtil.convertObjToStr(mdsReceiptEntryTO.getServiceTaxAmt())); // Added by nithya on 21-11-2019 for KD-783
        }
        ...
        setTxtForfietbonus(CommonUtil.convertObjToStr(mdsReceiptEntryTO.getForfeitBonusAmtPayable())); //2093
