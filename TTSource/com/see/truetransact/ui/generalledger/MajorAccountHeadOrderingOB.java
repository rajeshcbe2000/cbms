 ...
 * @author bala
 */
public class MajorAccountHeadOrderingOB extends CObservable {
    ...
    private static MajorAccountHeadOrderingOB genLedgerOB;
    ...
    static {
        try {
            ...
            genLedgerOB = new MajorAccountHeadOrderingOB();
        } catch (Exception e) {
        }
    }

    public static MajorAccountHeadOrderingOB getInstance() {
        return genLedgerOB;
    }
     ...
     */
    public MajorAccountHeadOrderingOB() throws Exception {
        initiate();
    }
...
//    }
    ...
    public ArrayList populateData(HashMap mapID, CTable tblData) {
        ...
        System.out.println("Map Name : " + mapID.get(CommonConstants.MAP_NAME));
        System.out.println("Map      : " + mapID);
        List glList = ClientUtil.executeQuery(CommonUtil.convertObjToStr(mapID.get(CommonConstants.MAP_NAME)),null);
        _heading = new ArrayList();
        _heading.add("SL NO");
        _heading.add("TYPE");
        _heading.add("DESCRIPTION");
        _heading.add("MAJOR HEAD ID");
        _heading.add("NEW ORDER");
        data = new ArrayList();
        if (glList != null && glList.size() > 0) {
            for (int i = 0; i < glList.size(); i++) {
                //M.MJR_AC_HD_DESC,M.MJR_AC_HD_ID,M.MJR_AC_ORDER FROM MJR_AC_HD
                ArrayList arrList = new ArrayList();
                HashMap map = (HashMap)glList.get(i);
                arrList.add(CommonUtil.convertObjToStr(map.get("SLSNO")));
                arrList.add(CommonUtil.convertObjToStr(map.get("MJR_AC_HD_TYPE")));
                arrList.add(CommonUtil.convertObjToStr(map.get("MJR_AC_HD_DESC")));
                arrList.add(CommonUtil.convertObjToStr(map.get("MJR_AC_HD_ID")));
