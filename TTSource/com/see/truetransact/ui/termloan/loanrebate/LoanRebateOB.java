 ...
 * @author bala
 */
public class LoanRebateOB extends CObservable {
    ...
    private String rebateUpto = "";
    private String asAnWhenCustomer = "";
    private String prodId = "";
    private boolean checkClosedLoan = false;
    ...
    public LoanRebateOB() {
        try {
            ...
            map = new HashMap();
            map.put(CommonConstants.JNDI, "LoanRebateJNDI");
            map.put(CommonConstants.HOME, "termloan.loanrebate.LoanRebateHome");
            map.put(CommonConstants.REMOTE, "termloan.loanrebate.LoanRebate");
    ...
    public void doActionPerformedAuthorize(ArrayList list) {
        try {
            ArrayList rebatelist = new ArrayList();
            HashMap hash = new HashMap();
            LoanRebateTO loanRebateTO = null;
            System.out.println("list is" + list);
            if (list.size() > 0 && list != null) {
                for (int i = 0; i < list.size(); i++) {
                    loanRebateTO = new LoanRebateTO();
                    hash = (HashMap) list.get(i);
                    loanRebateTO.setAccNo(CommonUtil.convertObjToStr(hash.get("ACC_NUM")));
                    loanRebateTO.setIntAmount(CommonUtil.convertObjToStr(hash.get("AMOUNT")));
                    loanRebateTO.setNormalInterest(CommonUtil.convertObjToStr(hash.get("NORMALINTEREST")));
                    loanRebateTO.setRebateId(getRebateID());
                    loanRebateTO.setStatusBy(TrueTransactMain.USER_ID);
                    loanRebateTO.setStatusDt(ClientUtil.getCurrentDate());
                    loanRebateTO.setBranchCode(ProxyParameters.BRANCH_ID);
                    loanRebateTO.setRebateUpto(getProperDateFormat(hash.get("REBATE_UPTO")));
                    rebatelist.add(loanRebateTO);
                }
            }
            hash.put("ACCOUNTLIST", rebatelist);
            hash.put("COMMAND", "AUTHORIZE");
            hash.put("PROD_ID", "AUTHORIZE");
            hash = proxy.execute(hash, map);
...
//        loanRebateTO. setProdId(CommonUtil.convertObjToStr(getCbmProdId().getKeyForSelected()));
        //                    loanRebateTO. setProdId(CommonUtil.convertObjToStr((ComboBoxModel)getCbmProdId().getKeyForSelected()));
        loanRebateTO.setProdType(getProdType());
        loanRebateTO.setFromAccNo(getFromAccNo());
        loanRebateTO.setToAccNo(getToAccNo());
        loanRebateTO.setProdId(getCboProdId());
        loanRebateTO.setRebateUpto(DateUtil.getDateMMDDYYYY(getRebateUpto()));
        if (getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
            loanRebateTO.setStatus(CommonConstants.STATUS_CREATED);
        } else if (getCommand().equals(CommonConstants.TOSTATUS_UPDATE)) {
            loanRebateTO.setStatus(CommonConstants.STATUS_MODIFIED);
            loanRebateTO.setRebateId(getRebateID());

        } else {
            loanRebateTO.setStatus(CommonConstants.STATUS_DELETED);
            loanRebateTO.setRebateId(getRebateID());
        }
        loanRebateTO.setStatusBy(TrueTransactMain.USER_ID);
        loanRebateTO.setStatusDt(ClientUtil.getCurrentDate());
        loanRebateTO.setBranchCode(ProxyParameters.BRANCH_ID);
        masterList.add(loanRebateTO);
        return masterList;
    }

    public void doActionPerformed(ArrayList list) {
        try {
            ArrayList rebatelist = new ArrayList();
            HashMap hash = new HashMap();
            ...
            if (list.size() > 0 && list != null) {
                for (int i = 0; i < list.size(); i++) {
                    loanRebateTO = new LoanRebateTO();
                    hash = (HashMap) list.get(i);
                    Date rbdate = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(hash.get("REBATE_UPTO")));
                    Date rebateupto = (Date) currDt.clone();
                    rebateupto.setDate(rbdate.getDate());
                    rebateupto.setMonth(rbdate.getMonth());
                    rebateupto.setYear(rbdate.getYear());
                    loanRebateTO.setAccNo(CommonUtil.convertObjToStr(hash.get("ACCT_NUM")));
                    loanRebateTO.setAccName(CommonUtil.convertObjToStr(hash.get("ACCT_NAME")));
                    loanRebateTO.setRebateInt(CommonUtil.convertObjToStr(hash.get("REBATE_INTEREST")));
                    loanRebateTO.setIntAmount(CommonUtil.convertObjToStr(hash.get("INTEREST_AMOUNT")));
                    //          loanRebateTO.setProdId(CommonUtil.convertObjToStr(hash.get("PROD_ID")));
                    if (getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
                        loanRebateTO.setStatus(CommonConstants.STATUS_CREATED);
                    } else if (getCommand().equals(CommonConstants.TOSTATUS_UPDATE)) {
                        loanRebateTO.setStatus(CommonConstants.STATUS_MODIFIED);
                        loanRebateTO.setRebateId(getRebateID());
                        loanRebateTO.setSelect(CommonUtil.convertObjToStr(hash.get("SELECT")));
                    } else {
                        loanRebateTO.setStatus(CommonConstants.STATUS_DELETED);
                        loanRebateTO.setRebateId(getRebateID());
                    }
                    ...
                    loanRebateTO.setStatusDt(ClientUtil.getCurrentDate());
                    loanRebateTO.setBranchCode(ProxyParameters.BRANCH_ID);
                    loanRebateTO.setRebateUpto(rebateupto);
                    rebatelist.add(loanRebateTO);

                }
                System.out.println("loanRebateTO^$^$^$^$^$^$^" + loanRebateTO);
                System.out.println("loanRebateTOgetNormalInterest^$^$^$^$^$^$^" + loanRebateTO.getNormalInterest());
                System.out.println("rebatelist^$^$^$^$^$^$^" + rebatelist);
                hash.put("ACCOUNTLIST", rebatelist);
                hash.put("COMMAND", getCommand());
                hash.put("MASTERLIST", getMasterList());
                if (isCheckClosedLoan()) {
                    hash.put("CLOSED_LOAN_TRANSACTION", "CLOSED_LOAN_TRANSACTION");
                }
                hmap = proxy.execute(hash, map);
                String id = CommonUtil.convertObjToStr(hmap.get("REBATEID"));
                System.out.println("^^^^^^id is" + id);
                if (id.length() > 0 && getCommand().equals(CommonConstants.TOSTATUS_INSERT)) {
                    ClientUtil.showMessageWindow("LoanRebate ID:" + id);
                }
                setResult(getActionType());
            } else {
                ClientUtil.displayAlert("Select Atleast one record");
            }
            hash = null;
            rebatelist = null;
            list = null;
    ...
    public boolean checkClosedLoan(String accountNo) {
        ...
        List checkList = ClientUtil.executeQuery("getClosedLoanList", checkMap);
    ...
    public void setSelectAll(CTable table, Boolean selected) {
        ...
        for (int i = 0, j = table.getRowCount(); i < j; i++) {
            ...
            if (checkClosedLoan(CommonUtil.convertObjToStr(table.getValueAt(i, 1)))) {
                accountNo.append(CommonUtil.convertObjToStr(table.getValueAt(i, 1)));
                accountNo.append(",");
            }
        }
        System.out.println("accountNoX%$%$$^" + accountNo);
        if (selected) {
            if (accountNo.length() > 0) {
                ClientUtil.showMessageWindow("Following accounts are closed or inactive :" + accountNo);
            }
        }
    }
...
//            isSelectedRecord =(Boolean)tblData.getValueAt(i,0);
    ...
    public ArrayList getTableModel(CTable table) {
        ...
        for (int i = 0; i < v.size(); i++) {
            ...
            if (new Boolean(CommonUtil.convertObjToStr(lst.get(0))).booleanValue()) {
                ...
                totList.add(list);
            } else if (getActionType() == ClientConstants.ACTIONTYPE_EDIT && CommonUtil.convertObjToStr(lst.get(3)).length() > 0) {
                list = new ArrayList();
                list.add(lst.get(0));
                list.add(lst.get(1));
                list.add(lst.get(2));
    ...
    public void setRebateInt(String rebateInt) {
        this.rebateInt = rebateInt;
    }

    public String getRebateInt() {
        return rebateInt;
    }

    public void setIntAmount(String amount) {
        this.amount = amount;
    }
    ...
    public void setRebateUpto(String rebateUpto) {
        this.rebateUpto = rebateUpto;
    }

    public String getRebateUpto() {
        return rebateUpto;
    }

    public void setCboProdId(String prodId) {
        this.prodId = prodId;
    }
    ...
    public boolean isCheckClosedLoan() {
        return checkClosedLoan;
    }

    public void setCheckClosedLoan(boolean checkClosedLoan) {
        this.checkClosedLoan = checkClosedLoan;
    }
}