...
public class GoldLoanRepaymentOB extends CObservable{
    
    /** Creates a new instance of TermLoanRepaymentOB */
//    private GoldLoanRepaymentOB() throws Exception{
//        curDate = ClientUtil.getCurrentDate();
//        termLoanRepaymentOB();
//    }
    
    private       static GoldLoanRepaymentOB termLoanRepaymentOB;
    
    private final static Logger log = Logger.getLogger(GoldLoanRepaymentOB.class);
    ...
    private LinkedHashMap renewalRepaymentAll = new LinkedHashMap(); // Both displayed and hidden values in the table
    ...
    private HashMap repaymentRenewalEachRecord;
    ...
    private String tdtRenewalLastInstall;
    private String tdtRenewalfirstInstall;
...
//    }
    ...
    public GoldLoanRepaymentOB()  throws Exception{
        curDate = ClientUtil.getCurrentDate();
...
//    public static GoldLoanRepaymentOB getInstance() {
...
//    }
    ...
    public void setTermLoanRepaymentTO(ArrayList repaymentList, String acctNum){
        try{
            ...
            for (int i = repaymentList.size() - 1,j = 0;i >= 0;--i,++j){
                ...
                setTxtNoInstall(CommonUtil.convertObjToStr(termLoanRepaymentTO.getNoInstallments()));
                setCboRepayType(CommonUtil.convertObjToStr(getCbmRepayType().getDataForKey(CommonUtil.convertObjToStr(termLoanRepaymentTO.getRepaymentType()))));
                getCbmRepayType().setKeyForSelected(termLoanRepaymentTO.getRepaymentType());
    ...
    public HashMap setTermLoanRepayment(){
        ...
        try{
            ...
            Object[] objKeySet = (Object[]) keySet.toArray();
            HashMap oneRecord;
            System.out.println("#$#$# repaymentAll inside setTermLoanRepayment()... : "+repaymentAll);
            // To set the values for Insurance Transfer Object
            for (int i = keySet.size() - 1,j = 0;i >= 0;--i,++j){
                oneRecord = (HashMap) repaymentAll.get(objKeySet[j]);
                System.out.println("#$#$# oneRecord inside setTermLoanRepayment()... : "+oneRecord);
...
//                termLoanRepaymentTO.setDisbursementDt((Date)oneRecord.get(DISBURSEMENT_DT));
                ...
                if (oneRecord.get(INSTALLMENT_DETAILS) != null){  // && (oneRecord.get(COMMAND).equals(INSERT))  //Condition removed to insert in update mode also
                    ...
                    System.out.println("#$#$# interestRecords inside setTermLoanRepayment()... : "+interestRecords);
                    ...
                    for (int m = noOfInstallment - 1;m >= 0;--m){
                        ...
                        System.out.println("#$#$# interestOneRecord inside setTermLoanRepayment()... : "+interestOneRecord);
...
//                        termLoanInstallmentTO.setInstallmentDt((Date)interestOneRecord.get(INSTALLMENT_DATE));
                        ...
                        System.out.println("#$#$# termLoanInstallmentTO inside setTermLoanRepayment()... : "+termLoanInstallmentTO);
...
//                                termLoanInstallMultIntTO.setToDt((Date)multiIntRec.get(TO_DATE));
                        ...
                        System.out.println("#$#$# installmentMap inside setTermLoanRepayment()... : "+installmentMap);
...
//            }
            ...
            System.out.println("#$#$# repayInstallmentMap inside setTermLoanRepayment() final... : "+repayInstallmentMap);
        }catch(Exception e){
        ...
        public HashMap setTermLoanRenewalRepayment(String newLoanNo,boolean isRenewal){
        HashMap repayInstallmentMap = new HashMap();
        HashMap repaymentMap = new HashMap();
        HashMap installmentMap = new HashMap();
        HashMap installMultIntMap = new HashMap();
        ...
        try{
            ...
            java.util.Set interestKeySet;
            Object[] objInterestKeySet;
            java.util.Set keySet =  renewalRepaymentAll.keySet();
            Object[] objKeySet = (Object[]) keySet.toArray();
            HashMap oneRecord;
            System.out.println("#$#$# renewalRepaymentAll inside setTermLoanRepayment()... : "+renewalRepaymentAll);
            // To set the values for Insurance Transfer Object
            for (int i = keySet.size() - 1,j = 0;i >= 0;--i,++j){
                oneRecord = (HashMap) renewalRepaymentAll.get(objKeySet[j]);
                System.out.println("#$#$# oneRecord inside setTermLoanRepayment()... : "+oneRecord);
                termLoanRepaymentTO = new TermLoanRepaymentTO();
...
//                termLoanRepaymentTO.setDisbursementDt((Date)oneRecord.get(DISBURSEMENT_DT));
                ...
                if (oneRecord.get(INSTALLMENT_DETAILS) != null){  // && (oneRecord.get(COMMAND).equals(INSERT))  //Condition removed to insert in update mode also
                    ...
                    for (int m = noOfInstallment - 1;m >= 0;--m){
                        ...
                        termLoanInstallmentTO.setBalanceAmt(CommonUtil.convertObjToDouble(interestOneRecord.get(BALANCE)));
                        termLoanInstallmentTO.setCommand("INSERT");//CommonUtil.convertObjToStr(interestOneRecord.get(COMMAND)
...
//                        termLoanInstallmentTO.setInstallmentDt((Date)interestOneRecord.get(INSTALLMENT_DATE));
                        ...
                        if (interestOneRecord.containsKey(MULTIPLE_INTEREST)){
                            ...
                            for (int n = installMultiIntList.size()-1;n>=0;--n){
                                multiIntRec = (HashMap) installMultiIntList.get(n);
                                termLoanInstallMultIntTO = new TermLoanInstallMultIntTO();
                                termLoanInstallMultIntTO.setAcctNum("");
                                if (interestOneRecord.get(COMMAND).equals(INSERT)){
                                    termLoanInstallMultIntTO.setCommand(CommonConstants.TOSTATUS_INSERT);
                                }else if (interestOneRecord.get(COMMAND).equals(UPDATE)){
                                    termLoanInstallMultIntTO.setCommand(CommonConstants.TOSTATUS_UPDATE);
...
//                tblRepaymentTab.setDataArrayList(repaymentTabValues, repaymentTabTitle);
    ...
    public int addRepaymentDetails(boolean update){
        int option = -1;
        try{
            log.info("Add Repayment Details 2...");
            repaymentEachTabRecord = new ArrayList();
            repaymentEachRecord = new HashMap();
            HashMap result = new HashMap();
            
            insertRepayment(1);
            System.out.println("After insertRepayment");
            if (!update) {
                // If the table is not in Edit Mode
//                if (repaymentEachRecord.get(ACTIVE_STATUS).equals(YES) && CommonUtil.convertObjToDouble(getStrLimitAmt()).doubleValue() < (getActiveLoanAmt() + CommonUtil.convertObjToDouble(getTxtLaonAmt()).doubleValue())){//commented by abi regarding outstanding amt is greater
//                    repayTabWarning("loanExceedLimitWarning");
...
//                }
                ...
                repaymentEachTabRecord.add(3, repaymentEachTabRecord.get(0));
                repaymentEachRecord.put(REF_SCHEDULE_NUMBER, repaymentEachRecord.get(SLNO));
//                System.out.println("Before tableUtilRepayment.insertTableValues(");
//                result = tableUtilRepayment.insertTableValues(repaymentEachTabRecord, repaymentEachRecord);
//                System.out.println("After tableUtilRepayment.insertTableValues(");
                
//                repaymentTabValues = (ArrayList) result.get(TABLE_VALUES);
                repaymentAll = new LinkedHashMap(); 
                repaymentAll.put(String.valueOf(0), repaymentEachRecord);
                System.out.println("inside addRepaymentDetails : repaymentAll : "+repaymentAll);
//                repaymentAll = (LinkedHashMap) result.get(ALL_VALUES);
//                option = CommonUtil.convertObjToInt(result.get(OPTION));
...
//                tblRepaymentTab.setDataArrayList(repaymentTabValues, repaymentTabTitle);
    ...
    public int addRenewalRepaymentDetails(boolean update ,GoldLoanOB ob){
        int option = -1;
        try{
            log.info("Add Repayment Details 2...");
            repaymentEachTabRecord = new ArrayList();
            repaymentRenewalEachRecord = new HashMap();
            HashMap result = new HashMap();
            
            insertRenewalRepayment(1,ob);
            System.out.println("After insertRepayment");
            if (!update) {
                // If the table is not in Edit Mode
//                if (repaymentEachRecord.get(ACTIVE_STATUS).equals(YES) && CommonUtil.convertObjToDouble(getStrLimitAmt()).doubleValue() < (getActiveLoanAmt() + CommonUtil.convertObjToDouble(getTxtLaonAmt()).doubleValue())){//commented by abi regarding outstanding amt is greater
...
//                repaymentRenewalEachRecord.add(3, repaymentEachTabRecord.get(0));
                repaymentRenewalEachRecord.put(REF_SCHEDULE_NUMBER, repaymentRenewalEachRecord.get(SLNO));
//                System.out.println("Before tableUtilRepayment.insertTableValues(");
//                result = tableUtilRepayment.insertTableValues(repaymentEachTabRecord, repaymentEachRecord);
//                System.out.println("After tableUtilRepayment.insertTableValues(");
                
//                repaymentTabValues = (ArrayList) result.get(TABLE_VALUES);
                renewalRepaymentAll = new LinkedHashMap(); 
                renewalRepaymentAll.put(String.valueOf(0), repaymentRenewalEachRecord);
                System.out.println("inside addRepaymentDetails : renewalRepaymentAll : "+renewalRepaymentAll);
//                repaymentAll = (LinkedHashMap) result.get(ALL_VALUES);
//                option = CommonUtil.convertObjToInt(result.get(OPTION));
                
//                tblRepaymentTab.setDataArrayList(repaymentTabValues, repaymentTabTitle);
                if (repaymentRenewalEachRecord.get(ACTIVE_STATUS).equals(YES)){
                    setActiveLoanAmt(activeLoanAmt+CommonUtil.convertObjToDouble(getTxtLaonAmt()).doubleValue());
                }
            }else{
                option = updateRepaymentTab(1);
            ...
            setChanged();
            repaymentEachTabRecord = null;
            repaymentRenewalEachRecord = null;
            result = null;
        }catch(Exception e){
            log.info("Error in addRepaymentDetails..."+e);
            parseException.logException(e,true);
    ...
    private void insertRepayment(int recordPosition){
        ...
        private void insertRenewalRepayment(int recordPosition ,GoldLoanOB ob){
//        repaymentEachTabRecord.add(String.valueOf(recordPosition));
//        repaymentEachTabRecord.add(getTxtLaonAmt());
//        repaymentEachTabRecord.add(getTxtRepayScheduleMode());
//        repaymentEachTabRecord.add(getStrRefScheduleNumber());
        
        repaymentRenewalEachRecord.put(SLNO, String.valueOf(recordPosition));
        //        repaymentEachRecord.put(PRODUCT_ID, getLblProdID_RS_Disp());
        if (getRdoDoAddSIs_Yes()){
            repaymentRenewalEachRecord.put(ADD_SIS, YES);
        }else if (getRdoDoAddSIs_No()){
            repaymentRenewalEachRecord.put(ADD_SIS, NO);
        }else{
            repaymentRenewalEachRecord.put(ADD_SIS, " ");
        }
        
        if (getRdoPostDatedCheque_Yes()){
            repaymentRenewalEachRecord.put(POST_DT_CHQ_ALLOW, YES);
        }else if(getRdoPostDatedCheque_No()){
            repaymentRenewalEachRecord.put(POST_DT_CHQ_ALLOW, NO);
        }else{
            repaymentRenewalEachRecord.put(POST_DT_CHQ_ALLOW, " ");
        }
        
//        if (getRdoActive_Repayment()){
            repaymentRenewalEachRecord.put(ACTIVE_STATUS, YES);
//        }
//        else if (getRdoInActive_Repayment()){
//            repaymentRenewalEachRecord.put(ACTIVE_STATUS, NO);
//        }else{
//            repaymentRenewalEachRecord.put(ACTIVE_STATUS, " ");
//        }
        repaymentRenewalEachRecord.put(DISBURSEMENT_ID, getStrDisbursementID());
        repaymentRenewalEachRecord.put(SCHEDULE_MODE, getTxtRepayScheduleMode());
        repaymentRenewalEachRecord.put(LOAN_AMT, CommonUtil.convertObjToStr(ob.getTxtRenewalLimit_SD()));
        repaymentRenewalEachRecord.put(REPAY_TYPE,CommonUtil.convertObjToStr(getCbmRepayType().getKeyForSelected()));
        repaymentRenewalEachRecord.put(REPAY_FREQ, CommonUtil.convertObjToStr(ob.getCbmRenewalRepayFreq().getKeyForSelected()));
        repaymentRenewalEachRecord.put(NO_INSTALLMENTS, CommonUtil.convertObjToStr(ob.getTxtRenewalNoInstallments()));
        repaymentRenewalEachRecord.put(FROM_DATE, getTdtRepayFromDate());
        repaymentRenewalEachRecord.put(INSTAL_STDT, getTdtRenewalLastInstall());
        repaymentRenewalEachRecord.put(TO_DATE, getTdtRenewalLastInstall());
        repaymentRenewalEachRecord.put(REPAY_MOROTORIUM, getTxtNoMonthsMora());
        
        repaymentRenewalEachRecord.put(TOT_BASE_AMT, CommonUtil.convertObjToStr(getTxtRenewalLimit_SD()));
        repaymentRenewalEachRecord.put(TOT_INSTALL_AMT, CommonUtil.convertObjToStr(getTxtRenewalLimit_SD()));
        repaymentRenewalEachRecord.put(LAST_INSTALL_AMT, CommonUtil.convertObjToStr(getTxtRenewalLimit_SD()));
        repaymentRenewalEachRecord.put(PENULTI_AMT_INSTALL, CommonUtil.convertObjToStr(getTxtRenewalLimit_SD()));
        repaymentRenewalEachRecord.put(INSTALLMENT_DETAILS, getInstallmentAll());
        System.out.println("FORCHECK"+getInstallmentAll());
        repaymentRenewalEachRecord.put(MORATORIUM_INTEREST, getMoratoriumInterestAll());
        repaymentRenewalEachRecord.put(DISBURSEMENT_DT, getTdtDisbursement_Dt());
        repaymentRenewalEachRecord.put(REF_SCHEDULE_NUMBER, getStrRefScheduleNumber());
        
        repaymentRenewalEachRecord.put(COMMAND, "");
    }
...
//            }
    ...
    /**
     * Getter for property renewalRepaymentAll.
     * @return Value of property renewalRepaymentAll.
     */
    public java.util.LinkedHashMap getRenewalRepaymentAll() {
        return renewalRepaymentAll;
    }
    
    /**
     * Setter for property renewalRepaymentAll.
     * @param renewalRepaymentAll New value of property renewalRepaymentAll.
     */
    public void setRenewalRepaymentAll(java.util.LinkedHashMap renewalRepaymentAll) {
        this.renewalRepaymentAll = renewalRepaymentAll;
    }
    
    /**
     * Getter for property repaymentRenewalEachRecord.
     * @return Value of property repaymentRenewalEachRecord.
     */
    public java.util.HashMap getRepaymentRenewalEachRecord() {
        return repaymentRenewalEachRecord;
    }
    
    /**
     * Setter for property repaymentRenewalEachRecord.
     * @param repaymentRenewalEachRecord New value of property repaymentRenewalEachRecord.
     */
    public void setRepaymentRenewalEachRecord(java.util.HashMap repaymentRenewalEachRecord) {
        this.repaymentRenewalEachRecord = repaymentRenewalEachRecord;
    }
     ...
     * @param tdtRenewalSanctionDate New value of property tdtRenewalSanctionDate.
     */
     ...
     * @param cboRenewalSanctioningAuthority New value of property cboRenewalSanctioningAuthority.
     */
     ...
     * @param cboRenewalPurposeOfLoan New value of property cboRenewalPurposeOfLoan.
     */
     ...
     * @param txtRenewalSanctionRemarks New value of property txtRenewalSanctionRemarks.
     */
     ...
     * @param tdtRenewalAccountOpenDate New value of property tdtRenewalAccountOpenDate.
     */
     ...
     * @param txtRenewalNoInstallments New value of property txtRenewalNoInstallments.
     */
     ...
     * @param tdtRenewalDemandPromNoteExpDate New value of property tdtRenewalDemandPromNoteExpDate.
     */
     ...
     * @param tdtRenewalDemandPromNoteDate New value of property tdtRenewalDemandPromNoteDate.
     */
     ...
     * @param cbmRenewalSanctioningAuthority New value of property cbmRenewalSanctioningAuthority.
     */
     ...
     * @param cbmRenewalPurposeOfLoan New value of property cbmRenewalPurposeOfLoan.
     */
    ...
    /**
     * Getter for property tdtRenewalLastInstall.
     * @return Value of property tdtRenewalLastInstall.
     */
    public java.lang.String getTdtRenewalLastInstall() {
        return tdtRenewalLastInstall;
    }
    
    /**
     * Setter for property tdtRenewalLastInstall.
     * @param tdtRenewalLastInstall New value of property tdtRenewalLastInstall.
     */
    public void setTdtRenewalLastInstall(java.lang.String tdtRenewalLastInstall) {
        this.tdtRenewalLastInstall = tdtRenewalLastInstall;
    }
    
    /**
     * Getter for property tdtRenewalfirstInstall.
     * @return Value of property tdtRenewalfirstInstall.
     */
    public java.lang.String getTdtRenewalfirstInstall() {
        return tdtRenewalfirstInstall;
    }
    
    /**
     * Setter for property tdtRenewalfirstInstall.
     * @param tdtRenewalfirstInstall New value of property tdtRenewalfirstInstall.
     */
    public void setTdtRenewalfirstInstall(java.lang.String tdtRenewalfirstInstall) {
        this.tdtRenewalfirstInstall = tdtRenewalfirstInstall;
    }
