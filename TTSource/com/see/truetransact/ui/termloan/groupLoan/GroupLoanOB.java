...
public class GroupLoanOB extends CObservable{
    ...
    private final static Logger log = Logger.getLogger(GroupLoanOB.class);
...
//            tblSHGDetails= new EnhancedTableModel(null, tableTitle);
    ...
    public void getEditTableDetails(HashMap whereMap){            
        try{
            ...
            List editList = ClientUtil.executeQuery("getGroupTransAuthorize", whereMap);
            ...
            if(editList!=null && editList.size()>0){
                for (int i = 0;i<editList.size();i++){
                    ...
                    rowList.add(dataMap.get("PRINCIPLE"));
                    rowList.add(dataMap.get("INTEREST"));
                    rowList.add(dataMap.get("PENAL"));
                    rowList.add(dataMap.get("CHARGES"));
                    //rowList.add(dataMap.get("TOT_DUE"));
                    princ_Due = CommonUtil.convertObjToDouble(dataMap.get("PRINCIPLE")).doubleValue();
                    ...
                    rowList.add(dataMap.get("RECEIPT"));
...
//        }
        if(isDailyGrpLoan()){
            data.put("DAILY_GROUP_LOAN_DATA",getFinalList());
            //data.put("GROUP_ID",getTxtGroupId());
        }else{
            data.put("SHG_TABLE_DATA",getFinalList());
        }
