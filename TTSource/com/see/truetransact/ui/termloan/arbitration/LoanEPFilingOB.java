 ...
 * @author bala
 */
public class LoanEPFilingOB extends CObservable {
...
//    private ComboBoxModel cbmNoticeType;
    ...
    public HashMap insertCharges(HashMap whereMap) {
        ...
        obj.put("EP_FILING", "EP_FILING");
        if (whereMap.get("EP_LIST") != null) {
            obj.put("EP_LIST", whereMap.get("EP_LIST"));
            obj.put("FILING_DT", whereMap.get("FILING_DT"));
...
//                selectedList.add(_tableModel.getValueAt(i, 1));
          ...
          if(accOpMap.size()>0 &&  accOpMap.get(0)!=null)
            {
             ...
             if(accOpMap.get(0)!=null)
                charge=CommonUtil.convertObjToStr(mapop.get("CHARGE"));
            }
    ...
    public ArrayList populateData(HashMap mapID, CTable tblData) {
        String epWithPresentValues = "N";//KD-3449
        if(TrueTransactMain.CBMSPARAMETERS.containsKey("LOAN_EP_WITH_CURRENT_LOAN_STATUS") && TrueTransactMain.CBMSPARAMETERS.get("LOAN_EP_WITH_CURRENT_LOAN_STATUS") != null && CommonUtil.convertObjToStr(TrueTransactMain.CBMSPARAMETERS.get("LOAN_EP_WITH_CURRENT_LOAN_STATUS")).equals("Y")){
         epWithPresentValues = "Y";
        }        
        ...
        System.out.println("Screen   : filingOB");
...
//            }
            ...
            _heading.add("NAME");
            _heading.add("PRINC_DUE");
            _heading.add("INT_DUE");
            _heading.add("PENAL");
            _heading.add("CHARGES");
            _heading.add("ARC_FEE");
            _heading.add("INSP_ORD_DATE");
            _heading.add("ATTACH_FEE");
            _heading.add("SALES_FEE");
            _heading.add("POSTAGE_CHARGE");
            _heading.add("EP_FEE");
            _heading.add("TOT_EP_AMOUNT");
            _heading.add("FILE_NO");
            _heading.add("EP_ID");
            _heading.add("EP NO");
        }
...
//        System.out.println("lst"+list);
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            System.out.println("prodType-------"+prodType);
             ...
             { double epfee=0;
                ...
                newList.add(CommonUtil.convertObjToDouble(map.get("DUE_AMT")));//principal_due
                newList.add(CommonUtil.convertObjToDouble(map.get("PENAL_AMT")));
                newList.add(CommonUtil.convertObjToDouble("0"));
                double chargeMDS=0.0;
                chargeMDS=CommonUtil.convertObjToDouble(map.get("CHARGE"));
                newList.add(chargeMDS);
                newList.add(CommonUtil.convertObjToDouble(map.get("ARC_FEE")));
                newList.add((Date) (map.get("INSPECTOR_ORDER_DATE")));//7
                newList.add(CommonUtil.convertObjToDouble(map.get("APP_FEE")));//8
                newList.add(CommonUtil.convertObjToDouble(map.get("SALES_FEE")));//9
                newList.add(CommonUtil.convertObjToDouble(map.get("POST_FEE")));//10
                 epfee=CommonUtil.convertObjToDouble(map.get("EP_FEE"));
                newList.add(CommonUtil.convertObjToStr(getRoundVal(epfee)));

                amt = CommonUtil.convertObjToDouble(map.get("DUE_AMT")) + CommonUtil.convertObjToDouble(map.get("PENAL_AMT"))
                         + chargeMDS
                        + CommonUtil.convertObjToDouble(map.get("ARC_FEE")) + epfee
                        + CommonUtil.convertObjToDouble(map.get("POST_FEE"))
                        + CommonUtil.convertObjToDouble(map.get("APP_FEE"))
                        + CommonUtil.convertObjToDouble(map.get("SALES_FEE"));
                newList.add(amt);
                newList.add((CommonUtil.convertObjToStr(map.get("FILE_NO"))));//11
                newList.add((CommonUtil.convertObjToStr(map.get("EP_ID"))));//11
                newList.add((CommonUtil.convertObjToStr(map.get("LOAN_EP_NO"))));
                data.add(newList);
            ...
            newList.add(CommonUtil.convertObjToStr(map.get("NAME")));
            newList.add(CommonUtil.convertObjToDouble(map.get("PRINCIPAL_DUE")));//principal_due
            if (editFlag.equals("N") && epWithPresentValues.equals("Y")) { //KD-3449
                String intr = getInterestCalc(CommonUtil.convertObjToStr(map.get("ACT_NUM")), whereMap, map);
                map.put("INT_DUE", intr);
                newList.add(CommonUtil.convertObjToDouble(intr));
                String penalintr = getPenalCalc(CommonUtil.convertObjToStr(map.get("ACT_NUM")), whereMap, map);
                map.put("PENAL", penalintr);
                newList.add(CommonUtil.convertObjToDouble(penalintr));
            } else {
            newList.add(CommonUtil.convertObjToDouble(map.get("INT_DUE")));
             //newList.add(getInterestCalc(CommonUtil.convertObjToStr(map.get("ACT_NUM")),CommonUtil.convertObjToStr(whereMap.get("OVER_DUE_DT"))));
            newList.add(CommonUtil.convertObjToDouble(map.get("PENAL")));
            }
            newList.add(CommonUtil.convertObjToDouble(map.get("CHARGES")));
            newList.add(CommonUtil.convertObjToDouble(map.get("ARC_FEE")));
            newList.add((Date) (map.get("INSPECTOR_ORDER_DATE")));//7
             newList.add(CommonUtil.convertObjToDouble(map.get("APP_FEE")));//8
              newList.add(CommonUtil.convertObjToDouble(map.get("SALES_FEE")));//9
            ...
            amt = CommonUtil.convertObjToDouble(map.get("PRINCIPAL_DUE")) + CommonUtil.convertObjToDouble(map.get("INT_DUE"))
                    + CommonUtil.convertObjToDouble(map.get("PENAL")) + CommonUtil.convertObjToDouble(map.get("CHARGES"))
                    + CommonUtil.convertObjToDouble(map.get("ARC_FEE")) + epfee
                    + CommonUtil.convertObjToDouble(map.get("POST_FEE"))
                    +CommonUtil.convertObjToDouble(map.get("SALES_FEE"))
                    +CommonUtil.convertObjToDouble(map.get("APP_FEE"));
        ...
        //System.out.println("@@@$$$ sizeList : "+data);
...
//public String getInterestCalc(String acct_num,String date)
//    {
//         String inter="0";
//          HashMap singleAuthorizeMapOpBal = new HashMap();
//           singleAuthorizeMapOpBal.put("ACCT_NUM", acct_num);
//           singleAuthorizeMapOpBal.put("CURR_DATE", date);
//           singleAuthorizeMapOpBal.put(CommonConstants.BRANCH_ID,  ProxyParameters.BRANCH_ID);
//           List aListOp= ClientUtil.executeQuery("getInterestForLoan", singleAuthorizeMapOpBal);
    ...
    private void setTblModel(CTable tbl, ArrayList tblData, ArrayList head) {
        ...
        TableModel tableModel = new TableModel(tblData, head) {
            public boolean isCellEditable(int rowIndex, int mColIndex) {
                if (mColIndex == 0 || mColIndex == 9 ||  mColIndex == 10 || mColIndex == 11 || mColIndex == 12  || mColIndex == 13 || mColIndex == 14 || mColIndex == 3 || mColIndex == 4 || mColIndex == 5 || mColIndex == 6 || mColIndex == 16) {
                    return true;
                } else {
                    return false;
                }
...
//            if (relationalOperator.equals("Or"))
    ...
    //KD-3449
    public String getInterestCalc(String acct_num, HashMap whereMap, HashMap map) {        
        Date filingDt ;
        List aListOp = null;
        if(whereMap.containsKey("FILING_DT") && whereMap.get("FILING_DT") != null){
            filingDt = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(whereMap.get("FILING_DT")));
        }else{
            filingDt = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(whereMap.get("TODAY_DT")));
        }      
        String behavesLike = CommonUtil.convertObjToStr(map.get("BEHAVES_LIKE"));
        Date opDate = getProperDateFormat(DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(map.get("ACCT_OPEN_DT")))); 
        String inter = "0";
        HashMap singleAuthorizeMapOpBal = new HashMap();
        try {          
            singleAuthorizeMapOpBal.put("ACCT_NUM", acct_num);
            singleAuthorizeMapOpBal.put("CURR_DATE", getProperDateFormat(filingDt));
            singleAuthorizeMapOpBal.put("OPEN_DATE", opDate);
            singleAuthorizeMapOpBal.put(CommonConstants.BRANCH_ID, ProxyParameters.BRANCH_ID);
            if(behavesLike.equalsIgnoreCase("OD")){ 
    ...
    public String getPenalCalc(String acct_num, HashMap whereMap,HashMap map) {
        //ACC OPEN DATE
        Date filingDt = null;
        String behavesLike = CommonUtil.convertObjToStr(map.get("BEHAVES_LIKE"));
        if(whereMap.containsKey("FILING_DT") && whereMap.get("FILING_DT") != null){
        ...
        try {
            ...
            singleAuthorizeMapOpBal.put(CommonConstants.BRANCH_ID, ProxyParameters.BRANCH_ID);
            if(behavesLike.equalsIgnoreCase("OD")){ 
              aListOp = ClientUtil.executeQuery("getAdvanceEPPenal", singleAuthorizeMapOpBal); 
            }else{
              aListOp = ClientUtil.executeQuery("getLoanEPPenal", singleAuthorizeMapOpBal);
            }
            if (aListOp.size() > 0 && aListOp.get(0) != null) {
                HashMap mapop = (HashMap) aListOp.get(0);
                inter = CommonUtil.convertObjToStr(mapop.get("PENAL"));
            }
        } catch (Exception e) {
     ...
     public Date getProperDateFormat(Object obj) {
        Date currDate = null;
        Date curDt = ClientUtil.getCurrentDate();
        if (obj!=null && obj.toString().length()>0) {
            Date tempDt= DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(obj));
            currDate=(Date)curDt.clone();
