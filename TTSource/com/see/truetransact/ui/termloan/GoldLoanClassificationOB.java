...
public class GoldLoanClassificationOB extends CObservable{
    
    /** Creates a new instance of TermLoanClassificationOB */
    public GoldLoanClassificationOB() throws Exception{
//        termLoanClassificationOB();
        curDate = ClientUtil.getCurrentDate();
    }
    
    private final static Logger log = Logger.getLogger(GoldLoanClassificationOB.class);
    ...
    private boolean rdoPriority = false;
    private boolean rdoNonPriority = false;    
...
//    public static GoldLoanClassificationOB getInstance() {
...
//        setRdoDirectFinance(false)
        ...
        setChkQIS(false);
        setRdoNonPriority(false);
        setRdoPriority(false);
        setCboCommodityCode("");
        setCboSectorCode1("");
        setCboPurposeCode("");
        setCboIndusCode("");
    ...
    public void setTermLoanClassificationTO(TermLoanClassificationTO termLoanClassificationTO){
        try{
            ...
            if (CommonUtil.convertObjToStr(termLoanClassificationTO.getPrioritySector()).equals(YES)){
                setRdoPriority(true);
            }else if (CommonUtil.convertObjToStr(termLoanClassificationTO.getPrioritySector()).equals(NO)){
                setRdoNonPriority(true);
            }
    ...
    public TermLoanClassificationTO setTermLoanClassification(){
        ...
        try{
            ...
            if(getRdoPriority() == true){
                termLoanClassificationTO.setPurpose(YES);
                termLoanClassificationTO.setPrioritySector(YES);
            }else if(getRdoNonPriority() == true){
                termLoanClassificationTO.setPurpose(NO);
                termLoanClassificationTO.setPrioritySector(NO);
            }
    ...
    public void populateClassiDetailsFromProd(){
        try{
            ...
            if (resultList.size() > 0){
                ...
                if (CommonUtil.convertObjToStr(retrieve.get("PURPOSE")).equals(YES)){
                    setRdoPriority(true);
                }else{
                    setRdoNonPriority(true);
                }                
...
//                        ttNotifyObservers();
    ...
    /**
     * Getter for property rdoPriority.
     * @return Value of property rdoPriority.
     */
    public boolean getRdoPriority() {
        return rdoPriority;
    }
    
    /**
     * Setter for property rdoPriority.
     * @param rdoPriority New value of property rdoPriority.
     */
    public void setRdoPriority(boolean rdoPriority) {
        this.rdoPriority = rdoPriority;
    }
    
    /**
     * Getter for property rdoNonPriority.
     * @return Value of property rdoNonPriority.
     */
    public boolean getRdoNonPriority() {
        return rdoNonPriority;
    }
    
    /**
     * Setter for property rdoNonPriority.
     * @param rdoNonPriority New value of property rdoNonPriority.
     */
    public void setRdoNonPriority(boolean rdoNonPriority) {
        this.rdoNonPriority = rdoNonPriority;
    }
