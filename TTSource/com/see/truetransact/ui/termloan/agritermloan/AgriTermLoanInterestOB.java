...
import com.see.truetransact.transferobject.termloan.agritermloan.AgriTermLoanInterestTO;
...
public class AgriTermLoanInterestOB extends CObservable{
    
    /** Creates a new instance of TermLoanInterestOB */
    private AgriTermLoanInterestOB() throws Exception{
    ...
    private final static Logger log = Logger.getLogger(AgriTermLoanInterestOB.class);
    ...
    private AgriTermLoanInterestTO existAgriTermLoanInterestTO=null;
    ...
    static {
        try {
            termLoanInterestOB = new AgriTermLoanInterestOB();
        } catch(Exception e) {
            log.info("try: " + e);
    ...
    public static AgriTermLoanInterestOB getInstance() {
    ...
    public void setTermLoanInterestTO(ArrayList interestList, String acctNum){
        try{
            AgriTermLoanInterestTO  termLoanInterestTO;
            ...
            for (int i = interestList.size() - 1,j = 0;i >= 0;--i,++j){
                termLoanInterestTO = (AgriTermLoanInterestTO) interestList.get(j);
    ...
    private void setMax_Del_Interest_No(String acctNum){
        try{
            ...
            List resultList = ClientUtil.executeQuery("getSelectAgriTermLoanInterestMaxSLNO", transactionMap);
    ...
    private void setTermLoanInterestTO(HashMap eachRecs){
        ...
        existAgriTermLoanInterestTO=new AgriTermLoanInterestTO();
        existAgriTermLoanInterestTO.setFromDt(DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(eachRecs.get(FROM_DATE))));
        existAgriTermLoanInterestTO.setToDt(DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(eachRecs.get(TO_DATE))));
        existAgriTermLoanInterestTO.setInterest(CommonUtil.convertObjToDouble(eachRecs.get(INTEREST)));
        existAgriTermLoanInterestTO.setPenalInterest(CommonUtil.convertObjToDouble(eachRecs.get(PENALTY_INTEREST)));
        existAgriTermLoanInterestTO.setAgainstClearingInt(CommonUtil.convertObjToDouble(eachRecs.get(AGAINST_CLEAR_INT)));
        existAgriTermLoanInterestTO.setFromAmt(CommonUtil.convertObjToDouble(eachRecs.get(FROM_AMOUNT)));
        existAgriTermLoanInterestTO.setToAmt(CommonUtil.convertObjToDouble(eachRecs.get(TO_AMOUNT)));
    }
    public HashMap setTermLoanInterest(){
        HashMap interestMap = new HashMap();
        try{
            AgriTermLoanOB termLoanOB = AgriTermLoanOB.getInstance();
            ...
            if (!(termLoanOB.getCbmIntGetFrom().getKeyForSelected().equals("PROD") || termLoanOB.getCbmIntGetFrom().getKeyForSelected().equals(""))){
                ...
                for (int i = keySet.size() - 1,j = 0;i >= 0;--i,++j){
                    oneRecord = (HashMap) interestAll.get(objKeySet[j]);
                    objTermLoanInterestTO = new AgriTermLoanInterestTO();
                    ...
                    objTermLoanInterestTO.setRetCreateDt(DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(oneRecord.get("RET_CREATE_DT"))));
    ...
    public void setValByProdID(){
        try{
            ...
            List resultList = (List) ClientUtil.executeQuery("getAgriProdIntDetails", transactionMap);
        ...
        try{
            ...
            if(keySet.size() > 0){
                for (int i = keySet.size() - 1,j = 0;i >= 0;--i,++j){
                    ...
                    //                    if(j==0)
                    intiateDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(eachRecs.get("FROM_DATE")));
                ...
                if (!update) {
                    ...
                    for(int a=(interestTabValues.size()-1);a>(-1);a--){
                        ...
                        if(a==(interestTabValues.size()-1))
                            fromDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(selectList.get(1)));
                        strtoDt=CommonUtil.convertObjToStr(selectList.get(2));
                        if(!strtoDt.equals(""))
                            toDt=DateUtil.getDateMMDDYYYY(strtoDt);
                        ...
                        if(currFromDt !=null){
                            currFromDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(selectList.get(1)));
    ...
    private boolean checkDuplicateRecord(){
        ...
        if(tabList !=null && tabList.size()>0){
            Date fromDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(interestEachTabRecord.get(1)));
            for(int i=0;i<tabList.size();i++){
                double toAmt=0;
                ArrayList singleList=(ArrayList)tabList.get(i);
                if(singleList !=null && singleList.size()>0){
                    Date tblfromDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(singleList.get(1)));
    ...
    //    /* check wheather  max amount range hit or not
    //     */
    //     public void maxAmountRangeCheck(){
    //         HashMap eachRecs=new HashMap();
    //         LinkedHashMap totInterestLinkMap=new LinkedHashMap();
    ...
    //                 System.out.println("one by one "+eachRecs);
    //
    //
    //                 Date fromDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr((eachRecs.get(FROM_DATE))));
    //                 Double fromAmt=CommonUtil.convertObjToDouble(eachRecs.get(FROM_AMOUNT));
    //                 Double toAmt=CommonUtil.convertObjToDouble(eachRecs.get(TO_AMOUNT));
    //                 Date newRecordFromDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(interestEachRecord.get(FROM_DATE)));
    //                 if(previusFromDt ==null || (fromDt.equals(previusFromDt))){
    //                     totInterestLinkMap.put(
    //
    ...
    //
    //                 }
    //                 if(toAmt.equals(maxAmt)){
    //                     maxAmtStatus=true;
    //                 }
    //                 }
    //                 previusFromDt=fromDt;
    //             }
    //     }
    //     }
    ...
    //to setting the next slab list
     /* action type set as true while deleteing record other wise false
      */
    ...
    public boolean slabBasedEnableDisable(boolean actionTypeDelete){
        ...
        Date fromDt=DateUtil.getDateMMDDYYYY(tdtFrom);
        ...
        if(interestAll !=null && interestAll.size()>=1){
            ...
            for(int i=0;i<interestAll.size();i++){
                ...
                Date mapDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(tempto.get("FROM_DATE")));
    ...
    public void validateSlabRate(int recordPosition){
        ...
        //        //        boolean amt=false;
        if(existAgriTermLoanInterestTO !=null){
            if(interestAll!=null){
                existFromDt =existAgriTermLoanInterestTO.getFromDt();
                existFromDt=DateUtil.addDays(existFromDt, -1);
                changeFromDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(interestEachTabRecord.get(1)));
                existingToAmt=existAgriTermLoanInterestTO.getToAmt().doubleValue();
                changeToAmt=CommonUtil.convertObjToDouble(interestEachTabRecord.get(4)).doubleValue();
                if(existingToAmt!=changeToAmt){
                    modifyList(existAgriTermLoanInterestTO.getFromDt(),changeToAmt,existingToAmt,null,null,recordPosition);
                }
                if(DateUtil.dateDiff(existAgriTermLoanInterestTO.getFromDt(),changeFromDt)!=0){
                    modifyList(existAgriTermLoanInterestTO.getFromDt(),0,0,existFromDt,changeFromDt,recordPosition);
                }
            }
        }
    }
    ...
    public void  modifyList(Date roiDate,double Amt,double existAmt ,Date existFromDt,Date changeDt,int recordPosition){
        if(interestAll !=null ){
            ...
            for(int i=0;i<interestAll.size();i++){
                ...
                Date fromDt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(tempMap.get("FROM_DATE")));
                Date toDate=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(tempMap.get(TO_DATE)));
    ...
    void dynamicInterestDetails(HashMap singleMap,ArrayList oneList,Object dynamicStart_dt,int newReocordPos){
        if(!(singleMap.get("INT_ACTIVE_STATUS")).equals("N")){
            ...
            Date from_dt=DateUtil.getDateMMDDYYYY(tdtFrom);
            ...
            tempHashMap.put(AUTHORIZE_STATUS, "");
    ...
    private List insertInterest(int recordPosition){
        ...
        map.put(FROM_DATE,DateUtil.getDateMMDDYYYY(getTdtFrom()));
        return  lst=ClientUtil.executeQuery("getSelectAgriTermLoanAcctInterest",map);
    }
    ...
    private void updateToDate(int selectedRow){
        ...
        Date from_dt=null;
        from_dt=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(singleList.get(1)));
        from_dt=DateUtil.addDays(from_dt, -1);
        java.util.Set set=interestAll.keySet();
        Object objKeySet[]=(Object[])set.toArray();
        for(int i=totTabList.size()-1;i>=0;i--){
            ...
            if(selectedRow>i){
                Date toDate=DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(singleList.get(2)));
