...
import com.see.truetransact.transferobject.termloan.agritermloan.AgriTermLoanInstallmentTO;
import com.see.truetransact.transferobject.termloan.agritermloan.AgriTermLoanInstallMultIntTO;
import com.see.truetransact.transferobject.termloan.agritermloan.AgriTermLoanRepaymentTO;
...
public class AgriTermLoanRepaymentOB extends CObservable{
    
    /** Creates a new instance of TermLoanRepaymentOB */
    private AgriTermLoanRepaymentOB()  throws Exception{
    ...
    private final static Logger log = Logger.getLogger(AgriTermLoanRepaymentOB.class);
    ...
    static {
        try {
            termLoanRepaymentOB = new AgriTermLoanRepaymentOB();
        } catch(Exception e) {
            log.info("try: " + e);
    ...
    public static AgriTermLoanRepaymentOB getInstance() {
    ...
    public void setTermLoanRepaymentTO(ArrayList repaymentList, String acctNum){
        try{
            ...
            setActiveLoanAmt(0.0);
            AgriTermLoanRepaymentTO termLoanRepaymentTO;
            ...
            for (int i = repaymentList.size() - 1,j = 0;i >= 0;--i,++j){
                termLoanRepaymentTO = (AgriTermLoanRepaymentTO) repaymentList.get(j);
                ...
                repaymentRecordMap.put(FROM_DATE, DateUtil.getStringDate(termLoanRepaymentTO.getFirstInstallDt()));
    ...
    private void setMax_Del_Schedule_No(String act_Num){
        try{
            ...
            List resultList = ClientUtil.executeQuery("getSelectAgriTermLoanRepaymentMaxSLNO", transactionMap);
    ...
    public HashMap setTermLoanRepayment(){
        ...
        try{
            ...
            AgriTermLoanRepaymentTO termLoanRepaymentTO;
            AgriTermLoanInstallmentTO termLoanInstallmentTO;
            AgriTermLoanInstallMultIntTO termLoanInstallMultIntTO;
            ...
            for (int i = keySet.size() - 1,j = 0;i >= 0;--i,++j){
                oneRecord = (HashMap) repaymentAll.get(objKeySet[j]);
                termLoanRepaymentTO = new AgriTermLoanRepaymentTO();
...
//                termLoanRepaymentTO.setFirstInstallDt((Date)oneRecord.get(FROM_DATE));
                termLoanRepaymentTO.setFirstInstallDt(DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(oneRecord.get(FROM_DATE))));
...
//                termLoanRepaymentTO.setDisbursementDt((Date)oneRecord.get(DISBURSEMENT_DT));
                ...
                if (oneRecord.get(INSTALLMENT_DETAILS) != null){  // && (oneRecord.get(COMMAND).equals(INSERT))  //Condition removed to insert in update mode also
                    ...
                    for (int m = noOfInstallment - 1;m >= 0;--m){
                        ...
                        termLoanInstallmentTO = new AgriTermLoanInstallmentTO();
...
//                        termLoanInstallmentTO.setInstallmentDt((Date)interestOneRecord.get(INSTALLMENT_DATE));
                        termLoanInstallmentTO.setInstallmentDt(DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(interestOneRecord.get(INSTALLMENT_DATE))));
                        ...
                        if (interestOneRecord.containsKey(MULTIPLE_INTEREST)){
                            ...
                            for (int n = installMultiIntList.size()-1;n>=0;--n){
                                ...
                                termLoanInstallMultIntTO = new AgriTermLoanInstallMultIntTO();
...
//                termLoanRepaymentTO.setDisbursementDt((Date)oneRecord.get(DISBURSEMENT_DT));
    ...
    public void setFirstInstallDate(){
        try{
            ...
            List resultList        = ClientUtil.executeQuery("getAgriFacilityExpDate", transactionMap);
            if (resultList.size() > 0){
    ...
    public Date chkLastRepayDate(int tableCount){
        ...
        try{
            HashMap lastRecord = (HashMap) repaymentAll.get(((ArrayList) tblRepaymentTab.getDataArrayList().get(tableCount-1)).get(0));
            lastRepayDate = DateUtil.getDateMMDDYYYY(CommonUtil.convertObjToStr(lastRecord.get(TO_DATE)));
        }catch(Exception e){
...
//                }
    ...
    private void insertRepayment(int recordPosition){
        ...
        repaymentEachRecord.put(FROM_DATE, getTdtFirstInstall());
        ...
        try{
            ...
            for (int i = keySet.size() - 1, j = 0;i >= 0;--i, ++j){
                if ((CommonUtil.convertObjToStr(((HashMap) repaymentAll.get(objKeySet[j])).get(SLNO))).equals(strRecordKey)){
                    ...
                    setTdtFirstInstall(CommonUtil.convertObjToStr(eachRecs.get(FROM_DATE)));
