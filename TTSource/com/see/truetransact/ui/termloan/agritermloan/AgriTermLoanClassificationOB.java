...
public class AgriTermLoanClassificationOB extends CObservable{
    
    /** Creates a new instance of TermLoanClassificationOB */
    private AgriTermLoanClassificationOB() throws Exception{
    ...
    private final static Logger log = Logger.getLogger(AgriTermLoanClassificationOB.class);
    ...
    private boolean rdoPrioritySector = false;
    ...
    static {
        try {
            termLoanClassificationOB = new AgriTermLoanClassificationOB();
        } catch(Exception e) {
            log.info("try: " + e);
    ...
    public static AgriTermLoanClassificationOB getInstance() {
...
//        setChkPrioritySector(false);
    ...
    public void setTermLoanClassificationTO(AgriTermLoanClassificationTO  termLoanClassificationTO){
        try{
...
//                setChkPrioritySector(false);
    ...
    public AgriTermLoanClassificationTO setTermLoanClassification(){
        final AgriTermLoanClassificationTO termLoanClassificationTO = new AgriTermLoanClassificationTO();
        try{
    ...
    public void populateClassiDetailsFromProd(){
        try{
            ...
            java.util.List resultList = ClientUtil.executeQuery("getAgriProdClassiDetails", transactionMap);
...
//                    setChkPrioritySector(false);
    ...
    public void updateAssetStatus(){
        ...
        if(actNum !=null)
            if(! actNum.equals("")){
                String prevAssetStatus=getCboAssetCode();
                Date prevDate=null;
                if(getTdtNPADate()!=null && getTdtNPADate().length()>0)
                    prevDate = DateUtil.getDateMMDDYYYY(getTdtNPADate());
                else
                    prevDate = new Date();
                ...
                setCboAssetCode(CommonUtil.convertObjToStr(getCbmAssetCode().getSelectedItem()));//tKeyForSelected()));
                java.util.HashMap assetMap =new java.util.HashMap();
                
                assetMap.put("ACT_NUM",actNum);
                assetMap.put("ASSET_STATUS",CommonUtil.convertObjToStr(getCbmAssetCode().getKeyForSelected()));//getCboAssetCode());
                ...
                if(lst.size()>0){
                    ...
                    if(getTdtNPAChangeDt() !=null && getTdtNPAChangeDt().length()>0){
                        java.util.Date changeDt = DateUtil.getDateMMDDYYYY(getTdtNPAChangeDt());
...
//    }
     ...
     */
    public boolean isRdoDirectFinance() {
    ...
    /**
     * Getter for property rdoPrioritySector.
     * @return Value of property rdoPrioritySector.
     */
    public boolean isRdoPrioritySector() {
        return rdoPrioritySector;
    }
    
    /**
     * Setter for property rdoPrioritySector.
     * @param rdoPrioritySector New value of property rdoPrioritySector.
     */
    public void setRdoPrioritySector(boolean rdoPrioritySector) {
        this.rdoPrioritySector = rdoPrioritySector;
    }
