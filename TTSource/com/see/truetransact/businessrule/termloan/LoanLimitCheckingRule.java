 ...
 * @author  152713
 */
public class LoanLimitCheckingRule extends ValidationRule{
    private SqlMap sqlMap = null;
    /** Creates a new instance of LoanLimitCheckingRule */
    public LoanLimitCheckingRule() throws Exception{
    ...
    private void getTarget(HashMap inputMap) throws Exception{
         ...
         */
        List list = null;
        // For Corporate Loan purpose changes made by Rajesh
        if (inputMap.containsKey("BEHAVES_LIKE") && CommonUtil.convertObjToStr(inputMap.get("BEHAVES_LIKE")).equals("CORP_LOAN")) {
            list = (List) sqlMap.executeQueryForList("getCorpTLBalanceForTrench", inputMap);
        } else {
            list = (List) sqlMap.executeQueryForList("getTLBalance", inputMap);
        }
...
//            if (availableBalance==0 && loanBalancePrincipal > 0 )
            
            if (inputMap.get(TransactionDAOConstants.TO_STATUS).equals(CommonConstants.STATUS_CREATED) && multipleDisbursementAllowed.equals("N")&& CommonUtil.convertObjToStr(inputMap.get("DEBIT_LOAN_TYPE")).equals("DP")){
                List debitChkList = sqlMap.executeQueryForList("getDisbursementDetailsTL", inputMap);
